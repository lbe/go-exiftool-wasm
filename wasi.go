...
// mem(), which re-fetches the slice on each call to avoid stale references
// after memory growth.
//
// Single-owner invariant:
//   - A wasiState is owned by exactly one eval goroutine for its entire
...
func (w *wasiState) assertSingleOwner() {
	if !w.assertOwner {
		return
	}
	...
	if w.ownerGID != gid {
		panic(fmt.Sprintf("wasiState single-owner invariant violated: owner goroutine=%d current goroutine=%d", w.ownerGID, gid))
	}
}
...
func currentGID() uint64 {
	...
	line := string(b[:n])
	// runtime.Stack begins with: "goroutine <id> ["
...
func (w *wasiState) readBytes(ptr int32, length int32) []byte {
	if ptr == 0 || length == 0 {
		return nil
	}
	mem := w.mem()
	return mem[ptr : ptr+length]
}
...
func (w *wasiState) Xfd_read(fd int32, iovsPtr int32, iovsCount int32, nreadPtr int32) int32 {
	...
	for i := int32(0); i < iovsCount; i++ {
		...
		var n int
		var err error
		// Use ReadAt for files that support it (osFile). Xfd_write uses
		// WriteAt with entry.offset which does NOT advance the kernel file
		// position. Using Read() after WriteAt would read from the stale
		// kernel position instead of entry.offset.
		...
		w.logTrace("fd_read fd=%d read %d bytes at offset %d", fd, n, entry.offset-int64(n))
		if err != nil {
			if err == io.EOF {
				break
			}
...
func (w *wasiState) Xfd_seek(fd int32, offset int64, whence int32, newOffsetPtr int32) int32 {
	...
	// Update entry.offset to stay in sync. Xfd_write uses WriteAt with
	// entry.offset and Xfd_tell returns entry.offset, so this MUST be kept
	// current after every seek. Without this, writes after seek go to the
	// wrong position and tell() returns stale values.
	entry.offset = n
	w.fds[fd] = entry
...
func (w *wasiState) Xfd_tell(fd int32, offsetPtr int32) int32 {
	...
	// Return entry.offset directly. Xfd_write uses WriteAt with entry.offset
	// which does NOT update the kernel file position. Using s.Seek(0, SeekCurrent)
	// would return the stale kernel position, not the WASI-level offset.
	binary.LittleEndian.PutUint64(w.mem()[offsetPtr:], uint64(entry.offset))
	return int32(wasiESuccess)
}
