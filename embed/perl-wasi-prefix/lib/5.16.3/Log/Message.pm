...
sub new {
    ...
    my $conf = new Log::Message::Config( %hash, default => $CONFIG )
      or return undef;
...
sub _new_stack {
    ...
    my $tmpl = {
        stack  => { default => [] },
        config => {
            default     => bless( {}, 'Log::Message::Config' ),
            required    => 1,
            strict_type => 1
        },
    };
    ...
    my $args = check( $tmpl, \%hash, $CONFIG->verbose )
      or (
        warn(
            loc(
                q[Could not create a new stack object: %1],
...
sub _get_conf {
    ...
    return
        defined $self->{CONFIG}->$what() ? $self->{CONFIG}->$what()
      : defined $CONFIG->$what()         ? $CONFIG->$what()
      :                                    undef;
}
