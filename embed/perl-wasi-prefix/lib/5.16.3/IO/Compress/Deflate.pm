...
sub mkDeflateHdr($$$;$) {
    ...
    $flg =
      bitmask( $flg, $fcheck, ZLIB_FLG_FCHECK_OFFSET, ZLIB_FLG_FCHECK_BITS );
...
sub mkHeader {
    ...
    if ( ZLIB_VERNUM >= 0x1210 ) {
        ...
        elsif ( $level < 6 )  { $lflag = ZLIB_FLG_LEVEL_FAST }
        elsif ( $level == 6 ) { $lflag = ZLIB_FLG_LEVEL_DEFAULT }
        else                  { $lflag = ZLIB_FLG_LEVEL_SLOWEST }
    }
    else {
        $lflag = ( $level - 1 ) >> 1;
        $lflag = 3 if $lflag > 3;
