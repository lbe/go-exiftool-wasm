...
$needBinmode =
  ( $^O eq 'MSWin32'
      || ( $] >= 5.006 && eval ' ${^UNICODE} || ${^UTF8LOCALE} ' ) ) ? 1 : 1;
...
sub isaFilehandle($) {
    use utf8;
    return (
        defined $_[0] and ( UNIVERSAL::isa( $_[0], 'GLOB' )
...
sub IO::Compress::Base::Validator::new {
    ...
    if ( $outType eq 'filename' ) {
        $obj->croakError(
            "$reportClass: output filename is undef or null string")
          if !defined $_[1] || $_[1] eq '';

        if ( -e $_[1] ) {
            if ( -d _ ) {
                return $obj->saveErrorString(
                    "output file '$_[1]' is a directory");
            }
        }
    }
...
sub IO::Compress::Base::Validator::validateInputFilenames {
    ...
    foreach my $filename (@_) {
        $self->croakError(
            "$self->{reportClass}: input filename is undef or null string")
        ...
        if ( -d _ ) {
            return $self->saveErrorString(
                "input file '$filename' is a directory");
        }
...
sub ParseParameters {
    ...
    return $_[1]
      if @_ == 2
      && defined $_[1]
      && UNIVERSAL::isa( $_[1], "IO::Compress::Base::Parameters" );
...
sub IO::Compress::Base::Parameters::_checkType {
    ...
    if ( $type & Parse_any ) {
        ...
        return $self->setError(
            "Parameter '$key' must be a code reference, got '$value'")
