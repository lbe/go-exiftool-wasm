...
sub parseRawExtra {
    ...
    while ( $offset < $XLEN ) {
        ...
        my $subLen =
          unpack( "v",
            substr( $data, $offset, GZIP_FEXTRA_SUBFIELD_LEN_SIZE ) );
        ...
        my $bad =
          validateExtraFieldPair( [ $id, substr( $data, $offset, $subLen ) ],
            $strict, $gzipMode );
        return $bad if $bad;
    ...
    if ( ref $data eq 'ARRAY' ) {
        if ( ref $data->[0] ) {
            ...
            for ( my $ix = 0 ; $ix <= @$data - 1 ; $ix += 2 ) {
                my $bad =
                  validateExtraFieldPair( [ $data->[$ix], $data->[ $ix + 1 ] ],
                    $strict, $gzipMode );
                return $bad if $bad;
        ...
        while ( my ( $id, $info ) = each %$data ) {
            my $bad =
              validateExtraFieldPair( [ $id, $info ], $strict, $gzipMode );
            return $bad if $bad;
