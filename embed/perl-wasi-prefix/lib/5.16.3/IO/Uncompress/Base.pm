...
sub smartRead {
    ...
    if ( defined *$self->{FH} ) {
        ...
        else { $status = *$self->{FH}->read( $$out, $get_size ) }
    }
        ...
        *$self->{EventEof} = 1 if $got <= 0;
        ...
        substr( $$out, $offset ) =
          substr( $$buf, *$self->{BufferOffset}, $get_size );
        if ( *$self->{ConsumeInput} ) { substr( $$buf, 0, $get_size ) = '' }
        else { *$self->{BufferOffset} += length($$out) - $offset }
    }
    ...
    if ( defined *$self->{FH} || defined *$self->{InputEvent} ) {
        ...
        *$self->{InputLengthRemaining} += length( $_[0] );
        ...
        if ( $len > *$self->{BufferOffset} ) {
            *$self->{Prime} =
              substr( $_[0], 0, $len - *$self->{BufferOffset} )
              . *$self->{Prime};
...
sub smartSeek {
    ...
    if ( defined *$self->{FH} ) { *$self->{FH}->seek( $offset, $position ) }
    else {
...
sub smartTell {
    ...
    if   ( defined *$self->{FH} ) { return *$self->{FH}->tell() }
    else                          { return *$self->{BufferOffset} }
}
...
sub smartEof {
    ...
    elsif ( defined *$self->{InputEvent} ) { *$self->{EventEof} }
    else { *$self->{BufferOffset} >= length( ${ *$self->{Buffer} } ) }
}
...
sub TrailerError {
    my ($self) = shift;
    return $self->saveErrorString( G_ERR, "Trailer Error: $_[0]",
        STATUS_ERROR );
}
...
sub ckInputParam {
    ...
    $self->croakError(
"$from: input parameter not a filename, filehandle, array ref or scalar ref"
    ) if !$inType;
...
sub _inf {
    ...
    my $x =
      new IO::Compress::Base::Validator( $class, *$obj->{Error}, $name, $input,
        $output )
      or return undef;
...
sub _rd2 {
    ...
    while (1) {
        ...
        while ( ( $status = $z->read( $x->{buff} ) ) > 0 ) {
            if ($fh) {
                syswrite $fh, ${ $x->{buff} }
                  or return $z->saveErrorString( undef,
        ...
        if ( !$x->{oneOutput} ) {
            ...
            if ( $ot eq 'array' ) { push @$output, $x->{buff} }
            elsif ( $ot eq 'hash' ) { $output->{$input} = $x->{buff} }
...
sub readBlock {
    ...
    if ( $status == 0 ) {
        *$self->{Closed}    = 1;
        *$self->{EndStream} = 1;
        return $self->saveErrorString( STATUS_ERROR, "unexpected end of file",
            STATUS_ERROR );
    }
...
sub _raw_read {
    ...
    if ( $status == STATUS_OK ) {
        ...
        $status =
          *$self->{Uncomp}->uncompr( \$temp_buf, $buffer,
            defined *$self->{CompressedInputLengthDone} || $self->smartEof(),
            $outSize );
...
sub gotoNextStream {
    ...
    if ( !*$self->{NewStream} ) {
        ...
        $status = $self->read($buffer) while $status > 0;
...
sub read {
    ...
    if ( ref $_[0] ) {
        $self->croakError(
            *$self->{ClassName} . "::read: buffer parameter is read-only" )
          if readonly( ${ $_[0] } );

        $self->croakError(
            *$self->{ClassName} . "::read: not a scalar reference $_[0]" )
    ...
    $self->croakError(
        *$self->{ClassName} . "::read: length parameter is negative" )
      if $length < 0;
...
sub getline {
    ...
    return undef
      if *$self->{Closed}
      || ( !length *$self->{Pending} && *$self->{EndStream} );
...
sub getlines {
    my $self = shift;
    $self->croakError(
        *$self->{ClassName} . "::getlines: called in scalar context\n" )
      unless wantarray;
    my ( $line, @lines );
...
sub eof {
    ...
    return (
        *$self->{Closed} || ( !length *$self->{Pending}
...
sub seek {
    ...
    if ( $whence == SEEK_SET ) {
        ...
        $target = $position;
        $self->croakError(
            *$self->{ClassName} . "::seek: SEEK_END not allowed" );
    }
    else {
    ...
    while (
        (
            $got =
            $self->read( my $buffer, min( $offset, *$self->{BlockSize} ) )
        ) > 0
      )
    {
        $offset -= $got;
