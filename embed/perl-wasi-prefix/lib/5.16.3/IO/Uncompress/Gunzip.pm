...
sub ckMagic {
    ...
    *$self->{HeaderPending} = $magic;
    ...
    return $self->HeaderError(
        "Minimum header size is " . GZIP_MIN_HEADER_SIZE . " bytes" )
...
sub chkTrailer {
    ...
    *$self->{Info}{ISIZE} = $ISIZE;
    ...
    if ( *$self->{Strict} ) {
        ...
        return $self->TrailerError(
            "ISIZE mismatch. Got $ISIZE" . ", expected $exp_isize" )
          if $ISIZE != $exp_isize;
    }
    ...
    *$self->{HeaderPending} = $keep;
    ...
    if ( $flag & GZIP_FLG_FEXTRA ) {
        ...
        if ( $XLEN && *$self->{'ParseExtra'} ) {
            my $bad =
              IO::Compress::Zlib::Extra::parseRawExtra( $EXTRA, \@EXTRA, 1, 1 );
