...

{
    ...
    sub provides {
        ...
        if ( length $args{prefix} ) { $args{prefix} =~ s{/$}{};
            for my $v ( values %$p ) {
    ...
    while ( defined( my $line = <$fh> ) ) {
        ...
        if ( $in_pod || $line =~ /^=cut/ ) {
            ...
            if ( $line =~ $PKG_REGEXP ) {
                ...
                $vers{$pkg} = ( defined $2 ? $2 : undef )
                  unless exists( $vers{$pkg} );
                ...
                unless ( defined $vers{$vers_pkg} && length $vers{$vers_pkg} ) {
                    $vers{$vers_pkg} =
                      $self->_evaluate_version_line( $vers_sig, $vers_fullname,
                        $line );
                }
                else {
