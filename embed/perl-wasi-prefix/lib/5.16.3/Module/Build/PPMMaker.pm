...
sub make_ppd {
    ...
    if ( exists $args{codebase} ) {
        @codebase =
          ref $args{codebase} ? @{ $args{codebase} } : ( $args{codebase} );
    }
    else {
    ...
    foreach my $info (qw(name author abstract version)) {
        ...
        $dist{$info} = $build->$method()
          or die "Can't determine distribution's $info\n";
    }
...
PPD
    ...
    foreach my $type (qw(requires)) {
        ...
        while ( my ( $modname, $spec ) = each %$prereq ) {
            ...
            $ppd .=
              qq!        <REQUIRE NAME="$modname" VERSION="$min_version" />\n!;
        }
    }
...
sub _ppd_version {
    ...
    return join ',', ( split( /\./, $version ), (0) x 4 )[ 0 .. 3 ];
}

sub _varchname { my ( $self, $config ) = @_;
