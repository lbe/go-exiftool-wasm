...
sub _mktime {
    ...
    $class =
      eval { ( ref $class ) && ( ref $class )->isa('Time::Piece') }
      ? ref $class
      : $class;
    if ( ref($time) ) {
        $time->[c_epoch] = undef;
        return wantarray ? @$time : bless [ @$time[ 0 .. 9 ], $islocal ],
          $class;
    }
    _tzset();
    my @time =
...
sub tzoffset {
    ...
    my $delta =
      24 * ( $j->( _crt_localtime($epoch) ) - $j->( _crt_gmtime($epoch) ) );

    return Time::Seconds->new(
        int( $delta * 60 + ( $delta >= 0 ? 0.5 : -0.5 ) ) * 60 );
}
...
sub hms {
    ...
    sprintf( "%02d$sep%02d$sep%02d",
        $time->[c_hour], $time->[c_min], $time->[c_sec] );
}
...
sub datetime {
    ...
    return
      join( $seps{T}, $time->date( $seps{date} ), $time->time( $seps{time} ) );
}
...
sub strptime {
    ...
    return
      scalar $time->_mktime( \@vals, ( ref($time) ? $time->[c_islocal] : 0 ) );
}
