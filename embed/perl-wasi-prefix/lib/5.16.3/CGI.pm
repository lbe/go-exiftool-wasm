...
use CGI::Util
  qw(rearrange rearrange_header make_attributes unescape escape expires ebcdic2ascii ascii2ebcdic);

use constant XHTML_DTD => [
...
/multipart\/related.+boundary=\"?([^\";,]+)\"?.+start=\"?\<?([^\"\>]+)\>?\"?/
              )
            {
                ...
                $self->add_parameter($param);
                my ($value) =
                  $self->read_multipart_related( $start, $boundary,
                    $content_length, 0 );
                push( @{ $self->{param}{$param} }, $value );
                if ($MOD_PERL) {
    ...
    my ( $pack, $func_name );
    {
        ...
        if ( !$code ) {
            ...
            if (
                ...
                || $EXPORT{$base}
                || ( %EXPORT_OK
                    || grep( ++$EXPORT_OK{$_}, &expand_tags(':html') ) )
                && $EXPORT_OK{$base}
              )
            {
...
sub find_tempdir {
    ...
    unless ( defined $TMPDIRECTORY ) {
        @TEMP = (
            "${SL}usr${SL}tmp", "${SL}var${SL}tmp",
            "C:${SL}temp",      "${SL}tmp",
            "${SL}temp",        "${vol}${SL}Temporary Items",
        ...
        if ( $CGI::OS eq 'WINDOWS' ) {
            ...
            unshift( @TEMP, $ENV{WINDIR} . $SL . 'TEMP' )
