...
sub DumpElem {
    ...
    elsif ( $self->{veryCompact} && ref $v && ( ref $v eq 'HASH' )
        and !grep( ref $_, values %$v ) )
    {
        my @a     = sort keys %$v;
        my $depth = $#a;
        ...
        my @b =
          map { $self->stringify($_) . " => " . $self->stringify( $$v{$_} ) }
          @a[ 0 .. $depth ];
        local $" = ', ';
        print "@b$shortmore\n";
    }
...
sub unwrap {
    ...
    if ( UNIVERSAL::isa( $v, 'HASH' ) ) {
        ...
        if ( $self->{compactDump} && !grep( ref $_, values %{$v} ) ) {
            ...
            for (@sortKeys) {
                push @keys,
                  $self->stringify($_) . " => " . $self->stringify( $v->{$_} );
            }
            $short .= join ', ', @keys;
            $short .= $shortmore;
            ( print "$short\n" ), return
        ...
        my $more;
        $tArrayDepth =
          $#$v < $self->{arrayDepth} - 1 ? $#$v : $self->{arrayDepth} - 1
