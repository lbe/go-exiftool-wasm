...
my $tmpl = [
    name => 0, mode => 1, uid => 1, gid => 1, size => 0, mtime => 1, chksum =>
      1, type => 0, linkname => 0, magic => 0, version => 0, uname =>
      0, gname => 0, devmajor => 1, devminor => 1, prefix => 0,
...
sub _prefix_and_file {
    ...
    my $prefix = File::Spec::Unix->catdir( grep { length } $vol, @dirs );
    return ( $prefix, $file );
}
