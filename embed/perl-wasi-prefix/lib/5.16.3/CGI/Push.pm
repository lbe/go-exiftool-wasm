...
sub do_push {
    ...
    my (@header);
    my (
        $type,   $callback, $delay, $last_page, $cookie,
        $target, $expires,  $nph,   @other
      )
      = rearrange(
        [
            TYPE, NEXT_PAGE, DELAY, LAST_PAGE,
            [ COOKIE, COOKIES ], TARGET, EXPIRES, NPH
        ],
        @p
      );
    ...
    print
"WARNING: YOUR BROWSER DOESN'T SUPPORT THIS SERVER-PUSH TECHNOLOGY.${boundary}$CGI::CRLF";
    ...
    print
"WARNING: YOUR BROWSER DOESN'T SUPPORT THIS SERVER-PUSH TECHNOLOGY.$CGI::CRLF";
}
...
sub push_delay {
    ...
    return defined($delay) ? $self->{'.delay'} = $delay : $self->{'.delay'};
}
