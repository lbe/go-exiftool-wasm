...
@EXPORT_OK =
  qw(carpout fatalsToBrowser warningsToBrowser wrap set_message set_die_handler set_progname cluck ^name= die);
...
sub carpout {
    ...
    open( SAVEERR, ">&STDERR" );
    open( STDERR,  ">&$no" )
      or ( print SAVEERR "Unable to redirect STDERR: $!\n" and exit(1) );
}
...
END
    ...
    if ($CUSTOM_MSG) {
        if ( ref($CUSTOM_MSG) eq 'CODE' ) {
            ...
            if ($@) {
                print STDERR q(error while executing the error handler: $@);
            }
