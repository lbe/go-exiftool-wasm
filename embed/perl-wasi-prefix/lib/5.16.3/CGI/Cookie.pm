...
sub new {
    ...
    shift
      if ref $params[0]
      && eval {
        $params[0]->isa('Apache::Request::Req') || $params[0]->isa('Apache');
      };
...
sub expires {
    my ( $self, $expires ) = @_;
    $self->{'expires'} = CGI::Util::expires( $expires, 'cookie' )
      if defined $expires;
    return $self->{'expires'};
}
...
sub max_age {
    my ( $self, $max_age ) = @_;
    $self->{'max-age'} = CGI::Util::expire_calc($max_age) - time()
      if defined $max_age;
    return $self->{'max-age'};
}
...
sub httponly { my ( $self, $httponly ) = @_;
    $self->{'httponly'} = $httponly if defined $httponly;
