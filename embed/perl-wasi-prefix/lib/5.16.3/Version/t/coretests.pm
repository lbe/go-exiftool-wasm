...
if ( $Test::More::VERSION < 0.48 ) { local $^W;
    *main::use_ok = sub ($;@) {
        my ( $pkg, $req, @args ) = @_;
...
sub BaseTests {
    ...
    unlike( $warning, qr/v-string without leading 'v' deprecated/,
        'No leading v' );
...

    {
      ...
      SKIP: {
            skip "version require'd instead of use'd, cannot test $qv_declare",
              3
        ...
        {
            my ( $fh, $filename ) =
              tempfile( 'tXXXXXXX', SUFFIX => '.pm', UNLINK => 1 );
      ...
      SKIP: { skip
"version require'd instead of use'd, cannot test UNIVERSAL::VERSION",
              2
              unless defined $qv_declare;
            my ( $fh, $filename ) =
              tempfile( 'tXXXXXXX', SUFFIX => '.pm', UNLINK => 1 );
            ...
            like(
                $@,
                qr/Invalid version format \(non-numeric data\)/,
...
EOF
  ...
  SKIP: {
        ...
        ok $CLASS->$method(1.2.3) < $CLASS->$method(1.2.3.1),
          'Compare 3 and 4 digit v-strings';
        ok $CLASS->$method(v1.2.3) < $CLASS->$method(v1.2.3.1),
