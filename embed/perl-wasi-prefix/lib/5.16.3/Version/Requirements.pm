...
sub _version_object {
    ...
    $version =
        ( !defined $version )                ? version->parse(0)
...
sub __modify_entry_for {
    ...
    my $new =
      ( $old || 'Version::Requirements::_Range::Range' )->$method($version);
...
sub as_string_hash {
    ...
    my %hash =
      map { ; $_ => $self->{requirements}{$_}->as_string }
...

{
    ...
    sub with_minimum {
        ...
        Carp::confess(
            "illegal requirements: minimum above exact specification");
    }
    ...
    sub with_maximum {
        ...
        Carp::confess(
            "illegal requirements: maximum below exact specification");
    }
...
{
    package Version::Requirements::_Range::Range;
    ...
    sub _clone {
        ...
        my %guts = (
            (
                exists $s->{minimum}
                ? ( minimum => version->new( $s->{minimum} ) )
                : ()
            ),
            (
                exists $s->{maximum}
                ? ( maximum => version->new( $s->{maximum} ) )
                : ()
            ),
            ...
            (
                exists $s->{exclusions}
                ? ( exclusions =>
                      [ map { version->new($_) } @{ $s->{exclusions} } ] )
                : ()
            ),
        );
    ...
    sub as_modifiers {
        my ($self) = @_;
        my @mods;
        push @mods, [ add_minimum => $self->{minimum} ]
          if exists $self->{minimum};
        push @mods, [ add_maximum => $self->{maximum} ]
          if exists $self->{maximum};
        push @mods,
          map { ; [ add_exclusion => $_ ] } @{ $self->{exclusions} || [] };
        return \@mods;
    }
    ...
    sub as_string {
        ...
        for my $pair ( [qw( >= > minimum )], [qw( <= < maximum )], ) {
            my ( $op, $e_op, $k ) = @$pair;
            if ( exists $self->{$k} ) {
    ...
    sub with_exact_version {
        ...
        Carp::confess(
            "illegal requirements: exact specification outside of range")
        ...
        if ( defined $self->{minimum} and defined $self->{maximum} ) {
            if ( $self->{minimum} == $self->{maximum} ) {
                Carp::confess("illegal requirements: excluded all values")
                  if grep { $_ == $self->{minimum} }
                  @{ $self->{exclusions} || [] };

                return Version::Requirements::_Range::Exact->_new(
                    $self->{minimum} );
            }
