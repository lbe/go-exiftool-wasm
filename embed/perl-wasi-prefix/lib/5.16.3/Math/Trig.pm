...
our @EXPORT = ( @{ $Math::Complex::EXPORT_TAGS{'trig'} }, @angcnv, @areal );
...
sub great_circle_distance {
    ...
    return $rho *
      acos_real(
        cos($lat0) * cos($lat1) * cos( $theta0 - $theta1 ) +
          sin($lat0) * sin($lat1) );
}
...
sub great_circle_destination {
    ...
    my $phi1 =
      asin_real( sin($lat0) * cos($dst) + cos($lat0) * sin($dst) * cos($dir0) );

    my $theta1 = $theta0 + atan2(
        sin($dir0) * sin($dst) * cos($lat0),
