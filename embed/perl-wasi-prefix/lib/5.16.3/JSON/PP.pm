...
BEGIN {
    ...
    if ( $] < 5.008 ) {
        my $helper =
          $] >= 5.006 ? 'JSON::PP::Compat5006' : 'JSON::PP::Compat5005';
...
sub encode_json ($) { ( $JSON ||= __PACKAGE__->new->utf8 )->encode(@_);
}

sub decode_json { ( $JSON ||= __PACKAGE__->new->utf8 )->decode(@_);
}
...
sub new {
    ...
    my $self  = {
        ...
        FLAGS     => 0,
        fallback =>
          sub { encode_error('Invalid value. JSON can only reference.') },
        indent_length => 3,
    };
...

{
    ...
    sub PP_encode_json {
        ...
        (
            $ascii,        $latin1,        $utf8,
            $indent,       $canonical,     $space_before,
            $space_after,  $allow_blessed, $convert_blessed,
            $escape_slash, $bignum,        $as_nonblessed
          )
          = @{$idx}[
...
"%s::TO_JSON method returned same object as was passed instead of a new one",
                ...
                return $self->blessed_to_json($obj)
...
"json text or perl structure exceeds maximum nesting level (max_depth set too low?)"
        ...
        for my $k ( _sort($obj) ) {
            if (OLD_PERL) { utf8::decode($k) } push @res,
...
"json text or perl structure exceeds maximum nesting level (max_depth set too low?)"
    ...
    sub value_to_json {
        ...
        return $value if $flags & ( B::SVp_IOK | B::SVp_NOK )
          and !( $flags & B::SVp_POK );
...
sub _encode_surrogates { my $uni = $_[0] - 0x10000;
...
"malformed JSON string, neither array, object, number, string or atom"
        ...
        if ($utf8) {
            utf8::downgrade( $text, 1 )
              or Carp::croak("Wide character in subroutine entry");
        }
        else {
            utf8::upgrade($text);
        }
...
"malformed JSON string, neither array, object, number, string or atom"
                ...
                if ( $relaxed and $ch eq '#' ) { pos($text) = $at;
                    $text =~ /\G([^\n]*(?:\r\n|\r|\n|$))/g;
                    $at = pos($text);
                    next_chr;
                    next;
...
'json text or perl structure exceeds maximum nesting level (max_depth set too low?)'
        ...
        if ( defined $ch and $ch eq '}' ) {
            ...
            while ( defined $ch ) {
                $k =
                  ( $allow_barekey and $ch ne '"' and $ch ne "'" )
                  ? bareKey()
                  : string();
                white();
...
"malformed number (leading zero must not be followed by another digit)"
        ...
        if ( $v !~ /[.eE]/ and length $v > $max_intsize ) {
            if ($allow_bigint) { require Math::BigInt;
    ...
    sub decode_error {
        ...
        Carp::croak( $no_rep
            ? "$error"
            : "$error, at character offset $at (before \"$mess\")" );
...
sub _decode_surrogates { my $uni =
      0x10000 + ( hex( $_[0] ) - 0xD800 ) * 0x400 + ( hex( $_[1] ) - 0xDC00 );
    my $un = pack( 'U*', $uni );
    utf8::encode($un);
    return $un;
}
...
BEGIN {
    ...
    if ( $] >= 5.008 and $] < 5.008003 ) { package JSON::PP;
        require subs;
        subs->import('join');
        eval q|
            sub join {
...
BEGIN {
    ...
    else {  eval 'sub UNIVERSAL::a_sub_not_likely_to_be_here { ref($_[0]) }';
        *JSON::PP::blessed = sub {
        ...
        if (wantarray) {
            ...
            do {
                ...
                unless ( !$self->{incr_nest}
                    and $self->{incr_mode} == INCR_M_JSON )
                {
