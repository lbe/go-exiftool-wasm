...
sub stringify {
    ...
    for ($__) {
        ...
        if ( $tick eq "'" ) {
            ...
            s/([\"\\\$\@])/\\$1/g if $tick eq '"';
            s/\033/\\e/g;
            if ( ord('A') == 193 ) { s/([\000-\037\177])/'\\c'.chr(193)/eg;
            }
            else {
...
sub ShortArray {
    my $tArrayDepth = $#{ $_[0] };
    $tArrayDepth =
      $#{ $_[0] } < $arrayDepth - 1 ? $#{ $_[0] } : $arrayDepth - 1
...
sub DumpElem {
    ...
    elsif ( $veryCompact && ref $_[0] && ( ref $_[0] eq 'HASH' )
        and !grep( ref $_, values %{ $_[0] } ) )
    {
        my $end = 1;
        $short =
            $sp
          . "0..$#{$v}  '"
          . join( "' '", @{$v}[ 0 .. $tArrayDepth ] )
          . "'$shortmore";
    }
    else {
        print "$short\n";
        ...
        if ( $compactDump && !grep( ref $_, @{$v} ) ) {
            if ( $#$v >= 0 ) {
                $short =
                  ...
                  . "0..$#{$v}  "
                  . join( " ",
                    map { exists $v->[$_] ? stringify $v->[$_] : "empty" }
                      ( 0 .. $tArrayDepth ) )
                  . "$shortmore";
            }
            else {
...
sub matchlex {
    ...
    $var eq $_[1]
      or ( $_[1] =~ /^([!~])(.)([\x00-\xff]*)/ )
      and ( $1 eq '!' ) ^ ( eval { $var =~ /$2$3/ } );
}

sub matchvar {
    $_[0] eq $_[1]
      or ( $_[1] =~ /^([!~])(.)([\x00-\xff]*)/ )
      and ( $1 eq '!' ) ^ ( eval { ( $_[2] . "::" . $_[0] ) =~ /$2$3/ } );
}
...
sub hashUsage { my @keys = keys %{ $_[0] };
