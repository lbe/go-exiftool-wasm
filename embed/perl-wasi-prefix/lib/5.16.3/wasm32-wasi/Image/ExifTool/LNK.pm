...
my %fileAttributes = (
    BITMASK => {
        ...
        1  => 'Hidden',
        2  => 'System',
        3  => 'Volume', 4 => 'Directory',
        5  => 'Archive',
        6  => 'Encrypted?', 7 => 'Normal',
        8  => 'Temporary',
        9  => 'Sparse',
        10 => 'Reparse point',
        11 => 'Compressed',
...
%Image::ExifTool::LNK::ItemID = (
    ...
    0x00         => [
        {
            Name => 'ControlPanelCPL', Condition =>
              '$$valPt =~ /^.{5}\xff{3}/s',
            SubDirectory =>
              { TagTable => 'Image::ExifTool::LNK::ControlPanelCPL' },
        },
        {
            Name => 'GameFolderInfo', Condition => '$$valPt =~ /^.{4}GFSI/s',
            SubDirectory =>
              { TagTable => 'Image::ExifTool::LNK::GameFolderInfo' },
        },
        {
            Name => 'PropertyStore', Condition =>
              '$$valPt =~ /^.{6}\xee\xbb\xfe\x23/',
            SubDirectory =>
              { TagTable => 'Image::ExifTool::LNK::PropertyStore' },
        },
        {
            Name => 'MTPType2', Condition =>
              '$$valPt =~ /^.{6}\\x05\x20\x31\x10/',
    ...
    0x01 => {
        ...
        SubDirectory =>
          { TagTable => 'Image::ExifTool::LNK::ControlPanelInfo' },
    },
    0x1e => { Name => 'RootFolder',
        SubDirectory => { TagTable => 'Image::ExifTool::LNK::RootFolder' }, },
    0x1f => {
        Name         => 'RootFolder',
        SubDirectory => { TagTable => 'Image::ExifTool::LNK::RootFolder' },
    },
    0x2e => { Name => 'VolumeGUID',
    ...
    0x74 => {
        ...
        SubDirectory =>
          { TagTable => 'Image::ExifTool::LNK::UsersFilesFolder' },
    },
    0xff => { Name => 'VendorData',
        ValueConv => q{
...
%Image::ExifTool::LNK::RootFolder = (
    ...
    0x03         => {
        Name      => 'SortIndex',
        PrintHex  => 1,
        PrintConv => { 0x00 => 'Internet Explorer',
            0x42 => 'Libraries',
            0x44 => 'Users',
            0x48 => 'My Documents',
            0x4c => 'Public Folder',
...
%Image::ExifTool::LNK::URI = (
    ...
    46 => {
        ...
        RawConv =>
          '$self->Decode($val, ($$self{URIFlags} & 0x80) ? "UTF16" : "Latin"',
        Hook => '$varSize += $$self{FTPHostLen} - 1', },
    47 => {
        Name      => 'FTPUserNameLen',
        Condition => '$$self{URIDataSize} > 0',
        Hidden    => 1,
...
%Image::ExifTool::LNK::Beef0004 = (
    ...
    18 => {
        Name   => 'TargetFileName',
        Format => 'undef[$size - 20 - $varSize]',  RawConv => q{
            my @a = $val =~ /((?:..)*?)\0\0/sg;
...
%Image::ExifTool::LNK::UsersFilesFolder = (
    ...
    0x04         => {
        ...
        Format  => 'int16u',
        RawConv => 'undef',
        Hook    => '$varSize += Get16u($dataPt, $pos)', },
    0x06 => {
...
%Image::ExifTool::LNK::INI = (
    ...
    HotKey           => {},
    ShowCommand =>
      { PrintConv => { 1 => 'Normal', 2 => 'Minimized', 3 => 'Maximized' } },
    Modified => {
        Groups  => { 2 => 'Time' },
        Format  => 'int64u',
        RawConv => q{
...
sub DOSTime($) {
    ...
    return sprintf(
        '%.4d:%.2d:%.2d %.2d:%.2d:%.2d',
        ( ( $val >> 9 ) & 0x7f ) + 1980, ( $val >> 5 ) & 0x0f, ( $val >> 0 ) &
          0x1f, ( $val >> 27 ) & 0x1f, ( $val >> 21 ) & 0x3f, ( $val >> 15 ) &
          0x3e );
}
...
sub ProcessItemID($$$) {
    ...
    for ( ; ; ) {
        $pos + 2 > $dataLen
          and $et->Warn('Missing item ID list terminator'), last;
        ...
        if ( not $$tagTablePtr{$tag} ) {
            my %lkup =
              ( 0x20 => 0x2e, 0x21 => 0x2f, 0x30 => 0x31, 0x40 => 0x40 );
            my $srcTag = $lkup{ $tag & 0x71 } || $lkup{ $tag & 0x70 };
        ...
        AddTagToTable(
            ...
            $tag,
            {
                ...
                SubDirectory =>
                  { TagTable => 'Image::ExifTool::LNK::UnknownData' },
            }
        ...
        if ( $verbose > 1 and $beefStart ) {
            ...
            $et->VPrint(
                1,
                sprintf(
                    "$$et{INDENT}- Tag 0x%.4x Extension (%d bytes)\n",
                    $tag, $len
                )
            );
            $et->VerboseDump(
        ...
        while ($beefStart) {
            my $end = $pos + $size;
            if ( $beefStart + 8 > $end ) {
                $et->Warn('Garbage data in 0xbeef extension')
                  unless $beefStart == $end;
                last;
            }
            my $len = Get16u( $dataPt, $beefStart );
            my $beefID = Get32u( $dataPt, $beefStart + 4 );
            ( $beefID & 0xffff0000 ) == 0xbeef0000
              or $et->Warn('Invalid 0xbeef extension'), last;
            $beefStart + $len <= $end
              or $et->Warn('Truncated 0xbeef extension'), last;
            AddTagToTable(
                $tagTablePtr,
                $beefID,
                {
...
sub ProcessLinkInfo($$$) {
    ...
    if ( $lif & 0x01 ) {
        ...
        if ( $off and $off + 0x20 <= $dataLen ) {
            $et->HandleTag( $tagTablePtr, 'DriveType', undef, %opts,
                Start => $off + 4 );
            $et->HandleTag( $tagTablePtr, 'DriveSerialNumber', undef, %opts,
                Start => $off + 8 );
            ...
            if ( defined $val ) {
                ...
                $et->HandleTag(
                    $tagTablePtr, 'VolumeLabel', $val, %opts,
                    Start => $pos,
                    Size  => $size
                );
            }
        ...
        if ( defined $val ) {
            ...
            $val = $et->Decode( $val, 'UTF16' ) if $unicode;
            $et->HandleTag(
                $tagTablePtr, 'LocalBasePath', $val, %opts,
                Start => $pos,
                Size  => $size
            );
        }
        ...
        if ( $off and $off + 0x14 <= $dataLen ) {
            ...
            if ( $flg & 0x01 ) {
                ...
                if ( defined $val ) {
                    ...
                    $val = $et->Decode( $val, 'UTF16' ) if $unicode;
                    $et->HandleTag(
                        $tagTablePtr, 'DeviceName', $val, %opts,
                        Start => $pos,
                        Size  => $size
                    );
                }
            ...
            if ( $flg & 0x02 ) {
                ...
                $et->HandleTag( $tagTablePtr, 'NetProviderType', $val, %opts,
                    Start => $off + 0x10 );
            }
        }
    }
    ...
    if ( $off and $off < $dataLen ) {
        ...
        $et->HandleTag(
            $tagTablePtr, 'CommonPathSuffix', $val, %opts,
            Start => $off,
            Size  => length($val) + 1
        );
    }
    if ( $hdrLen >= 0x24 ) {
        ...
        if ( $off and $off < $dataLen ) {
            ...
            $et->HandleTag(
                $tagTablePtr, 'CommonPathSuffixUnicode', $val, %opts,
                Start => $off,
                Size  => length($val) + 1
            );
        }
...
sub ProcessLNK($$) {
    ...
    if ( $flags & 0x01 ) {
        $raf->Read( $buff, 2 ) == 2
          or $et->Warn('Missing target ID list'), return 1;
        $len = unpack( 'v', $buff );
        $raf->Read( $buff, $len ) == $len
          or $et->Warn('Truncated target ID list'), $len = length($buff);
        $et->HandleTag(
            $tagTablePtr, 0x10000, undef,
            DataPt  => \$buff,
    ...
    if ( $flags & 0x02 ) {
        $raf->Read( $buff, 4 ) == 4
          or $et->Warn('Missing link information'), return 1;
        $len = unpack( 'V', $buff );
        $len < 4 and $et->Warn('Invalid link information'), return 1;
        $raf->Read( $buf2, $len - 4 ) == $len - 4
          or $et->Warn('Truncated link information'), $len = length($buf2);
        $buff .= $buf2;
        $et->HandleTag(
            $tagTablePtr, 0x20000, undef,
            DataPt  => \$buff,
            DataPos => $raf->Tell() - $len,
            Size    => $len,
        );
    ...
    for ( $i = 0 ; $i < @strings ; ++$i ) {
        ...
        $raf->Read( $buff, 2 ) == 2
          or $et->Warn("Invalid $strings[$i] string"), return 1;
        my $pos = $raf->Tell();
        ...
        $len *= 2 if $isUnicode;
        $raf->Read( $buff, $len )
          or $et->Warn("Truncated $strings[$i] string"), return 1;
        if ($limit) {
            $len -= $isUnicode ? 2 : 1;
    ...
    while ( $raf->Read( $buff, 4 ) == 4 ) {
        ...
        last if $len < 4;
        $len -= 4;
        $raf->Read( $buf2, $len ) == $len
          or $et->Warn('Truncated extra data'), last;
        next unless $len > 4;
        $buff .= $buf2;
    ...
    if ($bad) {
        $et->Warn(
            sprintf( 'Unknown %d-byte overlay at offset 0x%x', $len, $pos ) );
    }
    elsif ($len) {
