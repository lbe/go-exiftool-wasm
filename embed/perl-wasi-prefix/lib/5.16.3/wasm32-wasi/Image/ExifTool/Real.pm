...
my %metadataFormat = (
    1 => 'string', 2 => 'string', 3 => 'flag', 4 => 'int32u', 5 =>
      'undef', 6 => 'string', 7 => 'string', 8 => 'string', 9 => undef, 10 =>
      undef, );
...
%Image::ExifTool::Real::Media = (
    ...
    CONT =>
      { SubDirectory => { TagTable => 'Image::ExifTool::Real::ContentDescr' } },
    MDPR =>
      { SubDirectory => { TagTable => 'Image::ExifTool::Real::MediaProps' } },
    PROP =>
      { SubDirectory => { TagTable => 'Image::ExifTool::Real::Properties' } },
    RJMD =>
      { SubDirectory => { TagTable => 'Image::ExifTool::Real::Metadata' } },
);
...
%Image::ExifTool::Real::Audio = (
    ...
    '.ra3' => {
        Name         => 'RA3',
        SubDirectory => { TagTable => 'Image::ExifTool::Real::AudioV3' }
    },
    '.ra4' => {
        Name         => 'RA4',
        SubDirectory => { TagTable => 'Image::ExifTool::Real::AudioV4' }
    },
    '.ra5' => {
        Name         => 'RA5',
        SubDirectory => { TagTable => 'Image::ExifTool::Real::AudioV5' }
    },
);
...
%Image::ExifTool::Real::MediaProps = (
    ...
    VARS         => { ID_LABEL => 'Sequence' },
    FORMAT       => 'int32u',
    PRIORITY     => 0, 0 => { Name => 'StreamNumber', Format => 'int16u' },
...
sub ProcessRealMeta($$$) {
    ...
    for ( ; ; ) {
        ...
        my ( $size, $type, $flags, $valuePos, $subPropPos, $numSubProps,
            $nameLen )
        ...
        if ( $valueLen and $format ) {
            if ( $format eq 'flag' ) {
                ...
                $$tagInfo{PrintConv}
                  or $$tagInfo{PrintConv} = '$self->ConvertDateTime($val)';
            }
            my $count = int( $valueLen / Image::ExifTool::FormatSize($format) );
            my $val   = ReadValue( $dataPt, $valuePos, $format, $count,
                $dirEnd - $valuePos );
            $et->HandleTag(
                $tagTablePtr, $tag, $val,
                DataPt  => $dataPt,
...
sub ProcessReal($$) {
    ...
    if ( $1 eq '.RMF' ) {
        ...
        while ( $raf->ReadLine($buff) ) {
            ...
            if ($type) {
                return 0
                  if $buff =~ /^http/ and $buff !~ /\.(ra|rm|rv|rmvb|smil)$/i;
                $et->SetFileType($type);
                return 1 if $fast3;
                undef $type;
            }
        ...
        and $buff =~ /^RMJE/ )
    {
        ...
        if (    $raf->Seek( -$metaSize - 12, 1 )
            and $raf->Read( $buff, $metaSize ) == $metaSize
            and $buff =~ /^RJMD/ )
        {
            my %dirInfo = (
                DataPt   => \$buff,
