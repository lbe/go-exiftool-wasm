...
my %structs = (
    ...
    CompressedQuickTime => [
        ...
        imageData =>
          q{binary[$val{size} - 68 - $val{maskSize} - $val{imageDescr}->{size} -
    ...
    Int16Data => [
        size => 'int16u', data => 'int8u[$val{size}]',
    ],
    Int32uData => [
        size => 'int32u', data => 'int8u[$val{size}]',
    ],
    LongComment => [
        kind => 'int16u',
        size => 'int16u', data => 'binary[$val{size}]',
    ],
    PixMap => [
        rowBytes   => 'int16u',
        bounds     => 'Rect',
    ...
    PixPat => [
        patType => 'int16u', pat1Data => 'int8u[8]',
...
experimental, and is only enabled with the Verbose or Unknown options.
    ...
    0x00a1 => [
        {
            Name => 'LongComment', Format => 'LongComment',
...
sub GetPixData($$) {
    ...
    my $rowBytes = $pixMap->{rowBytes} & 0x3fff;
    my $height =
      $pixMap->{bounds}->{botRight}->{v} - $pixMap->{bounds}->{topLeft}->{v};
    my ( $data, $size, $buff, $i );

    if ( $packType == 1 or $rowBytes < 8 ) { $size = $rowBytes * $height;
        return undef unless $raf->Read( $data, $size ) == $size;
    }
    elsif ( $packType == 2 ) { $size = int( $rowBytes * $height * 3 / 4 + 0.5 );
        return undef unless $raf->Read( $data, $size ) == $size;
    }
    else {
        $data = '';
...
sub ProcessPICT($$) {
    ...
    $verbose
      and printf $out "PICT version $vers%s\n", $extended ? ' extended' : '';
    ...
    for ( ; ; ) {
        ...
        if ( ref $val eq 'HASH' ) {
            if (    $$tagInfo{Name} eq 'CompressedQuickTime'
                and ref $val->{imageDescr} eq 'HASH'
                and $val->{imageDescr}->{compressor}
                and $val->{imageDescr}->{compressor} eq 'Photo - JPEG'
                and ref $val->{imageData} eq 'SCALAR'
                and $et->ValidateImage( $val->{imageData}, 'PreviewImage' ) )
            {
