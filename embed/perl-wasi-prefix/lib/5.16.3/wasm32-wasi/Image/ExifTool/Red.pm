...
%Image::ExifTool::Red::Main = (
    ...
    RED1 => {
        Name         => 'Red1Header',
        SubDirectory => { TagTable => 'Image::ExifTool::Red::RED1' }
    },
    RED2 => {
        Name         => 'Red2Header',
        SubDirectory => { TagTable => 'Image::ExifTool::Red::RED2' }
    },

    0x1000 => 'StartEdgeCode', 0x1001 =>
      { Name => 'StartTimecode', Groups => { 2 => 'Time' } }, 0x1002 =>
      { Name => 'OtherDate1',
        Groups => { 2 => 'Time' },
        ValueConv =>
          '$val =~ s/(\d{4})_(\d{2})_/$1:$2:/; $val =~ tr/_/ /; $val', },
    0x1003 => { Name => 'OtherDate2',
        Groups => { 2 => 'Time' },
        ValueConv =>
          '$val =~ s/(\d{4})_(\d{2})_/$1:$2:/; $val =~ tr/_/ /; $val', },
    0x1004 => { Name => 'OtherDate3',
        Groups => { 2 => 'Time' },
        ValueConv =>
          '$val =~ s/(\d{4})_(\d{2})_/$1:$2:/; $val =~ tr/_/ /; $val', },
    0x1005 => { Name => 'DateTimeOriginal',
        Description => 'Date/Time Original',
        Groups      => { 2 => 'Time' },
        ValueConv =>
'$val =~ s/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})/$1:$2:$3 $4:$5:/; $val',
        PrintConv => '$self->ConvertDateTime($val)', },
    0x1006 => 'SerialNumber', 0x1019 => 'CameraType', 0x101a =>
      { Name => 'ReelNumber', Groups => { 2 => 'Video' } }, 0x101b =>
      { Name => 'Take', Groups => { 2 => 'Video' } },
    0x1023 => { Name => 'DateCreated',
        Groups    => { 2 => 'Time' },
        ValueConv => '$val =~ s/(\d{4})(\d{2})/$1:$2:/; $val', },
    0x1024 => { Name => 'TimeCreated',
        Groups    => { 2 => 'Time' },
        ValueConv => '$val =~ s/(\d{2})(\d{2})/$1:$2:/; $val', },
    0x1025 => 'FirmwareVersion', 0x1029 =>
      { Name => 'ReelTimecode', Groups => { 2 => 'Time' } }, 0x102a =>
      'StorageType', 0x1030 => { Name => 'StorageFormatDate',
        Groups    => { 2 => 'Time' },
        ValueConv => '$val =~ s/(\d{4})(\d{2})/$1:$2:/; $val', },
    0x1031 => { Name => 'StorageFormatTime',
        Groups    => { 2 => 'Time' },
        ValueConv => '$val =~ s/(\d{2})(\d{2})/$1:$2:/; $val', },
    0x1032 => 'StorageSerialNumber', 0x1033 => 'StorageModel', 0x1036 =>
      'AspectRatio',  0x1042 => 'Revision',   0x1056 => 'OriginalFileName',
    0x106e => 'LensMake',
    0x106f => 'LensNumber', 0x1070 => 'LensModel',
    0x1071 => {
        Name        => 'Model',
    ...
    0x1096 => 'Filter',    0x10a0 => 'Brain',
    0x10a1 => 'Sensor',
    0x10be => 'Quality',
    0x200d => 'ColorTemperature',
    0x204b => 'RGBCurves', 0x2066 => {
        Name      => 'OriginalFrameRate',
        Groups    => { 2 => 'Video' },
        PrintConv => 'int($val * 1000 + 0.5) / 1000',
    },
    0x4037 => { Name => 'CropArea' }, 0x403b    => 'ISO',
...
%Image::ExifTool::Red::RED1 = (
    ...
    0x07 => { Name => 'RedcodeVersion', Format => 'string[1]' },   0x36 =>
      { Name => 'ImageWidth',  Format => 'int16u' }, 0x3a =>
      { Name => 'ImageHeight', Format => 'int16u' }, 0x3e =>
      { Name => 'FrameRate',
        Format    => 'rational32u',
        PrintConv => 'int($val * 1000 + 0.5) / 1000', },
...
sub ProcessR3D($$) {
    ...
    if ( $ver eq '1' ) {
        ...
        $pos += Get8u( \$buff, 0x40 ) * 0x18;
        $pos += Get8u( \$buff, 0x41 ) * 0x14;
        $pos += Get8u( \$buff, 0x42 ) * 0x10;
    }
    if ( $pos + 8 > length $buff ) {
        $dirLen = 0;
    }
    else {
        $dirLen = Get16u( \$buff, $pos );
        $pos += 2;
    }
    if ( $dirLen < 300 or $dirLen >= 2048 or $pos + $dirLen > length $buff ) {
        $buff =~ /\0\x0f\x10[\0\x06]/g
          or return $et->Warn(
            "Can't find Red directory. Please submit sample for testing");
        $pos    = pos($buff) - 4;
        $dirEnd = length $buff;
        undef $dirLen;
        $et->Warn(
            'This R3D file is different. Please submit a sample for testing');
    }
    else {
        $dirEnd = $pos + $dirLen;
    }
