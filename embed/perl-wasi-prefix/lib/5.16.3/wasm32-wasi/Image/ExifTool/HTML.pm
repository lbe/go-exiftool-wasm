...
%Image::ExifTool::HTML::Main = (
    ...
    robots                    => { List => 1 },
    title                     => {
        Notes => "the only extracted tag which isn't from an HTML META element"
    },
);
...
%Image::ExifTool::HTML::ncc = (
    ...
    charset   => { Name => 'CharacterSet' }, depth => {},
    files     => {},
    footnotes => {},
    generator => {},
    ...
    producer       => {},
    produceddate => { Name => 'ProducedDate', Groups => { 2 => 'Time' } }
    , revision => {},
...
sub SetHTMLCharset($$) {
    my ( $val, $et ) = @_;
    $$et{HTMLCharset} = $htmlCharset{ lc $1 }
      if $val =~ /charset=['"]?([-\w]+)/;
    return $val;
}
...
sub ProcessHTML($$) {
    ...
    for ( ; ; ) {
        ...
        if ( $tag eq 'meta' ) {
            ...
            if (   $attrs =~ /\bcontent\s*=\s*(['"])(.*?)\1/si
                or $attrs =~ /\bcontent\s*=\s*(['"]?)([\w:.-]+)/si )
            {
                $val = $2;
            }
            else {
            ...
            while ( $xml =~ /<([\w-]+):([\w-]+)(\s.*?)?>([^<]*?)<\/\1:\2>/g ) {
                ...
                $val = $et->Decode( $val, $$et{HTMLCharset} )
                  if $$et{HTMLCharset};
        ...
        unless ( $$table{$tag} ) {
            ...
            $info->{Groups}->{1} =
              ( $grp eq 'http-equiv' ? 'HTTP-equiv' : "HTML-$grp" )
              if $grp;
