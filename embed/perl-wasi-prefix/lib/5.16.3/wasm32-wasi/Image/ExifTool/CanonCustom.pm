...
%Image::ExifTool::CanonCustom::PersonalFuncValues = (
    ...
    4            => {
        ...
        ValueConvInv =>
          'Image::ExifTool::Canon::CanonEvInv(-log($val*8/1000)/log(2))/4',
    ...
    6 => {
        ...
        ValueConvInv =>
          '(Image::ExifTool::Canon::CanonEvInv(log($val)*2/log(2))+32)/4',
...
'$val < 2 ? $val : ($val < 1000 ? exp(($val/8-9)*log(2))*100 : 0)'
    ...
    0x0108 => {
        Name  => 'SafetyShift',
        Notes => 'value of 2 not used by some models', PrintConv => {
            0 => 'Disable',
            1 => 'Enable (Tv/Av)',
            2 => 'Enable (ISO speed)',
        },
    ...
    0x0109 => [
            ...
            PrintConvInv => '$val=~/0x([\dA-F]+)/i ? hex($1) : undef', },
        {
            Name      => 'UsableShootingModes',
            Count     => 2,
            PrintConv => [ \%disableEnable, 'sprintf("Flags 0x%x",$val)', ],
            PrintConvInv =>
              [ undef, '$val=~/0x([\dA-F]+)/i ? hex($1) : undef', ],
        }
    ],
    0x010a => [
    ...
    0x010c => [
        {
            ...
            Count     => 3,
            ValueConv =>
              [ undef, 'exp(-($val/8-7)*log(2))', 'exp(-($val/8-7)*log(2))', ],
            ValueConvInv => [
                undef,
        ...
        { Name => 'ShutterSpeedRange',
            ...
            Count     => 4,
            ValueConv =>
              [ 'exp(-$val/(1600*log(2)))', 'exp(-$val/(1600*log(2)))',
                'exp(-$val/(1600*log(2)))', 'exp(-$val/(1600*log(2)))',
              ],
            ValueConvInv => [
...
'"Manual: Hi " . Image::ExifTool::Exif::PrintExposureTime($val)',
    ...
    0x010d => [
        {
            ...
            Count     => 3,
            ValueConv => [
                undef, 'exp(($val/8-1)*log(2)/2)', 'exp(($val/8-1)*log(2)/2)',
            ],
            ValueConvInv => [
                undef,
        ...
        { Name => 'ApertureRange',
            Condition => '$count == 4',
            Count     => 4,
            ValueConv => [ 'exp($val/2400)', 'exp($val/2400)',
                'exp($val/2400)', 'exp($val/2400)',
            ],
            ValueConvInv => [
            ...
            PrintConv => [ 'sprintf("Manual: Closed %.2g",$val)',
                'sprintf("Open %.2g",$val)',
    ...
    0x010f => [
        ...
        },
        {
            Name => 'FlashSyncSpeedAv',
            Condition =>
              '$$self{Model} =~ /\b(450D|XSi|Kiss X2|1000D|XS|Kiss F)\b/',
    ...
    0x0110 => { Name => 'AEMicroadjustment',
        Count     => 3,
        PrintConv => [ \%disableEnable ], },
    0x0111 => { Name => 'FEMicroadjustment',
        Count     => 3,
        PrintConv => [ \%disableEnable ], },
    0x0112 => [
    ...
    0x0113 => { Name => 'ExposureCompAutoCancel',
        PrintConv => \%enableDisable, },
    0x0114 => { Name => 'AELockMeterModeAfterFocus',
        PrintConv => {
            BITMASK => { 0 => 'Evaluative',
                1 => 'Partial',
                2 => 'Spot',
                3 => 'Center-weighted', }
        },
    ...
    0x0204 => [
        {
            ...
            Condition =>
'$$self{Model} =~ /\b(50D|5D Mark II|500D|T1i|Kiss X3|1D Mark IV)\b/',
    ...
    0x040a => { Name => 'ViewfinderWarnings',
        PrintConv => {
            BITMASK => { 0 => 'Monochrome', 1 => 'WB corrected', 2 =>
                  'One-touch image quality', 3 => 'ISO expansion', 4 =>
                  'Spot metering', 6 => 'Noise reduction', 7 => 'HDR', }
        },
    },
    0x040b => { Name => 'LVShootingAreaDisplay',
        PrintConv => {
            0 => 'Masked',
            1 => 'Outlined',
        },
    },
    0x040c => { Name => 'LVShootingAreaDisplay',
        PrintConv => {
            0 => 'Masked',
            1 => 'Outlined',
        },
    ...
    0x0502 => {
        Name      => 'AIServoTrackingSensitivity',
        PrintConv => {
            -2 => 'Slow',        -1 => 'Medium Slow', 0 => 'Standard',
            1  => 'Medium Fast', 2  => 'Fast', },
    },
    0x0503 => {
        ...
        },
        {
            ...
            PrintConv => {
                ...
                3 => '19 Points, Multi-controller selectable', 4 =>
                  'Inner 9 Points, Multi-controller selectable', 5 =>
                  'Outer 9 Points, Multi-controller selectable', },
        },
    ],
    0x050a => [
        {
...
'AF point button: Auto selection; Rear dial: Manual selection',
    ...
    0x0510 => [ {
            Name => 'VFDisplayIllumination', Condition =>
              '$$self{Model} =~ /\b7D\b/',
            Notes     => '7D',
            PrintConv => {
                0 => 'Auto',
    ...
    0x0512 => { Name => 'SelectAFAreaSelectMode',
        PrintConv => [
            {
                0 => 'Disable',
                1 => 'Enable',
        ...
        PrintConvInv => [ undef, '$val=~/0x([\dA-F]+)/i ? hex($1) : undef', ],
    },
    0x0513 => { Name => 'ManualAFPointSelectPattern',
        PrintConv => {
            0 => 'Stops at AF area edges',
            1 => 'Continuous',
        },
    },
    0x0514 => { Name => 'DisplayAllAFPoints',
        PrintConv => \%enableDisable, },
    0x0515 => { Name => 'FocusDisplayAIServoAndMF',
        PrintConv => \%enableDisable, },
    0x0516 => { Name => 'OrientationLinkedAFPoint',
        PrintConv => {
            0 => 'Same for vertical and horizontal',
            1 => 'Select different AF points',
        },
    },
    0x0517 => { Name => 'MultiControllerWhileMetering',
        PrintConv => {
            0 => 'Off',
            1 => 'AF point selection',
        },
    },
    0x0518 => { Name => 'AccelerationTracking', },
    0x0519 => { Name => 'AIServoFirstImagePriority',
        PrintConv => { -1 => 'Release priority',
            0 => 'Equal priority',
            1 => 'Focus priority', },
    },
    0x051a => { Name => 'AIServoSecondImagePriority',
        PrintConv => { -1 => 'Shooting speed priority',
            0 => 'Equal priority',
            1 => 'Focus priority', },
    },
    0x051b => { Name => 'AFAreaSelectMethod',
        PrintConv => {
            0 => 'AF area selection button',
            1 => 'Main dial',
        },
    },
    0x051c => { Name => 'AutoAFPointColorTracking',
        PrintConv => {
            0 => 'On-Shot AF only',
            1 => 'Disable',
        },
    },
    0x051d => { Name => 'VFDisplayIllumination',
        PrintConv => [
            {
                0 => 'Auto',
                1 => 'Enable',
            ...
            {
                0 => 'Non-illuminated', 1 => 'Illuminated', }
        ],
    },
    0x051e => { Name => 'InitialAFPointAIServoAF',
        PrintConv => {
            0 => 'Auto',
    ...
    0x0610 => [
        ...
        { Name => 'ContinuousShootingSpeed',
            Count     => 3,
            PrintConv => [ \%disableEnable, '"Hi $val"', '"Lo $val"', ],
            PrintConvInv =>
              [ undef, '$val=~/(\d+)/ ? $1 : 0', '$val=~/(\d+)/ ? $1 : 0', ], }
    ],
    0x0611 => {
        Name         => 'ContinuousShotLimit',
        Count        => 2,
        PrintConv    => [ \%disableEnable, '"$val shots"', ],
        PrintConvInv => [ undef, '$val=~/(\d+)/ ? $1 : 0', ],
    },
    0x0612 => [
    ...
    0x0703 => {
        ...
        PrintConv => {
            ...
            3 => 'AF point selection swapped with Exposure comp', 4 =>
              'ISO speed swapped with Exposure comp', },
    },
    0x0704 => [
        {
            Name      => 'SetButtonWhenShooting',
            ...
            Notes =>
'40D, 50D and 5DmkII; value of 5 is new for 50D, and 6 is new for 5DmkII',
            PrintConv => {
                0 => 'Normal (disabled)',
                1 => 'Image quality',
                2 => 'Picture style',
                3 => 'Menu display',
                4 => 'Image playback',
                5 => 'Quick control screen', 6 => 'Record movie (Live View)', },
        },
        {
            Name      => 'SetButtonWhenShooting',
            Condition => '$$self{Model} =~ /\b60D\b/',
        ...
        },
        {
            Name => 'SetButtonWhenShooting',
            Condition =>
'$$self{Model} =~ /\b(450D|XSi|Kiss X2|550D|T2i|Kiss X4|600D|T3i|Kiss X5)\b/',
        ...
        PrintConv => [
            {
                ...
                2 => 'Play memo (hold:record memo)', 3 =>
                  'Rating (protect/memo:disable)', }
        ],
    },
    0x070a => {
    ...
    0x070c => { Name => 'CustomControls', },
    0x070d => { Name => 'StartMovieShooting',
        PrintConv => {
            0 => 'Default (from LV)',
            1 => 'Quick start (FEL button)',
        },
    },
    0x070e => { Name => 'FlashButtonFunction',
        PrintConv => {
            0 => 'Raise built-in flash',
            1 => 'ISO speed',
        },
    },
    0x070f => { Name => 'MultiFunctionLock',
        PrintConv => [
            { 0 => 'Off',
                1 => 'On', 2 => 'On (quick control dial)', 3 =>
                  'On (main dial and quick control dial)', },
            {
                BITMASK => { 0 => 'Main dial',
    ...
    0x0710 => { Name => 'TrashButtonFunction',
        PrintConv => {
            0 => 'Normal (set center AF point)',
            1 => 'Depth-of-field preview',
        },
    },
    0x0711 => { Name => 'ShutterReleaseWithoutLens',
        PrintConv => \%disableEnable, },
    0x0712 => { Name => 'ControlRingRotation',
        PrintConv => {
            0 => 'Normal',
            1 => 'Reversed',
        },
    },
    0x0713 => { Name => 'FocusRingRotation',
        PrintConv => {
            0 => 'Normal',
            1 => 'Reversed',
        },
    },
    0x0714 => { Name => 'RFLensMFFocusRingSensitivity',
        PrintConv => {
            0 => 'Varies With Rotation Speed',
            1 => 'Linked To Rotation Angle',
        },
    },
    0x0715 => { Name => 'CustomizeDials',  },
    0x080b => [
        {
            Name      => 'FocusingScreen',
    ...
    0x080c => [
        { Name => 'TimerLength',
            ...
            Count     => 3,
            PrintConv =>
              [ '"6 s: $val"', '"16 s: $val"', '"After release: $val"', ],
            PrintConvInv => [
                '$val=~/(\d+)$/ ? $1 : 0',
                '$val=~/(\d+)$/ ? $1 : 0',
                '$val=~/(\d+)$/ ? $1 : 0',
    ...
    0x080f => {
        Name => 'AddOriginalDecisionData', PrintConv => \%offOn,
    },
    0x0810 => {
    ...
    0x0812 => { Name => 'MemoAudioQuality',
        PrintConv => {
            0 => 'High (48 kHz)',
            1 => 'Low (8 kHz)',
        },
    },
    0x0813 => { Name => 'DefaultEraseOption',
        PrintConv => {
            0 => 'Cancel selected',
            1 => 'Erase selected',
            2 => 'Erase RAW selected', 3 => 'Erase non-RAW selected', },
    },
    0x0814 => { Name => 'RetractLensOnPowerOff',
        PrintConv => \%enableDisable, },
    0x0815 => { Name => 'AddIPTCInformation',
        PrintConv => \%disableEnable, },
    0x0816 => { Name => 'AudioCompression',
        PrintConv => \%enableDisable, }
);
    ...
    for ( ; $pos < $end ; ) {
        ...
        for ( $i = 0 ; $recPos + 8 < $recEnd ; ++$i, $recPos += 4 * $num ) {
            ...
            if ( $tag == 0x70c and $num == 0x66 and $nextRec + 8 < $recEnd ) {
                ...
                if ( $tmp == 0x70f ) {
                    $et->Warn( 'Fixed incorrect CanonCustom tag 0x70c size',
                        1 );
                    ++$num;
                }
            }
            ...
            if ($write) {
                ...
                $tagInfo =
                  $et->GetTagInfo( $tagTablePtr, $tag, \$val, undef, $num )
                  or next;
                ...
                if ( $tagInfo and not $oldInfo ) {
                    ...
                    $$tagInfo{Description} =~
                      s/CanonCustom Functions/Canon Custom Functions /;
                }
            }
        }
        if ( $i != $recCount ) {
            $et->Warn(
                'Missing '
                  . ( $recCount - $i )
                  . " entries in CanonCustom2 group $recNum directory",
                1
            );
        }
        $pos = $recEnd;
...
sub ProcessCanonCustom($$$) {
    ...
    my $len = Get16u( $dataPt, $offset );
    unless ( $len == $size
        or ( $$et{Model} =~ /\bD60\b/ and $len + 2 == $size ) )
    {
...
sub WriteCanonCustom($$$) {
    ...
    unless ( $len == $dirLen
        or ( $$et{Model} =~ /\bD60\b/ and $len + 2 == $dirLen ) )
    {
        $et->Warn("Invalid CanonCustom data");
        return undef;
    }
