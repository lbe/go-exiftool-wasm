...
%Image::ExifTool::IPTC::Main = (
    ...
    3 => {
        Name         => 'IPTCNewsPhoto',
        SubDirectory => { TagTable => 'Image::ExifTool::IPTC::NewsPhoto', },
    },
    7 => {
        Name         => 'IPTCPreObjectData',
        SubDirectory => { TagTable => 'Image::ExifTool::IPTC::PreObjectData', },
    },
    8 => {
        Name         => 'IPTCObjectData',
        SubDirectory => { TagTable => 'Image::ExifTool::IPTC::ObjectData', },
    },
    9 => {
        Name   => 'IPTCPostObjectData',
        Groups => { 1 => 'IPTC#' }, SubDirectory =>
          { TagTable => 'Image::ExifTool::IPTC::PostObjectData', },
    },
    240 => {
        Name         => 'IPTCFotoStation',
        SubDirectory => { TagTable => 'Image::ExifTool::IPTC::FotoStation', },
    },
);
...
%Image::ExifTool::IPTC::ApplicationRecord = (
    ...
    8 => {
        ...
        Format    => 'digits[2]',
        PrintConv => { '01' => 'Additional language', },
    },
    10 => {
        Name      => 'Urgency',
        Format    => 'digits[1]',
...
'$self->Options("DateFormat") ? $self->ConvertDateTime("1970:01:01 $val") : $val',
    ...
    231 => {
        ...
        Format => 'string[0,256]', ValueConv =>
          '$val =~ s/\0+/\n/g; $val', ValueConvInv => '$val',
    },
    232 => {
        Name   => 'ExifCameraInfo',
        Format => 'string[0,4096]', },
    255 => { Name => 'CatalogSets',
        List   => 1,
        Format => 'string[0,256]', Notes => 'written by iView MediaPro', },
);
...
sub TranslateCodedString($$$$) {
    ...
    if ( $$xlatPtr eq 'bad' ) {
        $et->Warn(
            'Some IPTC characters not converted (unsupported CodedCharacterSet)'
        );
        undef $$xlatPtr;
    }
    elsif ( not $read ) {
        ...
        $et->Warn(
'Some IPTC characters not converted (ISO 2022 shifting not supported)'
        );
    }
}
...
sub ProcessIPTC($$$) {
    ...
    if ( $tagTablePtr eq \%Image::ExifTool::IPTC::Main ) {
        ...
        if ( defined $isStd and not $$et{DIR_COUNT}{STD_IPTC} ) {
            ...
            if ($isStd) {
                ...
                if ( eval { require Digest::MD5 } ) {
                    if ( $pos or $dirLen != length($$dataPt) ) {
                        $md5 =
                          Digest::MD5::md5( substr $$dataPt, $pos, $dirLen );
                    }
                    else {
            ...
            if ( ( $Image::ExifTool::MWG::strict or $validate )
    ...
    if (    $dirLen >= 4
        and substr( $$dataPt, $pos, 1 ) ne "\x1c"
        and substr( $$dataPt, $pos + 3, 1 ) eq "\x1c" )
    {
        $et->Warn('IPTC data was improperly byte-swapped');
        my $newData = pack( 'N*',
            unpack( 'V*', substr( $$dataPt, $pos, $dirLen ) . "\0\0\0" ) );
        $dataPt = \$newData;
        $pos    = 0;
        $dirEnd = $pos + $dirLen;
    }
    if (
        $$et{REQ_TAG_LOOKUP}{iptc}
        or ( $$et{TAGS_FROM_FILE}
            and not $$et{EXCL_TAG_LOOKUP}{iptc} )
      )
    {
        if ( $pos or $dirLen != length($$dataPt) ) {
        ...
        $pos += 5;
         if ( $len & 0x8000 ) {
            ...
            if ( $pos + $n > $dirEnd or $n > 8 ) {
                $et->VPrint( 0,
"Invalid extended IPTC entry (dataset $rec:$tag, len $len)\n"
                );
                $success = 0;
                last;
            }
        ...
        if ( $pos + $len > $dirEnd ) {
            $et->VPrint( 0,
                "Invalid IPTC entry (dataset $rec:$tag, len $len)\n" );
            $success = 0;
            last;
        }
        ...
        if ( not $format ) {
            ...
            if ( $len < $min or $len > $max ) {
                my $should =
                  ( $min == $max )
                  ? $min
                  : ( $len < $min ? "$min min" : "$max max" );
                my $what = ( $len < $siz * $min ) ? 'short' : 'long';
                $et->Warn(
"IPTC $$tagInfo{Name} too $what ($len bytes; should be $should)",
                    1
                );
            }
        }
        if ($format) {
            if ( $format =~ /^int/ ) {
                if ( $len <= 8 ) { $val = 0;
                    ...
                    for ( $i = 0 ; $i < $len ; ++$i ) {
                        $val =
                          $val * 256 + ord( substr( $$dataPt, $pos + $i, 1 ) );
                    }
                }
            }
            elsif ( $format =~ /^string/ ) {
                if ( $val =~ s/\0+$// and $validate ) {
                    $et->Warn( "IPTC $$tagInfo{Name} improperly terminated",
                        1 );
                }
                if ( $rec == 1 ) {
