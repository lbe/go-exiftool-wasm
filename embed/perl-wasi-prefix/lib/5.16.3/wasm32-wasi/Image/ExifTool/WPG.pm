...
sub ProcessWPG($$) {
    ...
    for ( ; ; ) {
        my ( $type, $len, $getSize );
        if ( $raf->Read( $buff, $ver ) == $ver ) { if ( $ver == 1 ) {
                $type    = ord($buff);
                $len     = ReadVarInt($raf);
                $getSize = 1 if $type == 0x0f;
            }
            else {
                $type = unpack( 'xC', $buff );
                ReadVarInt($raf);
                $len = ReadVarInt($raf);
                $getSize = 1 if $type == 0x01;
                undef $type if $type > 0x3f;
            }
            if ($getSize) {
                $raf->Read( $buff, $len ) == $len
                  or $et->Warn('File format error'), last;
                ...
                if ( $ver == 1 ) {
                    ...
                    unless ( defined $err ) {
                        my ( $x1, $y1, $x2, $y2 ) =
                          ReadValue( \$buff, 13, $format, 4, $len - 13 );
                        $w = abs( $x2 - $x1 );
                        $h = abs( $y2 - $y1 );
                    }
                }
                if ( defined $err ) {
                    ...
                    $et->HandleTag( $tagTablePtr,
                        ImageWidthInches => $w / ( $xres || 1200 ) );
                    $et->HandleTag( $tagTablePtr,
                        ImageHeightInches => $h / ( $yres || 1200 ) );
                }
            }
            else {
