...
my %changeOffset = ( C => -273.15, );
my %changeScale = (
    G => 1 / 9.80665, kph => 3.6, deg => 180 / $pi, 'deg/sec' => 180 /
      $pi, '%' => 100, kPa => 1 /
      1000, rpm => 10,   km => 1 /
      1000, L   => 1000, mm => 1000, );
...
%Image::ExifTool::GM::mrlv = (
    ...
    tstm => {
        ...
        ValueConv => 'ConvertUnixTime($val, undef, 6)', PrintConv =>
...
%Image::ExifTool::GM::marl = (
    ...
    Latitude => {
        Name        => 'GPSLatitude',
        Description => 'GPS Latitude', Groups => { 2 => 'Location' },
    ...
    Heading => {
        ...
        Groups      => { 2 => 'Location' },
        PrintConv   => '$val > 360 ? "n/a" : sprintf("%.2f",$val)', },
    ABSActive             => {},
    AccelPos              => {},
    BatteryVoltage        => {},
    Beacon                => {},
...
sub Process_mrld($$$) {
    ...
    for ( $pos = 0 ; $pos + 448 <= $dirLen ; $pos += 448 ) {
        ...
        $a[3] = $et->Decode( $a[3], 'UTF8' );
        $_ & 0x8000000 and $_ -= 4294967296 foreach @a[ 6, 7 ];
        map { $_ = GetDouble( \$_, 0 ) } @a[ 8, 9, 10, 11 ];
        $a[5] = Get64u( \$a[5], 0 );
        my $chan    = $a[0];
        ...
        unless ($tagInfo) {
            $tagInfo =
              AddTagToTable( $tagTablePtr, $tag,
                { Name => $tag, Struct => \%channelStruct } );
            Image::ExifTool::XMP::AddFlattenedTags( $tagTablePtr, $tag, undef,
                1 );
        }
        if ($struct) {
        ...
        if ( $struct == 0 or $struct == 2 ) {
            $et->HandleTag( $tagTablePtr, "$tag$channel[$_]", $a[$_] )
              foreach 1 .. $#a;
        }
        ...
        my $off =
          $a[11] * ( $changeScale{$units} || 1 ) +
          ( $changeOffset{$units} || 0 );
        my $init = int( ( $a[6] + $a[7] ) / 2 );
         $$gmDict[$chan] =
          { Name => $name, Mult => $mult, Off => $off, Init => $init };
...
sub Process_marl($$$) {
    ...
    $$vals[0] = -1 unless defined $$vals[0];
    my $ts = $$vals[0];
    ...
    for ( $pos = 0 ; $pos + 8 <= $dataLen ; $pos += 8 ) {
        ...
        if ( $a2 == 0xc0 ) { last if $ah == 0xff;
            $chan         = $a[0] & 0x0fffffff;
            $gmChan       = $$gmDict[$chan] or next;
            $val          = $a[1] - ( $a[1] & 0x80000000 ? 4294967296 : 0 );
            $$vals[$chan] = $val;
            last if $pos + 16 > $dataLen;
            $pos += 8;
            @ts = unpack( "x${pos}NN", $$dataPt );
            $ts = $ts[0] * 4294967296 + $ts[1];
        }
        elsif ( $a2 == 0x40 ) { next unless defined $chan;
            $ts += $a[1];
            $chanDiff = ( $ah & 0x3f ) - ( $ah & 0x20 ? 0x40 : 0 );
            $chan += $chanDiff;
            $gmChan = $$gmDict[$chan] or next;
            defined $$vals[$chan] or $$vals[$chan] = $$gmChan{Init};
            $valDiff =
              ( $a[0] & 0x00ffffff ) - ( $a[0] & 0x00800000 ? 0x01000000 : 0 );
            $val = ( $$vals[$chan] += $valDiff );
        }
        else {
            next;
        }
        if ( $ts > $maxTS ) {
            if ($csv) {
                ...
                $et->HandleTag( $tagTablePtr, TimeStamp => $ts )
                  if $reqTimeStamp;
                $et->HandleTag( $tagTablePtr, GPSDateTime => $ts )
                  if $reqGPSDateTime;
            }
            $maxTS = $ts;
        }
        ...
        if ($verbose2) {
            ...
            my ( $cd, $vd ) =
              @ts
              ? ( '', '' )
              : ( sprintf( '%+d', $chanDiff ), sprintf( '%+d', $valDiff ) );
            printf
              "| %8.4x: %.8x %.8x chan$cd=%.2d $$gmChan{Name}$vd = $val$str\n",
              $p0, @a, $chan;
            printf(
                "| %8.4x: %.8x %.8x         TimeStamp = %.6f sec\n",
                $dataPos + $pos,
                @ts, $ts / 1e7
            ) if @ts;
        }
