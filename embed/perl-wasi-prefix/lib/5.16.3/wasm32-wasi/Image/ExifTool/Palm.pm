...
my %dateTimeInfo = (
    ...
    ValueConv => 'ConvertUnixTime($val, 1)', PrintConv =>
...
%Image::ExifTool::Palm::EXTH = (
    ...
    405 => { Name => 'RentalFlag', Format => 'int8u' }, 406 =>
...
sub ProcessPDB($$) {
    ...
    $raf->Seek( $offset + $off, 0 ) and $raf->Read( $buf2, $len ) == $len
      or $buf2 = '<err>';
    $$et{VALUE}{BookName} = $et->Decode( $buf2, $enc );
    my $flag = Get32u( \$buff, 128 );
    return 1 unless $flag & 0x40;

    $len = Get32u( \$buff, 20 ) + 16;

    unless ($raf->Seek( $offset + $len, 0 )
        and $raf->Read( $buf2, 12 ) == 12
        and substr( $buf2, 0, 4 ) eq 'EXTH'
        and ( $size = Get32u( \$buf2, 4 ) ) > 12 )
    {
    ...
    $raf->Read( $buff, $size ) == $size
      or $et->Warn('Truncated MOBI extended header'), return 1;
    my %dirInfo = (
        DataPt     => \$buff,
