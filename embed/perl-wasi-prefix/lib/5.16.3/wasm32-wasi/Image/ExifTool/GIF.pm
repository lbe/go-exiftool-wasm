...
%Image::ExifTool::GIF::Extensions = (
    ...
    WRITE_PROC => sub { return 1 }, 'NETSCAPE/2.0' => { Name => 'Animation',
        SubDirectory => { TagTable => 'Image::ExifTool::GIF::Animation' }, },
    'XMP Data/XMP' => { Name => 'XMP',
        IncludeLengthBytes => 2,
        Terminator         => q(<\\?xpacket end=['"][wr]['"]\\?>), Writable =>
          2, SubDirectory => { TagTable => 'Image::ExifTool::XMP::Main' }, },
    'ICCRGBG1/012' => { Name => 'ICC_Profile',
        Writable => 2, SubDirectory =>
          { TagTable => 'Image::ExifTool::ICC_Profile::Main' }, },
    'MIDICTRL/Jon' => { Name => 'MIDIControl',
        SubDirectory => { TagTable => 'Image::ExifTool::GIF::MIDIControl' }, },
    'MIDISONG/Dm7' => { Name => 'MIDISong',
        Groups => { 2 => 'Audio' },
        Binary => 1, },
    'C2PA_GIF/' => { Name => 'JUMBF',
...
sub ProcessGIF($$) {
    ...
    my $flags = Get8u( \$s, 4 );
    if ( $flags & 0x80 ) {  $length = 3 * ( 2 << ( $flags & 0x07 ) );
  ...
  Block:
    for ( ; ; ) {
        ...
        if ( ord($ch) == 0x2c ) {
            ...
            if ( ord($ch) & 0x80 ) { $length = 3 * ( 2 << ( ord($ch) & 0x07 ) );
