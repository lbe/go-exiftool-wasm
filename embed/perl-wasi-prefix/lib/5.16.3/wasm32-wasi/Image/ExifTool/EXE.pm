...
%Image::ExifTool::EXE::Main = (
    ...
    0 => {
        ...
        PrintConv => {
            ...
            0x014d => 'Intel i860', 0x0162 => 'MIPS R3000',
            0x0166 => 'MIPS little endian (R4000)',
            0x0168 => 'MIPS R10000',
            0x0169 => 'MIPS little endian WCI v2',
            0x0183 => 'Alpha AXP (old)', 0x0184 => 'Alpha AXP',
            0x01a2 => 'Hitachi SH3',
    ...
    44 => {
        ...
        PrintConv => {
            ...
            5  => 'OS/2 command line', 7 => 'POSIX command line',
            9  => 'Windows CE GUI',
            10 => 'EFI application',
            11 => 'EFI boot service',
...
%Image::ExifTool::EXE::PEString = (
    ...
    CharacterSet => {
        ...
        PrintConv   => {
            '0000' => 'ASCII',
            '03A4' => 'Windows, Japan (Shift - JIS X-0208)', '03A8' =>
              'Windows, Chinese (Simplified)',     '03B5' =>
              'Windows, Korea (Shift - KSC 5601)', '03B6' =>
              'Windows, Taiwan (Big5)', '04B0' => 'Unicode', '04E2' =>
    ...
    BuildDate => { Groups => { 2 => 'Time' } }, BuildVersion => {}, Comments =>
      {},
    CompanyName     => {},
    Copyright       => {}, FileDescription => {},
    FileVersion     => {},
    InternalName    => {},
    LegalCopyright  => {},
    LegalTrademarks => {},
...
%Image::ExifTool::EXE::MachO = (
    ...
    5 => {
        ...
        PrintHex  => 1,
        PrintConv => {
            -1 => 'Static library', 1 => 'Relocatable object',
...
sub ProcessPEResources($$) {
    ...
    for ( $item = 0 ; $item < $count ; ++$item ) {
        ...
        unless ($level) {
            my $resType = $resourceType{$name}
              || sprintf( 'Unknown (0x%x)', $name );
            if ($verbose) {
...
sub ExtractMachTags($$;$) {
    ...
    if ($info) {
        ...
        else { $et->OverrideFileType( 'Mach-O static library', undef, 'A' );
        }
        return 1;
    }
    return 0;
...
sub ProcessEXE($$) {
    ...
    if ( $buff =~ /^MZ/ and $size == 0x40 ) {
        ...
        {
            if ( $1 eq 'NE' ) {
                if ( $size >= 0x40 ) {  my $appFlags = Get16u( \$buff, 0x0c );
                    $ext = $appFlags & 0x80 ? 'DLL' : 'EXE';
                    $type = "Win16 $ext";
                }
            }
            elsif ( $1 eq 'PE' ) {
                if ( $size >= 24 ) { my $mach = Get16u( \$buff, 4 );
                    ...
                    for ( $pos = 0 ; $pos + 28 <= $len ; $pos += 28 ) {
                        ...
                        if ( $type == 2 ) { next unless $buf2 =~ /^(RSDS|NB10)/;
                            $tagTablePtr =
