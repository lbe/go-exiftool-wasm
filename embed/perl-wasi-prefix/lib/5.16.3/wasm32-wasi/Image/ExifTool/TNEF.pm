...
my %propType = (
    ...
    0x04 => 'float',
    0x05 => 'double',
    0x06 => 'int64s', 0x07 => 'double', 0x0a => 'int32s', 0x0b =>
      'int16s', 0x0d => 'undef', 0x14 => 'int64s',
    0x1e => 'string', 0x1f => 'Unicode', 0x40 => 'int64u', 0x48 =>
      'GUID', 0x102 => 'undef', );
...
%Image::ExifTool::TNEF::Main = (
    ...
    0x04800D => {
        Name   => 'Priority',
        Format => 'int16u', PrintConv => {
            0 => 'Low',
            1 => 'Normal',
            2 => 'High',
        },
    },
    0x038020 => { Name => 'MessageModifyDate', %dateInfo }, 0x069003 => {
        Name         => 'MessageProps',
    ...
    0x069002 => { Name => 'AttachRenderingData', Binary => 1 }, 0x069005 =>
      { Name => 'AttachInfo',
...
%Image::ExifTool::TNEF::MsgProps = (
    ...
    TAG_PREFIX   => 'MsgProps',
    VARS => { LONG_TAGS => 0, NO_LOOKUP => 1 }, 0x0002 =>
    ...
    0x0f02 => { Name => 'DeliveryOrRenewTime', %dateInfo }, 0x1000 =>
      { Name => 'MessageBodyText', Binary => 1 },
    0x1007 => 'SyncBodyCount',
    0x1008 => 'SyncBodyData',
    0x1009 => {
        Name    => 'MessageBodyRTF',
        Notes   => 'RTF message body, decompressed if necessary',
        RawConv => '$$val', ValueConv =>
    ...
    , '00062004_0000801A' => 'HomeAddress', '00062004_000080DA' =>
      'HomeAddressCountryCode', '00062008_00008554' => 'AppVersion',
);
...
%Image::ExifTool::TNEF::AttachInfo = (
    ...
    0x3705 => {
        ...
        PrintConv => {
            ...
            1 => 'AttachData', 2 =>
              'AttachLongPathName (recipients with access)',
...
sub ProcessProps($$$) {
    ...
    for ( $i = 0 ; $i < $entries ; ++$i ) {
        ...
        if ( $tag & 0x8000 ) {
            ...
            $pos += 24;
            if ( $idtype == 0 ) { $tag = $uid . sprintf( '_%.8x', $num );
            }
            elsif ( $idtype == 1 ) { last if $pos + $num > $dirLen or $num < 2;
                my $name =
        ...
        while ($count) {
            ...
            if ( not defined $val ) {
                ...
                {
                    my @a = split ' ', $val;
                    if ( $type == 0x06 ) { $_ = $_ / 10000 foreach @a;
                    }
                    elsif ( $type == 0x07 ) {  foreach (@a) {
