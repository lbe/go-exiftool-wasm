...
sub MixColumns() {
    ...
    for ( $i = 0 ; $i < 4 ; ++$i ) {
        ...
        $t1 = $state[0][$i] ^ $state[1][$i];
        $t1 = xtime($t1) & 0xff;
        $state[0][$i] ^= $t1 ^ $t2;
        $t1 = $state[1][$i] ^ $state[2][$i];
        $t1 = xtime($t1) & 0xff;
        $state[1][$i] ^= $t1 ^ $t2;
        $t1 = $state[2][$i] ^ $state[3][$i];
        $t1 = xtime($t1) & 0xff;
        $state[2][$i] ^= $t1 ^ $t2;
        $t1 = $state[3][$i] ^ $t0;
        $t1 = xtime($t1) & 0xff;
...
sub InvMixColumns() {
    ...
    for ( $i = 0 ; $i < 4 ; ++$i ) {
        ...
        my $d = $state[3][$i];
        $state[0][$i] =
          ( Mult( $a, 0x0e ) ^ Mult( $b, 0x0b ) ^ Mult( $c, 0x0d )
              ^ Mult( $d, 0x09 ) ) & 0xff;
        $state[1][$i] =
          ( Mult( $a, 0x09 ) ^ Mult( $b, 0x0e ) ^ Mult( $c, 0x0b )
              ^ Mult( $d, 0x0d ) ) & 0xff;
        $state[2][$i] =
          ( Mult( $a, 0x0d ) ^ Mult( $b, 0x09 ) ^ Mult( $c, 0x0e )
              ^ Mult( $d, 0x0b ) ) & 0xff;
        $state[3][$i] =
          ( Mult( $a, 0x0b ) ^ Mult( $b, 0x0d ) ^ Mult( $c, 0x09 )
              ^ Mult( $d, 0x0e ) ) & 0xff;
    }
}
