...
%Image::ExifTool::Microsoft::XMP = (
    ...
    CameraSerialNumber => {},
    DateAcquired =>
      { Groups => { 2 => 'Time' }, %Image::ExifTool::XMP::dateTimeInfo },
    FlashManufacturer => {},
    FlashModel        => {},
...
my %sRegions = (
    ...
    PersonDisplayName => {},
    PersonEmailDigest => {},
    PersonLiveIdCID   => {}, PersonSourceID => {},
);
...
%Image::ExifTool::Microsoft::Xtra = (
    ...
    VideoBitrate              => {},
    VideoFormat               => {},
    'WM/AlbumArtist' => { Name => 'AlbumArtist', Writable => 'Unicode' }
    , 'WM/AlbumCoverURL' => { Name => 'AlbumCoverURL', Writable => 'Unicode' }
    , 'WM/AlbumTitle'    => { Name => 'AlbumTitle',    Writable => 'Unicode' }
    , 'WM/BeatsPerMinute' => 'BeatsPerMinute',
    'WM/Category' => { Name => 'Category', Writable => 'Unicode', List => 1 },
    'WM/Composer' => { Name => 'Composer', Writable => 'Unicode' }
    , 'WM/Conductor' =>
      { Name => 'Conductor', Writable => 'Unicode', List => 1 },
    'WM/ContentDistributor' =>
      { Name => 'ContentDistributor', Writable => 'Unicode' },
    ...
    'WM/MediaClassPrimaryID' => {
        ...
        Writable  => 'GUID',
        PrintConv => { 'D1607DBC-E323-4BE2-86A1-48A42A28441E' => 'Music',
            ...
            'FCF24A76-9A57-4036-990D-E35DD8B244E1' =>
              'Other (not audio or video)', },
    },
    'WM/MediaClassSecondaryID' => {
        Name      => 'MediaClassSecondaryID',
        Writable  => 'GUID',
        PrintConv => { 'E0236BEB-C281-4EDE-A36D-7AF76A3D45B5' => 'Audio Book',
            ...
            '0B710218-8C0C-475E-AF73-4C41C0C8F8CE' =>
              'Home Video from Pictures',
            '00000000-0000-0000-0000-000000000000' => 'Unknown Content', },
    },
    ...
    'WM/OriginalAlbumTitle' =>
      { Name => 'OriginalAlbumTitle', Writable => 'Unicode' }
    , 'WM/OriginalArtist' => { Name => 'OriginalArtist', Writable => 'Unicode' }
    , 'WM/OriginalLyricist' =>
      { Name => 'OriginalLyricist', Writable => 'Unicode' }
    , 'WM/ParentalRating' =>
      { Name => 'ParentalRating', Writable => 'Unicode' },
    ...
    'WM/Provider'       => { Name => 'Provider', Writable => 'Unicode' }
    , 'WM/ProviderRating' => 'ProviderRating',
    'WM/ProviderStyle' => 'ProviderStyle',
    'WM/Publisher'     => { Name => 'Publisher', Writable => 'Unicode' }
    , 'WM/SharedUserRating' =>
      { Name => 'SharedUserRating', Writable => 'int64u' },
    ...
    'WM/Writer' =>
      { Name => 'Writer', Groups => { 2 => 'Author' }, Writable => 'Unicode' }
    ...
    'WM/EncodedBy',
    => { Name => 'EncodedBy', Writable => 'Unicode' },

    '{2CBAA8F5-D81F-47CA-B17A-F8D822300131} 100' => {
        Name      => 'DateAcquired', Groups => { 2 => 'Time' },
        Shift     => 'Time',
...
sub ReadXtraValue($$) {
    ...
    for ( $i = 0 ; ; ) {
        ...
        elsif ( $valType == 65 and $valLen > 4 ) { $format = 'variant';
            require Image::ExifTool::FlashPix;
            my $vPos = 0;
             $val =
              Image::ExifTool::FlashPix::ReadFPXValue( $et, \$val, $vPos,
                Image::ExifTool::FlashPix::VT_VARIANT(),
                $valLen, 1 );
        }
        else {
...
sub WriteXtraValue($$$) {
    ...
    foreach $val (@$vals) {
        ...
        if ( $format eq 'Unicode' ) {
            $dat = $et->Encode( $val, 'UTF16', 'II' ) . "\0\0";
            $type = 8;
        }
            ...
            $dat = Image::ExifTool::GetUnixTime( $val, 1 );
            if ($dat) {
        ...
        { $dat = Image::ExifTool::GetUnixTime($val);
            if ($dat) {
...
sub WriteXtra($$$) {
    ...
    return undef unless $delGroup or %$newTags;
    ...
    for ( ; ; ) {
        ...
        while ( $$newTags{$tag} ) {
            ...
            $$nvHash{CreateOnly} and delete( $$newTags{$tag} ), last;
            my $valPos = $pos + 8 + $tagLen;
            my $valLen = $size - 8 - $tagLen;
            my $val =
              ReadXtraValue( $et, substr( $$dataPt, $valPos, $valLen ) );
            foreach $val (@$val) {
                my $overwrite = $et->IsOverwriting( $nvHash, $val );
                $overwrite or push( @newVals, $val ), next;
                $et->VPrint( 1,
                    "  - Microsoft:$$newTags{$tag}{Name} = $val\n" );
                next if $done{$tag};
                $done{$tag} = 1;
        ...
        if ( $done{$tag} ) {
            ...
            if ( length $buff ) {
                $newData .=
                    Set32u( 8 + length($tag) + length($buff) )
                  . Set32u( length($tag) )
                  . $tag
                  . $buff;
            }
