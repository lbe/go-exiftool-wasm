...
my %profileClass = (
    ...
    nkpf   => 'Nikon Input Device Profile (NON-STANDARD!)',  cenc =>
      'ColorEncodingSpace Profile',
    'mid ' => 'MultiplexIdentification Profile',
    mlnk   => 'MultiplexLink Profile',
    mvis   => 'MultiplexVisualization Profile',
);
my %manuSig = ( 'NONE' => 'none',
    'none' => 'none', '' => '', '4d2p' => 'Erdt Systems GmbH & Co KG',
    ...
    'GOOG' => 'Google', 'GPRT' => 'Giantprint Pty Ltd',
    'GTMB' => 'Gretagmacbeth',
    ...
    'JPEG' => 'Joint Photographic Experts Group', 'JSFT' =>
      'Jetsoft Development',
    ...
    'LCCD' => 'Leeds Colour',
    'lcms' => 'Little CMS', 'LDAK' => 'Left Dakota',
    'LEAD' => 'Leading Technology, Inc.',
    'Leaf' => 'Leaf', 'LEXM' => 'Lexmark International, Inc.',
    'LINK' => 'Link Computer, Inc.',
    'LINO' => 'Linotronic',
    'Lino' => 'Linotronic', 'lino' => 'Linotronic', 'LITE' => 'Lite-On, Inc.',
...
%Image::ExifTool::ICC_Profile::Main = (
    ...
    chrm => {
        Name   => 'Chromaticity',
        Groups => { 1 => 'ICC_Profile#' }, SubDirectory => {
    ...
    devs => {
        Name => 'DeviceSettings', Groups => { 2 => 'Camera' },
    },
    gamt => 'Gamut',
    kTRC => {
        Name        => 'GrayTRC',
    ...
    ncol => 'NamedColor', ncl2 => 'NamedColor2',
    resp => 'OutputResponse',
    pre0 => 'Preview0',
    pre1 => 'Preview1',
    pre2 => 'Preview2',
    ...
    psd0 => 'PostScript2CRD0', psd1 => 'PostScript2CRD1', psd2 =>
      'PostScript2CRD2', ps2s => 'PostScript2CSA', ps2i =>
      'PS2RenderingIntent', rXYZ => 'RedMatrixColumn', rTRC => {
        Name        => 'RedTRC',
    ...
    tech => {
        ...
        PrintConv => {
            ...
            silk   => 'Silkscreen',
            flex   => 'Flexography',
            mpfs   => 'Motion Picture Film Scanner', mpfr =>
              'Motion Picture Film Recorder',  dmpc =>
              'Digital Motion Picture Camera', dcpj =>
    ...
    ciis => 'ColorimetricIntentImageState', cicp =>
      { Name => 'ColorRepresentation',
        SubDirectory =>
          { TagTable => 'Image::ExifTool::ICC_Profile::ColorRep' }, },
    scoe => 'SceneColorimetryEstimates', sape =>
      'SceneAppearanceEstimates',          fpce =>
      'FocalPlaneColorimetryEstimates',    rhoc =>
      'ReflectionHardcopyOrigColorimetry', rpoc =>
      'ReflectionPrintOutputColorimetry',  psid =>
      { Name => 'ProfileSequenceIdentifier',
        Binary => 1, },
    B2D0 => { Name => 'BToD0', Binary => 1 }, B2D1 =>
      { Name => 'BToD1', Binary => 1 }, B2D2 =>
      { Name => 'BToD2', Binary => 1 }, B2D3 =>
      { Name => 'BToD3', Binary => 1 }, D2B0 =>
      { Name => 'DToB0', Binary => 1 }, D2B1 =>
      { Name => 'DToB1', Binary => 1 }, D2B2 =>
      { Name => 'DToB2', Binary => 1 }, D2B3 =>
      { Name => 'DToB3', Binary => 1 }, rig0 =>
      { Name => 'PerceptualRenderingIntentGamut',
        PrintConv => { prmg => 'Perceptual Reference Medium Gamut', }, },
    rig2 => { Name => 'SaturationRenderingIntentGamut',
        PrintConv => { prmg => 'Perceptual Reference Medium Gamut', }, },
    meta => { Name => 'Metadata',
        SubDirectory => {
    ...
    smap   => 'SurfaceMap',
    hdgm   => { Name => 'HDGainMapInfo', Binary => 1 },

    Header => {
        Name         => 'ProfileHeader',
        SubDirectory => { TagTable => 'Image::ExifTool::ICC_Profile::Header', },
    },
);
...
%Image::ExifTool::ICC_Profile::Header = (
    ...
    24 => {
        ...
        Format => 'int16u[6]',
        ValueConv =>
          'sprintf("%.4d:%.2d:%.2d %.2d:%.2d:%.2d",split(" ",$val));',
        PrintConv => '$self->ConvertDateTime($val)',
    },
    36 => {
...
sub FormatICCTag($$$) {
    ...
    if ( $type eq 'dtim' and $size >= 20 ) {
        return sprintf(
            "%.4d:%.2d:%.2d %.2d:%.2d:%.2d",
            Get16u( $dataPt, $offset + 8 ),
            Get16u( $dataPt, $offset + 10 ),
            Get16u( $dataPt, $offset + 12 ),
            Get16u( $dataPt, $offset + 14 ),
            Get16u( $dataPt, $offset + 16 ),
            Get16u( $dataPt, $offset + 18 )
        );
    }
    if ( $type eq 'sf32' ) {
        return ReadValue(
    ...
    if ( $type eq 'XYZ ' ) {
        ...
        for ( $pos = 8 ; $pos + 12 <= $size ; $pos += 12 ) {
            $str and $str .= ', ';
            $str .=
              ReadValue( $dataPt, $offset + $pos, 'fixed32s', 3, $size - $pos );
        }
        return $str;
    }
    return undef;
...
sub ProcessMetadata($$$) {
    ...
    for ( $index = 0 ; $index < $num ; ++$index ) {
        ...
        next unless $namePtr and $valuePtr;
        if (   $namePtr < $minPtr
            or $namePtr + $nameLen > $dirLen
            or $valuePtr < $minPtr
            or $valuePtr + $valueLen > $dirLen )
        {
...
sub ValidateICC($) {
    ...
    my $col = substr( $$valPtr, 16, 4 );
    my $con = substr( $$valPtr, 20, 4 );
    my $match =
'(XYZ |Lab |Luv |YCbr|Yxy |RGB |GRAY|HSV |HLS |CMYK|CMY |[2-9A-F]CLR|nc..|\0{4})';
...
sub ProcessICC_Profile($$$) {
    ...
    if (
        (
            ( $$et{TAGS_FROM_FILE} and not $$et{EXCL_TAG_LOOKUP}{icc_profile} )
            or $$et{REQ_TAG_LOOKUP}{icc_profile}
        )
        and ( not $$dirInfo{Name} or $$dirInfo{Name} eq 'ICC_Profile' )
      )
    {
        ...
        if ( $size > 4 ) {
            ...
            if ( $fmt eq 'mluc' and not $subdir ) {
                ...
                for ( $i = 0 ; $i < $count ; ++$i ) {
                    ...
                    my $langInfo;
                    if (    $lang =~ s/^([a-z]{2})([A-Z]{2})$/\L$1-\U$2/i
                        and $lang ne 'en-US' )
                    {
                        $langInfo =
                          Image::ExifTool::GetLangInfo( $tagInfo, $lang );
                    }
