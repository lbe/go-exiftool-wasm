...
my %geoTiffFormat = (
    0      => 'int16u', 0x87af => 'int16u', 0x87b0 => 'double',
    0x87b1 => 'string',
);
...
%Image::ExifTool::GeoTiff::Main = (
    ...
    3072 => {
        ...
        PrintConv => {
            ...
            3786 => 'Popular Visualisation CRS / Mercator', 3857 =>
              'WGS 84 / Pseudo-Mercator', 20137 => 'Adindan UTM zone 37N',
    ...
    3075 => {
        ...
        PrintConv => {
            ...
            6     => 'Oblique Mercator Spherical', 7 => 'Mercator',
            ...
            16    => 'Oblique Stereographic', 17 => 'Equirectangular',
            18    => 'Cassini Soldner',
            19    => 'Gnomonic',
...
sub ProcessGeoTiff($) {
    ...
    if (    length($$dirData) >= 8
        and length($$dirData) >= 8 * ( Get16u( $dirData, 6 ) + 1 ) )
    {
        ...
        for ( $i = 0 ; $i < $numEntries ; ++$i ) {
            ...
            if ( not $format ) {
                $et->Warn(
                    "Unknown GeoTiff location ($loc) for $$tagInfo{Name}");
                next;
            }
            elsif ( $format eq 'double' ) { $dataPt = $doubleData;
            }
            elsif ( $format eq 'string' ) { $dataPt = $asciiData;
            }
            elsif ( $format eq 'int16u' ) { $dataPt = $dirData;
                unless ($loc) { $count = 1;
                    $offset = ( $pt + 6 ) / 2;
                }
            }
            ...
            if ( not $dataPt
                or length($$dataPt) < $size * ( $offset + $count ) )
            {
            ...
            $offset *= $size;
            $val =
              Image::ExifTool::ReadValue( $dataPt, $offset, $format,
