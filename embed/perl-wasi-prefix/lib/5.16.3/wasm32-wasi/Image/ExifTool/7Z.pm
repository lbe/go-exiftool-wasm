...
sub ReadPackInfo {
    ...
    my @packsizes;
    if ( $pid == 9 ) { for ( my $i = 0 ; $i < $numstreams ; $i++ ) {
...
sub RetrieveCodersInfo {
    ...
    if ( $pid == 0x0a ) { my $numfolders = scalar(@folders);
    ...
    if ( $pid != 0x00 ) { $et->VPrint( 0, "Invalid PID: $pid\n" );
        return 0;
    }
    return 1;
}
...
sub ReadUnpackInfo {
    ...
    return 0
      unless RetrieveCodersInfo( $et, $_[0], $out_unpackinfo{"folders"} );
...
sub ReadSubstreamsInfo {
    ...
    $_[0]->Read( $buff, 1 );
    my $pid = ord($buff);
    if ( $pid == 13 ) { $et->VPrint( 0, "Num unpack stream detected.\n" );
        for ( my $i = 0 ; $i < $numfolders ; $i++ ) {
            push(
                @{ $out_substreamsinfo{"num_unpackstreams_folders"} },
                ReadUInt64( $_[0] )
    ...
    if ( $pid == 9 ) { $et->VPrint( 0, "Size property detected.\n" );
        $out_substreamsinfo{"unpacksizes"} = ();
        for (
            my $i = 0 ;
            $i <
            scalar( @{ $out_substreamsinfo{"num_unpackstreams_folders"} } ) ;
            $i++
          )
        {
            my $totalsize = 0;
            for (
                my $j = 1 ;
                $j < @{ $out_substreamsinfo{"num_unpackstreams_folders"} }[$i] ;
                $j++
              )
            {
    ...
    for ( my $i = 0 ; $i < $numfolders ; $i++ ) {
        my $numsubstreams =
          @{ $out_substreamsinfo{"num_unpackstreams_folders"} }[$i];
    ...
    if ( $pid == 10 ) { $et->VPrint( 0, "CRC property detected.\n" );
        my @crcs;
...
sub ReadStreamsInfo {
    ...
    $_[0]->Read( $buff, 1 );
    my $pid = ord($buff);
    if ( $pid == 6 ) { my $packinfo = ReadPackInfo( $et, $_[0] );
        return 0 unless $packinfo;
        $out_streamsinfo{"packinfo"} = $packinfo;
        $_[0]->Read( $buff, 1 );
        $pid = ord($buff);
    }
    if ( $pid == 7 ) { $et->VPrint( 0, "Unpack info data detected.\n" );
    ...
    if ( $pid == 8 ) { $et->VPrint( 0, "Substreams info data detected.\n" );
        my $substreamsinfo = ReadSubstreamsInfo(
            $et, $_[0],
            $unpackinfo->{"numfolders"},
            $unpackinfo->{"folders"}
        );
...
sub GetDecompressor {
    ...
    @{ $out_decompressor{"_unpacked"} } =
      (0) x scalar( @{ $out_decompressor{"_unpacksizes"} } );
...
sub ReadFilesInfo {
    ...
    while (1) {
        $_[0]->Read( $buff, 1 );
        my $prop = ord($buff);
        if ( $prop == 0 ) { return \@out_files;
        }
        my $size = ReadUInt64( $_[0] );
        if ( $prop == 25 ) { $_[0]->Seek( $size, 1 );
            next;
        }
        $_[0]->Read( $buff, $size );
        my $buffer = File::RandomAccess->new( \$buff );
        if ( $prop == 14 ) { my @isempty = ReadBoolean( $buffer, $numfiles, 0 );
            my $numemptystreams = 0;
            for ( my $i = 0 ; $i < $numfiles ; $i++ ) {
                if ( $isempty[$i] == 0 ) {
        ...
        elsif ( $prop == 17 ) { $et->VPrint( 0, "Name prop detected.\n" );
            my $external;
        ...
        elsif ( $prop == 20 ) { $et->VPrint( 0, "Last write time detected.\n" );
            ReadTimes( $et, $buffer, $numfiles, "lastwritetime", \@out_files );
        }
        elsif ( $prop == 21 ) { $et->VPrint( 0, "File attributes detected.\n" );
            my $external;
...
sub Process7Z($$) {
    ...
    my $tagTablePtr = GetTagTable('Image::ExifTool::ZIP::RAR5');
    $et->HandleTag( $tagTablePtr, 'FileVersion',
        sprintf( '7z v%d.%.2d', $major_version, $minor_version ) );
    ...
    $et->VPrint( 0,
        "NextHeaderOffset: $nextheaderoffset, NextHeaderSize: $nextheadersize\n"
    );

    $raf->Seek( $nextheaderoffset, 1 );
    $raf->Read( $buff, 1 );
    my $pid = ord($buff);
    if ( $pid == 1 )
    { $et->VPrint( 0, "Normal header detected. trying to decode\n" );
    ...
    elsif ( $pid == 23 ) { unless ( eval { require Compress::Raw::Lzma } ) {
            $et->Warn(
                "Install Compress::Raw::Lzma to read encoded 7z information");
            return 1;
        }
        ...
        foreach my $folder ( @{ $streamsinfo->{"unpackinfo"}->{"folders"} } ) {
            ...
            my $decomporessor =
              GetDecompressor( $et, $folder, $compressed_size );
            if ( $decomporessor == 0 ) {
                $et->Warn("Invalid or corrupted file.");
                return 1;
            }
            ...
            while ( $remaining > 0 ) {
                $folder_data .=
                  Decompress( $et, $raf, $decomporessor, $remaining );
