...
my %xmlTag = (
    lat => 'lat', latitude => 'lat', latitudedegrees => 'lat', lon =>
      'lon', longitude      => 'lon', longitudedegrees => 'lon',  ele =>
      'alt', elevation      => 'alt', alt              => 'alt',  altitude =>
      'alt', altitudemeters => 'alt', 'time'           => 'time', fix =>
      'fixtype', hdop  => 'hdop',  vdop => 'vdop', pdop => 'pdop', sat =>
      'nsats',   atemp => 'atemp', when => 'time', coordinates =>
      'coords', coord => 'coords', begin => 'begin', end => 'time', course =>
      'dir', pitch => 'pitch', roll => 'roll', speed =>
      'speed', accuracy_horizontal => 'err',  wpt => '', trkpt => '', rtept =>
      '', trackpoint => '', placemark => '', );
        ...
        if ( not $format ) {
              ...
              )
            {
                ...
                foreach (@csvHeadings) {
                    ...
                    my $xtra = '';
                    s/^GPS ?//;
                    if (/^Time ?\(seconds\)$/i) {   $param = 'runtime';
                        if (    $trackFile
                            and $trackFile =~
...
'Error getting start time from file name for DJI CSV track file';
                    ...
                    elsif (/^Date ?Time/i) { $param = 'datetime';
                    }
                    elsif (/^Date/i) {
                        $param = 'date';
                    }
                    elsif (/^Time(?! ?\(text\))/i) { $param = 'time';
                    }
                    elsif (/^(Pos)?Lat/i) {
                        $param = 'lat';
                        /ref$/i and $param .= 'ref';
                        ...
                        elsif ($isColumbus) { $scaleSpeed = $otherConv{'km/h'};
                            $xtra = " in km/h";
                        }
                        else {
                            $xtra = ' in knots';
...
/"(latitudeE7|longitudeE7|latE7|lngE7|timestamp|startTime|point|durationMinutesOffsetFromStartTime|time)"\s*:\s*"?(.*?)"?,?\s*[\x0d\x0a]/
              )
            {
                ...
                elsif ( $1 eq 'startTime' ) { $startTime = GetTime($2);
                }
...
/^\$PMGNTRK,(\d+)(\d{2}\.\d+),([NS]),(\d+)(\d{2}\.\d+),([EW]),(-?\d+\.?\d*),([MF]),(\d{2})(\d{2})(\d+(\.\d*)?),A,(?:[^,]*,(\d{2})(\d{2})(\d+))?/
        ...
        if (
            ...
            or (
                ...
                and (
                       not defined $fixSecs
                    or ( $secs >= $fixSecs and $secs - $fixSecs >= 10 )
                    or
...
"  Interpolating $category between fixes (f=$f):\n",
    ...
    if ($fix) {
        ...
        if ( $geoloc and $geoloc =~ /\bgeotag\b/i ) {
            my $tag =
              ( $$nvHash{WantGroup} ? "$$nvHash{WantGroup}:" : '' )
              . 'Geolocate';
