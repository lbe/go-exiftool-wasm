...
my %versionInfo = (
    Format => 'undef[7]',
    ValueConv =>
      'sprintf("%.2x.%.2x %.4x:%.2x:%.2x Rev.%s", unpack("CCvCCa", $val))',
    ValueConvInv => q{
...
%Image::ExifTool::Reconyx::HyperFire = (
    ...
    0x00 => { Name => 'MakerNoteVersion',
        Writable     => 0, PrintConv => 'sprintf("0x%.4x", $val)',
        PrintConvInv => 'hex $val', },
    0x01 => { Name => 'FirmwareVersion',
        Format   => 'int16u[3]',
        Writable => 0, PrintConv => '$val=~tr/ /./; $val', },
    0x04 => { Name => 'FirmwareDate',
        Format    => 'int16u[2]',
        ValueConv => q{
            my @v = split(' ',$val);
    ...
    0x06 => {
        ...
        Format    => 'string[2]',
        PrintConv => {
            C => 'CodeLoc Not Entered', E => 'External Sensor', M =>
              'Motion Detection',
            T => 'Time Lapse',
        },
    },
    ...
    0x24 => 'Contrast', 0x25 => 'Brightness', 0x26 => 'Sharpness', 0x27 =>
      'Saturation', 0x28 => {
        Name      => 'InfraredIlluminator',
        PrintConv => { 0 => 'Off', 1 => 'On' },
      },
    0x29 => 'MotionSensitivity', 0x2a => { Name => 'BatteryVoltage',
        ValueConv    => '$val / 1000',
...
%Image::ExifTool::Reconyx::UltraFire = (
    ...
    0x1f         => { Name => 'Micro1Version', %versionInfo }, 0x26 =>
      { Name => 'BootLoaderVersion', %versionInfo }, 0x2d =>
      { Name => 'Micro2Version', %versionInfo }, 0x34 => {
        Name      => 'TriggerMode',
        Format    => 'undef[1]',
        PrintConv => {
            M => 'Motion Detection',
    ...
    0x3b => {
        ...
        Priority    => 0, Shift => 'Time',
        ValueConv =>
'sprintf("%.4d:%.2d:%.2d %.2d:%.2d:%.2d", reverse unpack("C5v",$val))',
        ValueConvInv => q{
            my @a = ($val =~ /\d+/g);
            return undef unless @a >= 6;
...
'my ($y,$d) = split " ", $val; sprintf("%.4x:%.2x:%.2x",$y,$d>>8,$d&0xff)',
    ...
    0x34 => {
        Name      => 'TriggerMode', Format => 'string[2]',
        PrintConv => {
            M => 'Motion Detection', T => 'Time Lapse', P =>
              'Point and Shoot', },
    },
    0x36 => {
        Name         => 'Sequence',
