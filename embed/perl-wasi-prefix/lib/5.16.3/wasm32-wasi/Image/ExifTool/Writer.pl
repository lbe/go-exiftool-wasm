...
my %tiffMap = (
    ...
    InteropIFD   => 'ExifIFD',
    MakerNotes   => 'ExifIFD',
    CanonVRD     => 'MakerNotes', NikonCapture => 'MakerNotes', PhaseOne =>
      'MakerNotes', );
my %exifMap = (
    IFD1         => 'IFD0',
...
my %jpegMap = (
    ...
    MetaIFD     => 'Meta',
    RMETA       => 'APP5',
    SEAL        => [ 'APP8', 'APP9' ], AROT => 'APP10',
    JUMBF       => 'APP11',
    Ducky       => 'APP12',
    Photoshop   => 'APP13',
    Adobe       => 'APP14',
    IPTC        => 'Photoshop',
    MakerNotes => [ 'ExifIFD', 'CIFF' ], CanonVRD =>
      'MakerNotes', NikonCapture => 'MakerNotes', Comment => 'COM',
);
my %dirMap = (
    JPEG => \%jpegMap,
    EXV  => \%jpegMap,
...
my %exifDirs = (
    ...
    previewifd   => 'PreviewIFD', metaifd => 'MetaIFD', makernotes =>
      'MakerNotes',
);
my %allFam0 = (
    exif       => 1,
...
sub SetNewValue($;$$%) {
    ...
    if ($wantGroup) {
        foreach ( split /:/, $wantGroup ) {
            ...
            if ( $g =~ s/^ID-//i ) { return 0 if defined $f and $f ne 7;
                $wantGroup[-1] = [ 7, $g ];
            }
            elsif ( defined $f ) {
                $f > 2 and return 0;
...
" (specify a writable tag name, not '${origTag}' literally)";
  ...
  TAG: foreach $tagInfo (@matchingTags) {
        ...
        if (
            not $writeGroup
            or ( $translateWriteGroup{$writeGroup}
                and
    ...
    foreach $tagInfo (@tagInfoList) {
        ...
        if ( defined $shift ) {
            ...
            if (    ( defined $shiftable and not $shiftable )
                and ( $shift or ( $shiftable eq '0' and $options{DelValue} ) ) )
            {
...
"$$self{INDENT2}$verb $wgrp1:$tag$fromList if value is '${_}'\n";
            ...
            if ( $$pref{$tagInfo} or $prf ) {
                ...
                elsif (( $$tagInfo{List} and not $options{DelValue} )
                    or not( $$nvHash{DelValue} and @{ $$nvHash{DelValue} } )
                    or grep( /^$/, @{ $$nvHash{DelValue} } ) )
                {
                    $$nvHash{IsCreating} =
        ...
        if ($writeAlso) {
            ...
            foreach $wtag ( sort keys %$writeAlso ) {
                my %opts = (
                    ...
                    Shift     => $options{Shift},
                    Replace => $options{Replace}, CreateGroups => $createGroups,
                    SetTags => \%alsoWrote, );
                undef $evalWarning;
...
sub SetNewValuesFromFile($$;@) {
    ...
    if ( ref $srcFile and UNIVERSAL::isa( $srcFile, 'Image::ExifTool' ) ) {
        ...
        $srcExifTool->Options(
            Binary      => 1,
            CoordFormat => $$options{CoordFormat}
              || '%d %d %.8f', Duplicates => 1,
            LimitLongValues => 10000000, List => 1,
            MakerNotes      => $$options{FastScan}
              && $$options{FastScan} > 1 ? undef : 1,
            RequestAll => $$options{RequestAll}
              || 1, StrictDate => defined $$options{StrictDate}
            ? $$options{StrictDate}
            : 1,
            Struct => $structOpt,
        );
...
sub SetFileModifyDate($$;$$$) {
    ...
    if ( $isOverwriting < 0 ) {  unless ( defined $originalTime ) {
...
sub Sanitize($$) {
    ...
    if ( $$self{OPTIONS}{Escape} ) {
        if ( $$self{OPTIONS}{Escape} eq 'XML' ) {
            ...
            $$valPt =
              Image::ExifTool::HTML::UnescapeHTML( $$valPt,
    ...
    if ( %{ $$self{DEL_GROUP} } ) {
        ...
        if ($delFlag) {
            if ( $$dirInfo{Permanent} ) {
                ...
                and $self->IsRawType()
                and (  not $$dirInfo{TagInfo}
                    or not defined $$dirInfo{TagInfo}{Permanent}
                    or $$dirInfo{TagInfo}{Permanent} )
                and not( $self->IsRawType() == 2 and $parent eq 'ExifIFD' )
              )
            {
...
sub VerboseInfo($$$%) {
    ...
    elsif ( $parms{Name} ) { $tag = $parms{Name};
        undef $hexID;
    }
    else {
        my $prefix;
...
sub WriteJPEG($$) {
  ...
  Prescan: for ( ; ; ) {
            ...
            and ( $marker < 0xd0 or $marker > 0xd7 ) )
        {
            ...
            $len -= 2;
            if ( ( $marker & 0xf0 ) == 0xe0 ) { my $n = $len < 64 ? $len : 64;
            ...
            and ( $marker == 0xc0 or $marker & 0x03 ) )
        {
            ...
            if ($oldOutfile) {
                ...
                if ( $$self{PREVIEW_INFO} ) {
                    ...
                    if ( length($buff) < $scanLen ) { my $buf2;
                        $buff .= $buf2
            ...
            {  if ( $$segDataPt =~ /^(.{0,4})Exif\0./is )
                {
                    ...
                    my %dirInfo = (
                        DataPt  => $segDataPt,
                        DataPos => -$hdrLen, DirStart => $hdrLen,
                        Base    => $segPos + $hdrLen,
                        Parent  => $markerName,
                        DirName => 'IFD0',
                    );
...
"Reading non-standard extended XMP (GUID $guid)"
                    ...
                    if ( defined $newData ) {
                        ...
                        if ( $doneDir{XMP} > 1 ) {
                            ...
                            foreach $dat ( @{ $extendedXMP{Main} } ) { next
                                  unless length $dat;
                                $s = pack( 'n',
                            ...
                            foreach $guid ( sort keys %extendedXMP ) { next
            ...
            elsif ( $marker == 0xea ) { if ( $$segDataPt =~ /^AROT\0\0/ ) {
                    $segType = 'AROT';
                    $$delGroup{AROT} and $del = 1;
                }
            }
            elsif ( $marker == 0xeb ) { if ( $$segDataPt =~ /^JP/ ) {
                    $segType = 'JUMBF';
            ...
            elsif ( $marker == 0xed ) { if ( $$segDataPt =~ /^$psAPP13hdr/ ) {
                    $segType = 'Photoshop';
                    if ( defined $combinedSegData ) {
                        $combinedSegData .=
            ...
            elsif ( $marker == 0xfe ) { my $newComment;
                unless ( $doneDir{COM} ) {
                    $doneDir{COM} = 1;
...
"$$tagInfo{DataTag} is too large (64 KiB max. for this file)"
    ...
    if ( $$tagTablePtr{IS_SUBDIR} ) {
        ...
        foreach $tagID ( @{ $$tagTablePtr{IS_SUBDIR} } ) {
            ...
            my $dat =
              $self->WriteDirectory( \%subdirInfo,
