...
my %defaults = (
    size => [ 800, 600 ], margin => [ 60, 15, 15, 30 ], legend => [ 0, 0 ]
    , txtpad => [ 10, 10 ], linespacing => 20,  cols => [
        qw(red green blue black orange gray fuchsia brown turquoise gold
          lime violet maroon aqua navy pink olive indigo silver teal)
    ],
    marks =>
      [qw(circle square triangle diamond star plus pentagon left down right)],
    stroke => 1, grid => 'darkgray', text => 'black', type => 'line', style =>
      '', xlabel => '', ylabel => '', title => '', nbins => 20,      Data => {}
    , Name => [],    );
...
my %markerData = (
    ...
    star =>
'<path d="M4 0.8 L5 2.625 7.043 3.011 5.617 4.525 5.881 6.589 4 5.7 2.119 6.589 2.383 4.525 0.957 3.011 3 2.625 z"',
    plus =>
'<path d="M2.75 1 l2.5 0 0 1.75 1.75 0 0 2.5 -1.75 0 0 1.75 -2.5 0 0 -1.75 -1.75 0 0 -2.5 1.75 0 z"',
    pentagon =>
      '<path d="M4 1 L6.853 3.073 5.763 6.427 2.237 6.427 1.147 3.073 z"',
...
sub new {
    ...
    foreach ( keys %defaults ) {
        ref $defaults{$_} eq 'HASH'
          and $$self{$_} = { %{ $defaults{$_} } }, next;
        ref $defaults{$_} eq 'ARRAY'
          and $$self{$_} = [ @{ $defaults{$_} } ], next;
        $$self{$_} = $defaults{$_};
    }
    return $self;
...
sub AddPoints($$$) {
    ...
    my ( $ee, $docNum, $data, $xmin, $xmax ) =
      @$self{qw(EE DocNum Data XMin XMax)};
    $$self{type} or $$self{type} = 'line';
    my $scat     = $$self{type} =~ /^s/ ? 1 : 0;
    my $xname    = $$self{Name}[0];
...
sub Draw($$) {
    my ( $self, $fp ) = @_;
    my ( $min, $max, $xmin, $xmax, $name, $style ) =
      @$self{qw(Min Max XMin XMax Name style)};
...
<title>$tmp</title>};
    for ( $plotNum = 0 ; $plotNum < $numPlots ; ++$plotNum ) {
        ...
        if ($hist) {
            $$self{nbins} > 0
              or $$self{Error} = 'Invalid number of histogram bins', return;
            $noLegend = 1;
        ...
        $min =
          ( $min > 0 ? int( $min / $dy ) : int( $min / $dy - 0.9999 ) ) * $dy;
        $max =
          ( $max > 0 ? int( $max / $dy + 0.9999 ) : int( $max / $dy ) ) * $dy;
        ...
        if ($hist) {
            ...
            if ( $$self{ymax} ) {
                ...
                $max < $_ and $max = $_ foreach @$hist;
            }
            $diff = GetRange( $min, $max );
            $dx   = $dy;
            $dy   = GetGridSpacing( $diff / $og[1] );
            $max =
              ( $max > 0 ? int( $max / $dy + 0.9999 ) : int( $max / $dy ) ) *
              $dy;
            $$data{ $name[0] } = $hist;
        }
        else {
        ...
        if ($scat) {
            $xmin =
              ( $xmin > 0 ? int( $xmin / $dx ) : int( $xmin / $dx - 0.9999 ) )
              * $dx;
            $xmax =
              ( $xmax > 0 ? int( $xmax / $dx + 0.9999 ) : int( $xmax / $dx ) )
              * $dx;
        }
        $diff  = GetRange( $min,  $max );
        $xdiff = GetRange( $xmin, $xmax );
        ...
        print $fp
"<rect x='$clip[0]' y='$clip[1]' width='$clip[2]' height='$clip[3]'/></clipPath>";

        if ( $style =~ /\b[mp]/ ) { for ( $i = 0 ; $i < @name ; ++$i ) {
                my @m = split /-/,
                  (      $$marks[$i]
                      || $defaults{marks}[ $i % @{ $defaults{marks} } ] );
                my ( $fill, $mark );
                $fill = $m[2] || $$cols[$i]
                  if $m[1] ? $m[1] =~ /^f/ : $style =~ /\bf/;
                $mark = $markerData{ $m[0] };
                $mark or $markID{$mark} = '', next;
...
"\n<marker id='$markID{$i}' markerWidth='8' markerHeight='8' refX='4'",
        ...
        print $fp "\n  text { fill: $$self{text} }\n</style>";
        print $fp
"\n<rect x='0' y='0' width='$size[0]' height='$size[1]' fill='$$self{bkg}'/>"
          if $$self{bkg};
        print $fp "\n<!-- X axis -->";
...
"\n<text x='10' y='${y}' transform='rotate(-90,10,$y)'>$ylabel</text>";
        ...
        for ( $x = int( $xmin / $dx - 1 ) * $dx ; ; $x += $dx ) {
            $px = int(
                ( $margin[0] + ( $x - $xmin ) * $width / $xdiff ) * 10 + 0.5 )
              / 10;
            ...
            if ( not $dx2
                or
                abs( $x / $dx2 - int( $x / $dx2 + ( $x > 0 ? 0.5 : -0.5 ) ) ) <
                0.01 )
            {
                printf $fp "\n<text x='${px}' y='${py}'>%g</text>", $x;
                $h += $$tpad[1] / 2;
            }
            length($grid) - $lastLen > 80
              and $grid .= "\n", $lastLen = length($grid);
...
"\n</g>\n<!-- Y axis -->\n<g dominant-baseline='middle' text-anchor='end'>";
        ...
        ( $grid, $lastLen ) = ( '', 0 );
        my ( $gx, $gw ) =
          ( $margin[0] - $$tpad[0] / 2, $width + $$tpad[0] / 2 );
        for ( $y = $min ; ; $y += $dy ) {
            $py =
              int( ( $margin[1] + $height - ( $y - $min ) * $yscl ) * 10 + 0.5 )
              / 10;
...
"\n<path stroke='$$self{text}' fill='none' d='M$margin[0] $margin[1] l0 $height $width 0 0 -$height z'/>";
        for ( $i = 0 ; $i < @name and not $noLegend ; ++$i ) {
            ...
            $y =
              $margin[1] +
              $$self{legend}[1] + 15 +
              $$self{linespacing} * ( $i + 0.5 );
            my $col = $$cols[$i];
            my $mark =
              $markID{$i} ? " marker-end='url(#$markID{$i})' fill='none'" : '';
            my $line =
              ( $style =~ /\bl/ )
              ? ' l-20 0'
              : sprintf( ' m%.4g 0', -5 * $wid );
            my $sw = ( $style =~ /\bm/ ? 1.5 : 2 ) * $wid;
            print $fp
...
"\n</g>\n<!-- Datasets -->\n<g fill='none' clip-path='url(#plot-area)'",
        ...
        foreach (@name) {
            ...
            print $fp ' V', $margin[1] + $height, " H$margin[0] z"
              if $hist and $fill;
            print $fp "'/>";
        }
        print $fp "\n</g>";
