...
sub ReadRecord($) {
    ...
    if ( $type == 0 ) { $buff = VarInt($dirInfo);
    }
    elsif ( $type == 1 ) { $buff = GetBytes( $dirInfo, 8 );
    }
    elsif ( $type == 2 ) { my $len = VarInt($dirInfo);
        if ($len) {
    ...
    elsif ( $type == 5 ) { $buff = GetBytes( $dirInfo, 4 );
    }
    ...
    for ( ; ; ) {
        ...
        if ( $$tagInfo{Format} ) {
            if ( $type == 0 ) {
                ...
                $val =
                  ReadValue( \$buff, 0, $$tagInfo{Format}, undef,
                    length($buff) );
            }
        }
        elsif ( $type == 0 ) { $val = $buff;
        ...
        elsif ( $type == 2 ) { if ( $$tagInfo{SubDirectory} ) {
            }
