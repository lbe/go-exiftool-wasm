...
%Image::ExifTool::PostScript::Main = (
    ...
    Copyright   => { Priority => 0, Writable => 'string' }, CreationDate => {
        Name         => 'CreateDate',
        Priority     => 0,
        Groups       => { 2 => 'Time' },
        Writable     => 'string',
    ...
    Pages   => { Priority => 0 },
    Routing => { Priority => 0, Writable => 'string' }, Subject =>
      { Priority => 0, Writable => 'string' },
    Title   => { Priority => 0, Writable => 'string' },
    Version => { Priority => 0, Writable => 'string' },  BeginPhotoshop => {
        Name         => 'PhotoshopData',
...
'extracted with L<ExtractEmbedded|../ExifTool.html#ExtractEmbedded> option',
    ...
    AI8_CreatorVersion   => { Name => 'AICreatorVersion' }, AI12_BuildNumber =>
      { Name => 'AIBuildNumber' },
);
...
sub SplitLine($$) {
    ...
    for ( ; ; ) {
        ...
        if ( $$dataPt =~ /\x0d/g ) { if ( defined $endl ) {
