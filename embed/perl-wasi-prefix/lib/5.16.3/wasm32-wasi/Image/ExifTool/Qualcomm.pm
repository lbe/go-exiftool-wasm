...
%Image::ExifTool::Qualcomm::Main = (
    ...
    VARS         => { ID_FMT => 'none', NO_LOOKUP => 1 }, NOTES => q{
...
%Image::ExifTool::Qualcomm::DualCamera = (
    ...
    'Current pipeline mirror flip setting' =>
      'CurrentPipelineMirrorFlipSetting',
...
sub ProcessQualcomm($$$) {
    ...
    while ( $pos + 3 < $dirEnd ) {
        ...
        unless ( defined $$tagTablePtr{$tag}
            or $Image::ExifTool::specialTags{$tag} )
        {
            my %tagInfo;
            if ( MakeNameAndDesc( $tag, \%tagInfo ) ) {
                $et->VPrint( 0, $$et{INDENT},
                    "[adding Qualcomm:$tagInfo{Name}]\n" );
