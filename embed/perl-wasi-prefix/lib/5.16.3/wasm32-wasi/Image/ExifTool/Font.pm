...
my %ttCharset = (
    ...
    Unicode => {
        0 => 'UCS2', 1 => 'UCS2', 2 => 'UCS2', 3 => 'UCS2', 4 => 'UTF16',  },
...
%Image::ExifTool::Font::Main = (
    ...
    name => { SubDirectory => { TagTable => 'Image::ExifTool::Font::Name' }, },
    C2PA => {
        SubDirectory =>
          { TagTable => 'Image::ExifTool::Jpeg2000::Main', Start => 20 },
    },
    PFM => {
        Name         => 'PFMHeader',
...
%Image::ExifTool::Font::Name = (
    ...
    2 => 'FontSubfamily',
    3 => 'FontSubfamilyID',
    4 => 'FontName', 5 => 'NameTableVersion',
    6 =>
      { Name => 'PostScriptFontName', Description => 'PostScript Font Name' },
    7  => 'Trademark',
    8  => 'Manufacturer',
    9  => 'Designer',
    10 => 'Description',
...
sub Unbrotli($$) {
    ...
    unless ( eval { require IO::Uncompress::Brotli } ) {
        $et->Warn(
            'Install IO::Compress::Brotli to decode Brotli-compressed metadata'
        );
        return 0;
    }
...
sub ProcessWOFF($$) {
    ...
    if ( $type eq 'woff' ) {
        unless ($raf->Seek( $off + 24, 0 )
            and $raf->Read( $tbl, $numTables * 20 ) == $numTables * 20 )
        {
        ...
        for ( $i = 0 ; $i < $numTables ; ++$i ) {
            ...
            next unless $processTag{$tag} or $verbose;
            $raf->Seek( $pos, 0 ) and $raf->Read( $buff, $compLen ) == $compLen
              or $et->Warn('Bad font table entry'), return 1;
            my $dataPos;
            if ( $compLen eq $len ) {
                $dataPos = $pos;
            }
    ...
    else { my $compSize = Get32u( \$buff, 20 );
        my ( $err, @entry, $entry );
        $raf->Seek( $off + 24, 0 )
          or $et->Warn('Error seeking to font table'), return 1;
        for ( $i = 0 ; $i < $numTables ; ++$i ) {
        ...
        if ( $flavor eq 'ttcf' ) {
            ...
            defined $n and $raf->Seek( 4, 1 )
              or $et->Warn('Error reading collection table'), return 1;
        ...
        $raf->Read( $buff, $compSize ) == $compSize
          or $et->Warn('Error reading font data'), return 1;
        ...
        foreach $entry (@entry) {
            ...
            if ( $processTag{$tag} or $verbose ) {
                ...
                my $err = ProcessTableEntry( $et, $i, $tag, \$dat, undef,
                    $transformed );
                $err and $et->Warn($err), return 1;
                last if defined $err;
            }
    ...
    if ($metaLen) {
        unless ($raf->Seek( $metaPos, 0 )
            and $raf->Read( $buff, $metaLen ) == $metaLen )
        {
        ...
        else { return 1 unless Unbrotli( $et, \$buff );
        }
...
sub ProcessFont($$) {
    ...
    { $raf->Seek( 6, 0 ) and $et->SetFileType('PFB')
          if $1;
        require Image::ExifTool::PostScript;
        $rtnVal = Image::ExifTool::PostScript::ProcessPS( $et, $dirInfo );
    }
    elsif ( $buff =~ /^\0[\x01\x02]/
        and $raf->Seek( 0, 2 )
        and  $raf->Tell() > 117
        and $raf->Tell() == unpack( 'x2V', $buff )
        and $raf->Seek( 0, 0 )
        and $raf->Read( $buff, 117 ) == 117
        and SetByteOrder('II')
        and $raf->Seek( Get32u( \$buff, 101 ), 0 )
        and $raf->Read( $buf2, 11 ) == 11
        and lc($buf2) eq "postscript\0" )
    {
        $et->SetFileType('PFM');
        return 1 if $$et{OPTIONS}{FastScan} and $$et{OPTIONS}{FastScan} == 3;
        SetByteOrder('II');
        ...
        if (    $raf->Seek( $nameOff, 0 )
            and $raf->Read( $buff, 256 )
            and $buff =~ /^([\x20-\xff]+)\0([\x20-\xff]+)\0/ )
        {
