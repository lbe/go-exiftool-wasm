...
%Image::ExifTool::PCAP::Main = (
    ...
    'IDB-5' => {
        ...
        ValueConv =>
          'join(":", unpack("(H4)8", $val)) . "/" . unpack("x16C",$val)',
    },
    'IDB-6' => {
        Name      => 'MACAddr',
    ...
    'IDB-9' => {
        ...
        Format => 'int8u',
        RawConv =>
          '$$self{TSResol} = ($val & 0x80 ? 2 ** -($val & 0x7f) : 10 ** -$val)',
    },
    'IDB-10' => 'TimeZone',
    'IDB-11' => {
        Name      => 'Filter',
...
sub ProcessPCAP($$) {
    ...
    if ( $buff =~
        /^\xa1\xb2(\xc3\xd4|\x3c\x4d)\0.\0.|(\xd4\xc3|\x4d\x3c)\xb2\xa1.\0.\0/s
      )
    {
        $type = 'PCAP';
        my $tmp = $1 || $2;
        $$et{TSResol} =
          ( $tmp eq "\xc3\xd4" or $tmp eq "\xd4\xc3" ) ? 1e-6 : 1e-9;
        $byteOrder = $buff =~ /^\xa1/ ? 'MM' : 'II';
        $verPos = 4;
    }
    elsif (
        $buff =~ /^\x0a\x0d\x0d\x0a.{4}(\x1a\x2b\x3c\x4d|\x4d\x3c\x2b\x1a)/s )
    {
        $type      = 'PCAPNG';
        ...
        $Image::ExifTool::static_vars{OverrideFileDescription}{CAP} =
        ...
        $$et{WindowsTS} = 1;
        my $val =
          sprintf( '%.4d:%.2d:%.2d %.2d:%.2d:%.2d.%.3d', @a[ 1, 2, 4 .. 8 ] );
        ...
        if ( $raf->Seek( 40, 0 ) and $raf->Read( $buff, 8 ) == 8 ) {
            ...
            {
                $et->HandleTag( $tagTablePtr, 1 => $buff );
            }
        }
        return 1;
    }
    ...
    if ( $type eq 'PCAP' ) {
        ...
        $raf->Read( $buff, 8 ) == 8
          or $et->Warn('Truncated PCAP file'), return 1;
    ...
    for ( ; ; ) {
        ...
        if ( $opt == 0 ) { last unless $raf->Read( $buff, 20 ) == 20;
            my $dirNum = Get32u( \$buff, 4 );
            if ( $dirNum == 1 ) {
                $et->HandleTag( $tagTablePtr,
                    LinkType => Get16u( \$buff, 12 ) );
                $dir = 'IDB';
                next;
            }
            elsif ( $dirNum == 6 ) { my $ts = 4294967296 * Get32u( \$buff, 16 );
                $raf->Read( \$buff, 4 ) == 4 or last;
                if ( $$et{TSResol} ) {
                    $ts =
                      ( $ts + Get32u( \$buff, 0 ) ) * $$et{TSResol} +
                      ( $$et{TSOff} || 0 );
                    $et->HandleTag( $tagTablePtr, TimeStamp => $ts );
                }
            }
        ...
        my $n = ( $len + 3 ) & 0xfffc;
        $raf->Read( $buff, $n ) == $n
          or $et->Warn("Error reading $dir options"), last;
        my $id = $$tagTablePtr{$opt} ? $opt : "$dir-$opt";
        $et->HandleTag(
            $tagTablePtr, $id, undef,
            DataPt      => \$buff,
            DataPos     => $raf->Tell() - $n,
            Size        => $len,
            MakeTagInfo => 1,
