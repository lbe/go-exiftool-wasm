...
my %mediaMatrix = (
    ...
    OTHER => sub {
        ...
        if ($inv) {
            $val =~ s/\(.*?\)//sg;
            $val =~ s/^\s*?PLUS\s+(V[0-9]+)\s+(U[0-9]+)\s*;/|PLUS|$1|$2|/s;
            $val =~ tr/;/|/;
            $val =~ tr/0-9A-Z|//dc;
            $val .= '|' unless $val =~ /\|$/;
            ...
            while ( $code =~ /(\d[A-Z]{3})/g ) {
                ...
                elsif ( $mmid =~ /^1I([A-Z])([A-Z])/ )
                { my $n = ( ord($1) - 65 ) * 26 + ord($2) - 65 + 1;
                    $val .= "; $mmid ($n Usage Items:)";
                }
                elsif ( $mmid =~ /^1UN([A-Z])/ ) { $val .= " (Usage Number $1)";
                }
                else {
                    $val .= " $mmid";
                }
...
sub ValidateMediaSummary($) {
    ...
    my @a = split /\|/, $val;
    @a >= 4 and $a[0] eq ''
      or warn("Not a valid Media Summary Code\n"), return 0;
    ...
    for ( $i = 1 ; $i <= $numUsages ; ++$i ) {
        my $usage = $a[ $i + 3 ];
        $usage =~ /^1I([A-Z])([A-Z])/
          or warn("Missing Media Usage $i item count\n"), return 0;
        length($usage) % 4
          and warn("Incorrect Media Usage $i length\n"), return 0;
        ...
        for ( $j = 1 ; $j <= $numItems ; ++$j ) {
            ...
            $item =~ /^\d[A-Z]{3}$/
              or warn(qq(Invalid item "$item" for Media Usage $i\n)), return 0;
        }
    }
