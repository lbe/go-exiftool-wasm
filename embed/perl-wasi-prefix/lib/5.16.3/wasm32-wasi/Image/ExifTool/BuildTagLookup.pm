...
my %tweakOrder = (
    JPEG    => '-', IPTC => 'Exif', GPS => 'XMP', Composite => 'Extra',
    CBOR    => 'JSON',
    GeoTiff => 'GPS',
...
my %formatOK = (
    ...
    unsigned    => 1,
    utf8        => 1,
    Unicode     => 1, GUID => 1, vt_filetime => 1, );
...
sub new {
    ...
    my (
        %tagNameInfo,  %id,               %longID,    %longName,
        %shortName,    %tableNum,         %tagLookup, %tagExists,
        %noLookup,     %tableWritable,    %sepTable,  %case,
        %structs,      %compositeModules, %isPlugin,  %flattened,
        %structLookup, @writePseudo,      %dupXmpTag
    );
    ...
    $self->{STRUCT_LOOKUP}     = \%structLookup;
    ...
    foreach $tableName (@tableNames) {
        ...
        $isIPTC = 1
          if $writeProc and $writeProc eq \&Image::ExifTool::IPTC::WriteIPTC;

        if (   $$table{GROUPS} and $$table{GROUPS}{0} eq 'XMP'
            or $$vars{ADD_FLATTENED} )
        {
      ...
      TagID: foreach $tagID (@keys) {
            ...
            foreach $tagInfo (@infoArray) {
                if (    $binaryTable
                    ...
                    and $$tagInfo{Writable} ne '1' )
                {
                    warn(
"$tableName $$tagInfo{Name} Writable should be 1, not $$tagInfo{Writable}\n"
                    );
                }
                ...
                if (    $isXMP
                    ...
                    and ( $$tagInfo{Writable} or $$table{WRITABLE} ) )
                {
                    $dupXmpTag{$name}
                      and warn "Duplicate writable XMP tag $name\n";
                    $dupXmpTag{$name} = 1;
                }
                if ( $name !~ /^[_A-Za-z][-\w]+$/
                    and
                ...
                if (    $$tagInfo{List}
                    and $$tagInfo{List} !~ /^(1|Alt|Bag|Seq|array|string)$/ )
                {
                    warn
"Warning: Unknown List type ($$tagInfo{List}) for $name in $tableName\n";
                }
                ...
                if ( $processBinaryData and $$table{WRITABLE} ) {
                    if (
                        ...
                        and $$tagInfo{Condition} =~ /\$valPt/
                        and ( $$tagInfo{Writable}
                            or not defined $$tagInfo{Writable} )
                      )
                    {
                        warn
"Warning: Tag not writable due to Condition - $short $name\n";
                    }
                    ...
                    if ( $format and $format =~ /^var_/ ) {
                        ...
                        unless ( defined $$tagInfo{Writable}
                            and not $$tagInfo{Writable} )
                        {
                            warn
"Warning: Var-format tag is writable - $short $name\n";
                        }
                        while ( $format =~ /\$val\{(.*?)\}/g ) {
                            my $id = $1;
                            $id = hex($id) if $id =~ /^0x/;
                            $datamember{$id} = $$table{$id}{Name}
                              if ref $$table{$id} eq 'HASH';
                            unless ( $datamember{$id} ) {
                                warn
"Warning: Unknown ID ($id) used in Format - $short $name\n";
                            }
                        }
                    }
                    elsif (
                    ...
                    {
                        warn
"Warning: \$val{} used in Format of writable tag - $short $name\n";
                    }
                }
                if ( $$tagInfo{Hidden} ) {
                    if (
                        ...
                        and ( not $$tagInfo{RawConv}
                            or $$tagInfo{RawConv} !~ /Unknown|undef/ )
                      )
                    {
                        warn
"Warning: $short $name is Hidden contrary to guidelines\n";
                    }
                ...
                if ( defined $$tagInfo{Writable} ) {
                    ...
                    unless ( $formatOK{$writable}
                        or ( $writable =~ /(.*)\[/ and $formatOK{$1} ) )
                    {
                        warn
"Warning: Unknown Writable ($writable) - $short $name\n";
                    }
                }
                ...
                if (
                    $writable
                    and not( $$table{WRITE_PROC}
                        or $tableName =~ /Shortcuts/
                        or $writable eq '2' )
                  )
                {
                    undef $writable;
                ...
                if (    $$tagInfo{List}
                    and $g[2] eq 'Time'
                    and $writable
                    and not $$tagInfo{Protected}
                    and not $$tagInfo{PrintConvInv} )
                {
                    warn
"Writable List-type Time tag $g[1]:$name has no PrintConvInv and is not Protected!\n";
                }
                if (    $$tagInfo{PrintConv}
                    and $$tagInfo{PrintConv} eq '$self->ConvertDateTime($val)' )
                {
                    warn "$short $name should be in 'Time' group!\n"
                      unless $g[2] eq 'Time';
                    if (    $writable
                        and not defined $$tagInfo{Shift}
                        and $short ne 'PostScript' )
                    {
                    ...
                    if (
                        $writable
                        and ( not $$tagInfo{PrintConvInv}
                            or $$tagInfo{PrintConvInv} !~ /InverseDateTime/ )
                      )
                    {
                        warn
                          "$short $name missing InverseDateTime PrintConvInv\n";
                    }
                }
                elsif (
                    $name =~ /DateTime(?!Stamp)/
                    and ( not $$tagInfo{Groups}{2}
                        or $$tagInfo{Groups}{2} ne 'Time' )
                    and $short ne 'DICOM'
                  )
                {
                    warn "$short $name should be in 'Time' group!\n";
                ...
                if (
                    $name eq 'DateTimeOriginal'
                    and ( not $$tagInfo{Description}
                        or $$tagInfo{Description} ne 'Date/Time Original' )
                  )
                {
                    warn "Need Description for $short DateTimeOriginal\n";
                }
                if (    $$tagInfo{Description}
                    and $$tagInfo{Description} ne
                    EscapeHTML( $$tagInfo{Description} ) )
                {
                ...
                if ( ref $struct ) {
                    ...
                    if ($struct) {
                        ...
                        warn
"Missing STRUCT_NAME for structure in $$tagInfo{Name}\n";
                        undef $strTable;
                    }
                }
                elsif ($struct) {
                ...
                my $isSub =
                  (       $subdir
                      and $$subdir{Start}
                      and $$subdir{Start} =~ /\$val\b/ );
                if ( $$tagInfo{SubIFD} ) {
                    warn
"Warning: Wrong SubDirectory Start for SubIFD tag - $short $name\n"
                      unless $isSub;
                }
                else {
                ...
                if ( $isXMP
                    and ( lc $tagID ne lc $name or $$tagInfo{NotFlat} ) )
                {
                    my $note;
                    if ( $$tagInfo{NotFlat} ) {
                ...
                if ( defined $format ) {
                    unless ( $formatOK{$format}
                        or $short eq 'PICT'
                        or ( $format =~ /^(var_)?(.*)\[/ and $formatOK{$2} ) )
                    {
                        warn
"Warning: Unknown Format ($format) for $short $name\n";
                    }
                }
                else {
                    $format = $defFormat;
                ...
                if ( $$tagInfo{Mask} ) {
                    ...
                    if ($bsh) {
                        push @values,
                          sprintf( '[val >> %d & 0x%x]', $bsh, $val >> $bsh );
                    }
                    else {
                    ...
                    if ( ref $printConv eq 'HASH' ) {
                        ...
                        foreach ( keys %$printConv ) {
                            ...
                            warn
"$short $name PrintConv value $hex is not in Mask!\n";
                        }
                    }
                }
                if ( ref($printConv) =~ /^(HASH|ARRAY)$/ ) {
                    ...
                    if ( ref $printConv eq 'ARRAY' ) {
                        for ( $index = 0 ; $index < @$printConv ; ++$index ) {
                            ...
                            elsif ( $$printConv[$index]
                                and $$printConv[$index] eq 'REPEAT'
                                and $index )
                            {
                            ...
                            if (    @printConvList >= 2
                                and $printConvList[-1] eq $printConvList[-2] )
                            {
                                if ( ref $indexList[-2] ) {
                        ...
                        $valueConvHash = $$tagInfo{ValueConv}
                          if ref $$tagInfo{ValueConv} eq 'HASH';
                    }
                    while ( defined $printConv ) {
                        if ( defined $index ) {
                            ...
                            if ( ref $idx ) {
                                ...
                                for ( $i = 0 ; $i < @$idx ; ++$i ) {
                                    ...
                                    {
                                        $rngStart = $$idx[$i]
                                          unless defined $rngStart;
                                        next;
                                    }
                                    push @i,
                                      ( defined($rngStart) ? "$rngStart-" : '' )
                                      . $$idx[$i];
                                }
                                ( $idx = join ', ', @i ) =~ s/(.*),/$1 and/;
                            }
                                ...
                                while ( @printConvList
                                    and $printConv eq $printConvList[0] )
                                {
                                    shift @printConvList;
                                    $index = shift @indexList;
                                }
                        ...
                        if ( $$tagInfo{SeparateTable} ) {
                            ...
                            if ( $$tagInfo{PrintSort} ) {
                                @pk = sort {
                                    NumbersFirst( $$printConv{$a},
                                        $$printConv{$b} )
                                } keys %$printConv;
                            }
                            else {
                                @pk =
                                  sort { NumbersFirst( $a, $b ) }
                                  keys %$printConv;
                            }
                            my $n = scalar @values;
                            my ( $bits, $i, $v );
                            foreach (@pk) {
                                  ...
                                  )
                                {
                                    ...
                                    if ( $index >= 0 ) {
                                        $index =
                                          sprintf( '0x%.*x', $dig, $index );
                                    }
                                    elsif ( $format
                                        and $format =~ /int(16|32)/ )
                                    {
                                        my $mask =
                                          { 16 => 0xffff, 32 => 0xffffffff }
                                          ->{$1};
                                        $index = sprintf( '0x%.*x',
                                            $dig, $index & $mask );
                                    }
                                }
                                elsif ( $$tagInfo{PrintString}
                                    or not /^[+-]?(?=\d|\.\d)\d*(\.\d*)?$/ )
                                {
                                    if ( $index =~
s/([\x00-\x1f\x7f-\xff])/sprintf("\\x%.2x",ord $1)/eg
                                ...
                                if ( $$printConv{$_} =~ /[\0-\x1f\x7f-\xff]/ ) {
                                    warn
"Warning: Special characters in $short $name PrintConv ($$printConv{$_})\n";
                                }
                            }
                            if ($bits) {
                                my @pk =
                                  sort { NumbersFirst( $a, $b ) } keys %$bits;
                                foreach (@pk) {
                ...
                if ( $subdir and not $$tagInfo{SeparateTable} ) {
                    if ($writable) {
                        ...
                        $writable = 'yes'
                          if $tw and $writable eq '1'
                          or $writable eq '2';
                        $writable = '-' . ( $tw ? $writable : '' );
                        $writable .= '!'
                          if $tw and ( $$tagInfo{Protected} || 0 ) & 0x01;
                        $writable .= '+'
                          if $$tagInfo{List}
                          or $$tagInfo{IsList};
                    ...
                    if ( not $writable ) {
                        ...
                        $writable = $format ? $format : 'yes'
                          if $writable eq '1'
                          or $writable eq '2';
                        my $count = $$tagInfo{Count} || 1;
                        if (    $writable
                            and $format
                            and $writable ne $format
                            and $writable ne 'string'
                            and $format   ne 'string'
                            and $Image::ExifTool::Exif::formatNumber{$writable}
                            and $Image::ExifTool::Exif::formatNumber{$format} )
                        {
                            my $n1 =
                              $Image::ExifTool::Exif::formatNumber{$format};
                            my $n2 =
                              $Image::ExifTool::Exif::formatNumber{$writable};
                            $count *=
                              $Image::ExifTool::Exif::formatSize[$n1] /
                ...
                my $lcName = lc($name);
                if (    $isShortcut{$lcName}
                    and $short ne 'Shortcuts'
                    and ( $$tagInfo{Writable} or not $$tagInfo{SubDirectory} ) )
                {
                    warn "WARNING: $short $name is a shortcut tag!\n";
                }
                $name .= '?' if $$tagInfo{Unknown};
                unless (@tagNames
                    and $tagNames[-1]   eq $name
                    and $writeGroup[-1] eq $writeGroup
                    and $writable[-1]   eq $writable )
                {
                    push @tagNames,   $name;
                ...
                if ( $$tagInfo{SubDirectory} ) {
                    ...
                    unless (
                        $tagExists{$lcName}
                        and ( not $subdirs{$lcName}
                            or $subdirs{$lcName} == $tagExists{$lcName} )
                      )
                    {
            ...
            if ( $tagID =~ /^(-)?\d+(\.\d+)?$/ ) {
                if ($1) {
                    ...
                    and not $binaryTable
                    and not $isIPTC
                    and not( $short =~ /^CanonCustom/ and $tagID < 256 ) )
                {
                    $tagIDstr = sprintf( '0x%.4x', $tagID );
                }
                elsif ( $tagID < 0x10000 ) {
            ...
            elsif ( $tagID =~ /^0x([0-9a-f]+)\.(\d+)$/ ) { $tagIDstr = $tagID;
            }
            else {
                if (
                    $tagID =~ s/([\x00-\x1f\x7f-\xff])/'\x'.unpack('H*',$1)/eg )
                {
                    $tagID =~ s/\\x00/\\0/g;
                    next if $tagID eq 'jP\x1a\x1a';
                    $tagIDstr = qq{"$tagID"};
                }
                else {
                    $tagIDstr = "'${tagID}'";
            ...
            push @$info,
              [
                $tagIDstr, \@tagNames, \@writable,
                \@values,  \@require,  \@writeGroup
              ];
        }
        if ( $processBinaryData and $$table{WRITABLE} ) {
            my %lookup = (
            ...
            foreach $var ( sort keys %lookup ) {
                ...
                if ( $$table{$var} ) {
                    foreach $tagID ( @{ $$table{$var} } ) {
                        ...
                        warn
                          sprintf( "Warning: Extra %s for %s tag %d (0x%.4x)\n",
                            $var, $short, $tagID, $tagID );
                    }
                }
                foreach $tagID ( sort keys %$hash ) {
                    warn sprintf(
                        "Warning: Missing %s for %s %s, tag %s (0x%.4x)\n",
            ...
            if ( ref $tagID eq 'HASH' or $rootID ) {
                ...
                foreach (@tagIDs) {
                    if (/^\d+$/) {
                        ...
                        $quot = '"'
                          if
s/([\x00-\x1f,\x7f-\xff])/sprintf('\\x%.2x',ord($1))/ge;
                        $_ = $quot . $_ . $quot;
                    }
                }
    ...
    if ($success) {
        ...
        if ($changed) {
            rename( $tmpFile, $file )
              or warn("Error renaming $tmpFile\n"), $success = 0;
        }
        else {
            unlink($tmpFile);
        }
...
sub NumbersFirst($$) {
    ...
    if ( $a =~ /^(-?[0-9]+)(\.(\d*))?$/ ) {
        ...
        $a2 =~ s/(\d+)/sprintf("%.3d",$1)/eg
          if $a2 =~ /^(APP|DMC-\w+ |dvtm_.*|(IDB|SHB)-)?[.0-9 ]*$/
          and length($a2) < 23;
        $b2 =~ s/(\d+)/sprintf("%.3d",$1)/eg
          if $b2 =~ /^(APP|DMC-\w+ |dvtm_.*|(IDB|SHB)-)?[.0-9 ]*$/
          and length($b2) < 23;
...
sub Doc2Html($) {
    ...
    $doc =~ s/I&lt;(.*?)&gt;/<i>$1<\/i>/sg;
    $doc =~
      s{L&lt;([^&]+?)\|\Q$homePage\E/(.*?)&gt;}{<a href="../$2">$1<\/a>}sg;
    $doc =~ s{L&lt;\Q$homePage\E/(.*?)&gt;}{<a href="../$1">$1<\/a>}sg;
    $doc =~
s{L&lt;([^&]+?)\|Image::ExifTool::TagNames/(\w+) ([^/&|]+) Tags&gt;}{<a href="$2.html#$3">$1</a>}sg;
    $doc =~
s{L&lt;([^&]+?)\|Image::ExifTool::TagNames/(\w+) Tags&gt;}{<a href="$2.html">$1</a>}sg;
...
sub GetTableOrder(@) {
    ...
    foreach $pos ( sort { $b <=> $a } keys %fixPos ) { my $fix = $fixPos{$pos};
        foreach (@$fix) {
...
sub CloseHtmlFiles($) {
    ...
    foreach $htmlFile ( keys %createdFiles ) {
        ...
        if ($preserveDate) {
            ...
            $fileDate = $1
              if @lines and $lines[-1] =~ m{<i>Last revised (.*)</i>};
        }
...
"(This document generated automatically by Image::ExifTool::BuildTagLookup)\n";
        ...
        if ( $htmlFile =~ /index\.html$/ ) {
            print HTMLFILE
              "'../index.html'>&lt;-- Back to ExifTool home page</a></p>\n";
        }
        else {
        ...
        if ($success) {
            ...
            if ($useNewFile) {
                ...
                rename $tmpFile, $htmlFile
                  or warn("Error renaming temporary file\n"), $success = 0;
            }
            else {
                ++$countSameFiles;
                unlink $tmpFile;
...
sub WriteTagNames($$) {
    ...
    print HTMLFILE
      "<th colspan=$columns><span class=l>Tag Table Index</span></th></tr>\n";
    print HTMLFILE "<tr class=b><td width='$percent%'>\n";

    my @tableNames =
      GetTableOrder( 'Image::ExifTool::Shortcuts::Main', @pluginTables );
        ...
        while (@sepTables) {
            ...
            if ( OpenHtmlFile( $htmldir, $tableName, 1 ) ) {
                ...
                foreach ( sort { NumbersFirst( $a, $b ) } keys %$printConv ) {
                    next
                      if
                      /^(Notes|PrintHex|PrintInt|PrintString|OTHER|BITMASK)$/;
                    ...
                    warn
"Warning: Special characters in $tableName PrintConv ($$printConv{$_})\n";
                }
                ...
                if ($bits) {
                    ...
                    foreach ( sort { NumbersFirst( $a, $b ) } keys %$bits ) {
                        ...
                        warn
"Warning: Special characters in $tableName PrintConv BITMASK ($$bits{$_})\n";
                    }
                }
        ...
        if ( $short eq 'Shortcuts' ) {
            ...
            $notes = $$table{NOTES};
            if ( $$table{NAMESPACE} ) {
                my $ns = $Image::ExifTool::XMP::stdXlatNS{ $$table{NAMESPACE} }
                  || $$table{NAMESPACE};
                my $msg =
                  "These tags belong to the ExifTool XMP-$ns family 1 group.";
                if ($notes) {
                    $notes =~ s/\s+$//;
                    $notes .= "\n\n" . $msg;
                }
        ...
        if ($id) {
            ...
            if ( $wTag < $longTag ) {
                if ( $wID - $longTag + $wTag >= 6 ) { $wID -= $longTag - $wTag;
                    $wTag = $longTag;
                }
        ...
        my $tagNameHeading =
          ( $short eq 'XMP' )
          ? 'Namespace'
          : ( $isStruct ? 'Field' : 'Tag' ) . ' Name';
        $line .= sprintf " %-${wTag}s", $tagNameHeading;
        $line .= sprintf " %-${wReq}s",
          $composite == 2 ? 'Refers To' : 'Derived From'
          if $composite;
        ...
        print HTMLFILE
"<th>Writable</th>$derived<th>Values / ${noteFont}Notes</span></th></tr>\n";
        my $rowClass  = 1;
        my $infoCount = 0;
        my $infoList;
        ...
        foreach $infoList (@$info) {
            ...
            while ( @tags or @reqs or @vals ) {
                ...
                $line .= sprintf( " %-${wReq}s", shift(@reqs) || '' )
                  if $composite;
                $line .= sprintf( " %-${wGrp}s", shift(@wGrp) || '-' )
                  if $showGrp;
                ++$n;
                if (@vals) {
                    my $val   = shift @vals;
            ...
            if (
                (
                        $isExif
                    and $Image::ExifTool::Validate::exifSpec{ hex $tagIDstr }
                )
                or ( $isRiff    and $tagIDstr =~ /(\w+)/    and $riffSpec{$1} )
                or ( $isXmpMain and $tagIDstr =~ /([-\w]+)/ and $xmpSpec{$1} )
              )
            {
            ...
            if ( $tagIDstr =~ /^(0x[0-9a-f]+)(\.\d+)?$/i ) {
                $tip =
                  sprintf( " title='$tagIDstr = %u%s'", hex($1), $2 || '' );
            }
            elsif ( $tagIDstr =~ /^(\d+)(\.\d*)?$/ ) {
            ...
            print HTMLFILE "<td class=c$wtip>", join( '<br>', @$writable ),
              "</td>\n";
            print HTMLFILE '<td class=n>', join( "\n  <br>", @$require ),
              "</td>\n"
              if $composite;
            print HTMLFILE "<td class=c>", join( '<br>', @$writeGroup ),
              "</td>\n"
              if $showGrp;
            print HTMLFILE "<td>";
                ...
                if ($isSubdir) {
                    ...
                    foreach (@$values) {
                        if (/^\(/) {
                            ...
                            push @values,
                              ( $smallNote ? $noteFontSmall : $noteFont )
                              . "$_</span>";
                            next;
                        }
                        if (/^\[/) {
                    ...
                    foreach (@$values) {
                        if (s/^\[!HTML\]//) {
                            ...
                            $close
                              or $_ = "<span class=s>$_", $close = '</span>';
                            print HTMLFILE $br, $_;
                            $br = "\n  <br>";
                        }
                    }
        ...
        unless ($infoCount) {
            ...
            ++$cols if $derived;
            print HTMLFILE
"<tr><td colspan=$cols class=c><i>[no tags known]</i></td></tr>\n";
        }
        print HTMLFILE "</table></td></tr></table></blockquote>\n\n";
    }
    close(HTMLFILE)                            or $success = 0;
