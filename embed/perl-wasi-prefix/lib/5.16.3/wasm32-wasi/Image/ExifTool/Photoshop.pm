...
%printFlags = (
    0 => 'Labels',
    1 => 'Corner crop marks',
    2 => 'Color bars', 3 => 'Registration marks',
    4 => 'Negative',
    5 => 'Emulsion down',
    6 => 'Interpolate', 7 => 'Description',
    8 => 'Print flags',
);
...
%Image::ExifTool::Photoshop::Main = (
    ...
    0x03ea => { Unknown => 1, Name => 'XMLData', Binary => 1 }, 0x03eb =>
      { Unknown => 1, Name => 'Photoshop2ColorTable' },
    0x03ed => {
        Name => 'ResolutionInfo',
        SubDirectory =>
    ...
    0x0422 => {
        Name => 'EXIFInfo', SubDirectory => {
            TagTable    => 'Image::ExifTool::Exif::Main',
            ProcessProc => \&Image::ExifTool::ProcessTIFF,
            WriteProc   => \&Image::ExifTool::WriteTIFF,
        },
    },
    0x0423 => { Unknown => 1, Name => 'ExifInfo2', Binary => 1 }, 0x0424 => {
        Name         => 'XMP',
    ...
    0x0429 => { Unknown => 1, Name => 'LayerComps' }, 0x042a =>
      { Unknown => 1, Name => 'AlternateDuotoneColors' }, 0x042b =>
      { Unknown => 1, Name => 'AlternateSpotColors' },    0x042d =>
    ...
    0x042e => { Unknown => 1, Name => 'HDRToningInfo' }, 0x042f =>
      { Unknown => 1, Name => 'PrintInfo' }, 0x0430 =>
      { Unknown => 1, Name => 'LayerGroupsEnabledID', Format => 'int8u' }
    , 0x0431 => { Unknown => 1, Name => 'ColorSamplersResource2' }, 0x0432 =>
      { Unknown => 1, Name => 'MeasurementScale' }, 0x0433 =>
      { Unknown => 1, Name => 'TimelineInfo' },     0x0434 =>
      { Unknown => 1, Name => 'SheetDisclosure' },  0x0435 => {
        Name => 'ChannelOptions', SubDirectory => {
            TagTable => 'Image::ExifTool::Photoshop::ChannelOptions',
            Start    => 4,
        },
      },
    0x0436 => { Unknown => 1, Name => 'OnionSkins' }, 0x0438 =>
      { Unknown => 1, Name => 'CountInfo' },            0x043a =>
      { Unknown => 1, Name => 'PrintInfo2' },           0x043b =>
      { Unknown => 1, Name => 'PrintStyle' },           0x043c =>
      { Unknown => 1, Name => 'MacintoshNSPrintInfo' }, 0x043d =>
      { Unknown => 1, Name => 'WindowsDEVMODE' },       0x043e =>
      { Unknown => 1, Name => 'AutoSaveFilePath' },     0x043f =>
      { Unknown => 1, Name => 'AutoSaveFormat' },       0x0440 =>
      { Unknown => 1, Name => 'PathSelectionState' },    0x0bb7 => {
    ...
    0x0bb8 => { Unknown => 1, Name => 'OriginPathInfo' },  0x1b58 =>
      { Unknown => 1, Name => 'ImageReadyVariables' }, 0x1b59 =>
      { Unknown => 1, Name => 'ImageReadyDataSets' },  0x1f40 =>
      { Unknown => 1, Name => 'LightroomWorkflow' },   0x2710 =>
      { Unknown => 1, Name => 'PrintFlagsInfo' },
);
...
sub ProcessLayersAndMask($$$) {
    ...
    if ( $len == 0 and $num == 0 ) {
        ...
        elsif ( $data =~ /^..8BIMMt16/s ) { $raf->Read( $data, $psiz ) == $psiz
              or return 0;
...
sub ProcessPSD($$) {
    ...
    if ($outfile) {
        ...
        my $oldIndent = $$et{INDENT};
        $$et{INDENT} .= '| ';
        if ( ProcessLayersAndMask( $et, \%dirInfo, $tagTablePtr )
            and $raf->Read( $data, 2 ) == 2 )
        {
            my %dirInfo = (
                DataPt  => \$data,
                DataPos => $raf->Tell() - 2,
