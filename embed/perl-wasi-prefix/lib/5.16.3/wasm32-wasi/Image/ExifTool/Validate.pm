...
my %stdFormat = (
    ...
    IFD => {
        ...
        0xc792 => 'int(16|32)u',
        0xc68e => 'int(16|32)u',
        0xc793 => '(int16u|int32u|rational64u)',
        0xc6d2 => '',
        0xcd43 => 'int(16|32)u',
        0xc6d3 => '',
        0xcd48 => '(string|int8u)',
...
my %validValue = (
    JPEG => {
        IFD0 => {
            0x100 => undef, 0x101 => undef, 0x102 => undef, 0x103 =>
              undef, 0x106 => undef, 0x111 => undef, 0x115 => undef, 0x116 =>
              undef, 0x117 => undef,   0x11c => undef,  0x201 =>
              undef, 0x202 => undef, 0x212   => undef, 0x213 =>
              '$val =~ /^[12]$/', },
        IFD1 => {
            0x100 => undef, 0x101 => undef, 0x102 => undef, 0x103 =>
              '$val == 6',    0x106 => undef,          0x111 => undef, 0x115 =>
              undef,          0x116 => undef,          0x117 => undef, 0x11a =>
              'defined $val', 0x11b => 'defined $val', 0x11c => undef, 0x128 =>
              '$val =~ /^[123]$/', 0x201 => 'defined $val', 0x202 =>
              'defined $val', 0x212 => undef, },
        ExifIFD => {
            0x9000 => 'defined $val and $val =~ /^\d{4}$/', 0x9101 =>
              'defined $val',                 0xa001 =>
              '$val == 1 or $val == 0xffff', 0xa002 =>
              'defined $val', 0xa003 => 'defined $val', },
        GPS => {
            0x00 => 'defined $val and $val =~ /^\d \d \d \d$/', 0x1b =>
              'not defined $val or $val =~ /^(GPS|CELLID|WLAN|MANUAL)$/', },
        InteropIFD => {}, },
    TIFF => {
        IFD0 => {
            0x100 => 'defined $val', 0x101 => 'defined $val',  0x103 => q{
                not defined $val or $val =~ /^(1|5|6|32773)$/ or
                    ($val == 2 and (not defined $val{0x102} or $val{0x102} == 1));
            }, 0x106 => '$val =~ /^[0123]$/', 0x111 =>
              'defined $val',  0x115 => q{
                my $pi = $val{0x106} || 0;
            ...
            0x116 => 'defined $val', 0x117 => 'defined $val', 0x11a =>
              'defined $val', 0x11b => 'defined $val', 0x128 =>
              'not defined $val or $val =~ /^[123]$/',  0x140 => q{
            ...
            0x201 => undef, 0x202 => undef, },
        ExifIFD => {
            0x9000 => 'defined $val', 0x9101 => undef, 0x9102 =>
              undef,                         0xa000 => 'defined $val', 0xa001 =>
              '$val == 1 or $val == 0xffff', 0xa002 => undef,          0xa003 =>
              undef, },
        InteropIFD => { 0x0001 => undef, },
        GPS        => {
            0x00 => 'defined $val and $val =~ /^\d \d \d \d$/', 0x1b =>
              '$val =~ /^(GPS|CELLID|WLAN|MANUAL)$/', },
    },
);
...
my %validateInfo = (
    ...
    PrintConv => {
        ...
        OTHER   => sub {
            my @val = split ' ', shift;
            my @rtn;
            push @rtn, sprintf( '%d Error%s', $val[0], $val[0] == 1 ? '' : 's' )
              if $val[0];
            push @rtn,
              sprintf( '%d Warning%s', $val[1], $val[1] == 1 ? '' : 's' )
              if $val[1];
            if ( $val[2] ) {
                my $str =
                  ( $val[1] == $val[2]
...
sub ValidateRaw($$$) {
    ...
    if ( $$tagInfo{Validate} ) {
        ...
        if ( $wrn or $err ) {
            my $name =
              $$tagInfo{Table}{GROUPS}{0} . ':'
              . Image::ExifTool::GetTagName($tag);
            $self->Warn( "Validate $name: $err", 1 ) if $err;
            $self->Warn( "$wrn for $name",       1 ) if $wrn;
        }
    }
    if (
        ref $$tagInfo{PrintConv} eq 'HASH'
        and (
            $$tagInfo{Table}{SHORT_NAME} eq 'GPS::Main'
            or (    $$tagInfo{Table} eq \%Image::ExifTool::Exif::Main
                and $exifSpec{ $$tagInfo{TagID} } )
        )
      )
    {
...
sub ValidateExifDate($) {
    ...
    if ( $val =~ /^\d{4}:(\d{2}):(\d{2}) (\d{2}):(\d{2}):(\d{2})$/ ) {
        ...
        for ( $i = 0 ; $i < @a ; ++$i ) {
            next
              if $a[$i] eq '  '
              or (  $a[$i] >= $validDateField[$i][1]
                and $a[$i] <= $validDateField[$i][2] );
...
sub ValidateExif($$$$$$$$) {
    my ( $et, $tagTablePtr, $tag, $tagInfo, $lastTag, $ifd, $count, $formatStr )
      = @_;

    $et->Warn("Entries in $ifd are out of order") if $tag <= $lastTag;

    if (    not defined $tagInfo
        and $$tagTablePtr{$tag}
    ...
    if ( defined $tagInfo ) {
        ...
        if (
            defined $$stdFmt{All}
            or (
                $tagTablePtr eq \%Image::ExifTool::Exif::Main
                and (
                       $exifSpec{$tag}
                    or $$stdFmt{$tag}
                    or (    $tag >= 0xc612
                        and $tag <= 0xcd48
                        and not defined $$stdFmt{$tag} )
                )
            )
            or $$tagTablePtr{SHORT_NAME} eq 'GPS::Main'
          )
        {
            ...
            if (
                ...
                and $wgp ne $ifd
                and $wgp ne 'All'
                and not $$ti{OffsetPair}
                and ( $ifd =~ /^(Sub|Profile)?IFD\d*$/ xor $wgp =~
                    /^(Sub)?IFD\d*$/ )
                and ( $$ti{Writable} or $$ti{WriteGroup} )
                and $ifd !~ /^SRF\d+$/
              )
            {
                $et->Warn(
                    sprintf(
                        'Wrong IFD for 0x%.4x %s (should be %s not %s)',
                        $tag, $$ti{Name}, $wgp, $ifd
                    )
                );
            }
            ...
            if (
                ...
                and $formatStr !~ /^$fmt$/
                and (  not $tagInfo
                    or not $$tagInfo{IsOffset}
                    or $Image::ExifTool::Exif::intFormat{$formatStr} )
              )
            {
                $et->Warn(
                    sprintf(
                        'Non-standard format (%s) for %s 0x%.4x %s',
                        $formatStr, $ifd, $tag, $$ti{Name}
                    )
                );
            }
        }
            ...
            not $otherSpec{ $$et{FileType} }
            or (    not $otherSpec{ $$et{FileType} }{$tag}
                and not $otherSpec{ $$et{FileType} }{All} )
          )
        {
            if (   $tagTablePtr eq \%Image::ExifTool::Exif::Main
                or $$ti{Unknown} )
            {
                $et->Warn(
                    sprintf(
                        'Non-standard %s tag 0x%.4x %s',
                        $ifd, $tag, $$ti{Name}
                    ),
                    1
                ) unless $otherSpec{$ifd};
            }
        }
        my $tiCount = $$ti{Count};
        if ($tiCount) {
            if (    $$ti{Format}
                and $$ti{Writable}
                and $Image::ExifTool::Exif::formatNumber{ $$ti{Format} }
                and $Image::ExifTool::Exif::formatNumber{ $$ti{Writable} } )
            {
                my $s1 = $Image::ExifTool::Exif::formatSize
                  [ $Image::ExifTool::Exif::formatNumber{ $$ti{Format} } ];
                my $s2 = $Image::ExifTool::Exif::formatSize
            ...
            if ( $tiCount > 0 and $count != $tiCount ) {
                $et->Warn(
                    sprintf(
                        'Non-standard count (%d) for %s 0x%.4x %s',
                        $count, $ifd, $tag, $$ti{Name}
                    )
                );
            }
        }
...
sub ValidateOffsetInfo($$$;$) {
    ...
    return
          if $$et{TIFF_TYPE} eq 'MRW'
      and $dirName eq 'IFD0'
      and $$et{Model} =~ /^DiMAGE A200/;
    ...
    while (%$offsetInfo) {
        ...
        unless ( defined $id2 and $$offsetInfo{$id2} ) {
            unless ( $$offsets[0]{NotRealPair}
                or ( defined $id2 and $id2 == -1 ) )
            {
                my $corr = $$offsets[0]{IsOffset} ? 'size' : 'offset';
                $et->Warn(
"$dirName:$$offsets[0]{Name} is missing the corresponding $corr tag"
                ) unless $minor;
            }
            next;
        }
        ...
        if ( @sizes != @offsets ) {
            $et->Warn(
                sprintf(
                    'Wrong number of values in %s 0x%.4x %s',
                    $dirName,
                    $$offsets[0]{TagID},
                    $$offsets[0]{Name}
                ),
        ...
        while (@offsets) {
            ...
            $et->Warn( "$dirName:$$offsets[0]{Name} is zero", $minor )
              if $start == 0;
            $et->Warn( "$dirName:$$sizes[0]{Name} is zero", $minor )
              if $start == $end;
            next unless $end > $fileSize;
            if ( $start >= $fileSize ) {
                if ( $start == 0xffffffff ) {
                    $et->Warn(
                        "$dirName:$$offsets[0]{Name} is invalid (0xffffffff)",
                        $minor );
                }
                else {
                    $et->Warn(
                        "$dirName:$$offsets[0]{Name} is past end of file",
                        $minor );
                }
            }
            else {
                $et->Warn(
"$dirName:$$offsets[0]{Name}+$$sizes[0]{Name} runs past end of file",
                    $minor
                );
            }
            last;
    ...
    if ( $validValue{$fileType} ) {
        ...
        foreach $grp ( sort keys %{ $validValue{$fileType} } ) {
            ...
            if ($verCheck) {
                ...
                ( $ver = $$et{VALUE}{$verTag} ) =~ tr/0-9//dc;
                undef $ver unless $ver =~ /^\d{4}$/;
            }
            foreach $key ( sort keys %{ $$et{VALUE} } ) {
                ...
                $et->Warn(
                    sprintf(
                        '%s tag 0x%.4x %s requires %s %s or higher',
                        $grp, $tag, $$et{TAG_INFO}{$key}{Name},
                        $verTag, $vstr
                    )
                );
            }
        ...
        if ( $extFileType and $extFileType ne $fileType ) {
            ...
            if ( $normExt and $normExt ne $fileExt ) {
                ...
                if ( ref $lkup or $lkup ne $normExt ) {
                    $et->Warn(
"File has wrong extension (should be $normExt, not $fileExt)"
                    );
                }
            }
        }
    }
    $et->Warn('Validation incomplete because FastScan option used')
