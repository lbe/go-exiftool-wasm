...
my %qtFmt = (
    ...
    66 => 'int16s',
    67 => 'int32s',
    70 => 'float', 71 => 'float', 72 => 'float', 74 => 'int64s',
    75 => 'int8u',
    76 => 'int16u',
    77 => 'int32u',
    78 => 'int64u',
...
my %processByMetaFormat = (
    meta => 1, data => 1, sbtl => 1, ctbx => 1, );

my %insvDataLen = (
    0x000 => 0, 0x200 => 0, 0x300 => 0, 0x400 => 16, 0x600 => 8, 0x700 =>
      53,       );
...
%Image::ExifTool::QuickTime::Stream = (
    ...
    gpmd => [
        {
            Name => 'gpmd_Kingslim', Condition =>
              '$$valPt =~ /^.{21}\0\0\0A[NS][EW]/s',
            SubDirectory => {
        ...
        {
            Name => 'gpmd_Rove', Condition =>
              '$$valPt =~ /^\0\0\xf2\xe1\xf0\xeeTT/',
            SubDirectory => {
                TagTable    => 'Image::ExifTool::QuickTime::Stream',
        ...
        {
            Name => 'gpmd_FMAS', Condition => '$$valPt =~ /^FMAS\0\0\0\0/',
            SubDirectory => {
                TagTable    => 'Image::ExifTool::QuickTime::Stream',
    ...
    RVMI => [
        { Name => 'RVMI_gReV',
            Condition => '$$valPt =~ /^gReV/', SubDirectory => {
        ...
        {
            Name      => 'RVMI_sReV',
            Condition => '$$valPt =~ /^sReV/', SubDirectory => {
...
'length $$valPt == 32 and $$valPt !~ /^\0\0\xe0\xff\xff\xff\xef\x41/',
        ...
        {
            Name      => 'Accelerometer', Condition => 'length $$valPt == 12',
            Format    => 'float',
            ByteOrder => 'Little-Endian',
        },
        {
            Name => 'PreviewImage', Condition => '$$valPt =~ /^\xff\xd8\xff/',
            Groups  => { 2 => 'Preview' },
            RawConv => '$self->ValidateImage(\$val,$tag)',
        }
    ],
    djmd => { Name => 'DJIMetadata',
...
%Image::ExifTool::QuickTime::RVMI_sReV = (
    ...
    4 => {
        Name   => 'GSensor',
        Format => 'int16s[3]', ValueConv =>
          'my @a=split " ",$val; $_/=1000 foreach @a; "@a"',
    },
);
...
%Image::ExifTool::QuickTime::tx3g = (
    ...
    Alt => {
        Name      => 'GPSAltitude',
        ValueConv => '$val =~ s/\s*m$//; $val', PrintConv => '"$val m"', },
    Yaw      => 'Yaw',
    Pitch    => 'Pitch',
    Roll     => 'Roll',
    GimYaw   => 'GimbalYaw',
...
/:(\d{4})-(\d{2})-(\d{2}) (\d{2}:\d{2}:\d{2})-([NS])(\d+\.\d+)-([EW])(\d+\.\d+)-S(\d+)/
        ...
        and length $$dataPt >= 282 )
    {
        ...
        if ( $val =~ /^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})$/ ) {
            ...
            if ( $$dataPt =~ /^\0\0..\xaa\xaa/s ) { $val = pack( 'C*',
                    map { $_ ^ 0xaa }
...
/^.{8}(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2}).(.{15})([NS])(\d{8})([EW])(\d{9})(\d{8})?/s
          )
        {
            (
              ...
              = ( $1, $2, $3, $4, $5, $6, $7, $8, $9 / 1e4, $10, $11 / 1e4,
                $12 );
            if ( defined $spd ) { $spd += 0;
            }
            elsif ( $buf2 =~ /^.{57}([-+]\d{4})(\d{3})/s ) {  $spd = $2 + 0;
            }
        }
...
/\$[A-Z]{2}GGA,(\d{2})(\d{2})(\d+(\.\d*)?),(\d+\.\d+),([NS]),(\d+\.\d+),([EW]),[1-6]?,(\d+)?,(\.\d+|\d+\.?\d*)?,(-?\d+\.?\d*)?,M?/s
        ...
        if ( length($$dataPt) < 0x78 ) {
            ...
            $lt = substr( $$dataPt, 0x2c, 20 ), $ln =
              substr( $$dataPt, 0x40, 20 ), /^[A-Za-z0-9+\/]{8,20}={0,2}\0*$/
              or $notEnc = 1, last foreach ( $lt, $ln );
...
'GPSLatitude/Longitude encryption is not yet known, so these will be wrong'
        ...
        and $dirLen >= 0x88 )
    {
        ...
        if ( substr( $$dataPt, 16, 3 ) eq 'IQS' ) {
            ...
            $trk = GetFloat( $dataPt, 0x58 );
                    if ( $$et{KodakVersion}
                and $$et{KodakVersion} eq '3.01.054' )
            {
...
my %ttLen = ( 0 => 12, 1 => 4, 2 => 12, 3 => 12,  5 => 92, 0xff => 4, );
...
sub ProcessTTAD($$$) {
    ...
    while ( $pos < $dirLen ) {
        ...
        if ( $type == 0xff ) { my $tm = Get32u( $dataPt, $pos );
            if ($resync) {
...
"Insta360 $insvLimit{$id}[0] data is huge. Processing only the first $insvLimit{$id}[1] records",
        ...
        if ($dlen) {
            ...
            elsif ( $id == 0x600 ) { for ( $p = 0 ; $p < $len ; $p += $dlen ) {
                    $$et{DOC_NUM} = ++$$et{DOC_COUNT};
                    $et->HandleTag( $tagTbl,
                        VideoTimeStamp =>
                          sprintf( '%.3f', Get64u( \$buff, $p ) / 1000 ) );
...
sub ScanMediaData($) {
    ...
    while ($dataLen) {
        ...
        if ( $buff !~ /(\0..\0freeGPS |GP\x06\0\0)/sg ) {
        ...
        elsif ( $1 eq "GP\x06\0\0" ) {  my $buffPos = pos($buff);
            my $filePos = $raf->Tell();
            ...
            my $size =
              Image::ExifTool::GoPro::ProcessGP6( $et,
