...
my %oleFormat = (
    0 => undef, 1 => undef, 2 => 'int16s', 3 => 'int32s', 4 => 'float', 5 =>
      'double', 6  => undef,      7  => 'VT_DATE',    8  => 'VT_BSTR',     10 =>
      'int32s', 11 => 'int16s',   12 => 'VT_VARIANT', 16 => 'int8s',       17 =>
      'int8u',  18 => 'int16u',   19 => 'int32u',     20 => 'int64s',      21 =>
      'int64u', 30 => 'VT_LPSTR', 31 => 'VT_LPWSTR',  64 => 'VT_FILETIME', 65 =>
      'VT_BLOB', 71 => 'VT_CF', 72 => 'VT_CLSID', );

my %oleFlags = (
    0x1000 => 'VT_VECTOR',
    0x2000 => 'VT_ARRAY', 0x4000 => 'VT_BYREF', 0x8000 => 'VT_RESERVED',
);

my %oleFormatSize = (
    VT_DATE => 8,
    VT_BSTR => 4, VT_VARIANT => 4, VT_LPSTR => 4, VT_LPWSTR =>
      4, VT_FILETIME => 8,
    VT_BLOB   => 4, VT_CF => 4, VT_CLSID => 16,
    VT_VECTOR => 4, );
...
%Image::ExifTool::FlashPix::Main = (
    ...
    "\x05Image Info" => {
        Name         => 'ImageInfo',
        SubDirectory => { TagTable => 'Image::ExifTool::FlashPix::ImageInfo', },
    },
    "\x05Image Contents" => {
        Name         => 'Image',
        SubDirectory => { TagTable => 'Image::ExifTool::FlashPix::Image', },
    },
    "Contents" => {
        Name         => 'Contents',
    ...
    "\x05Data Object" => { Name => 'DataObject',
        SubDirectory =>
          { TagTable => 'Image::ExifTool::FlashPix::DataObject', }, },
    "\x05Transform" => { Name => 'Transform',
        SubDirectory => { TagTable => 'Image::ExifTool::FlashPix::Transform', },
    },
    "\x05Operation" => { Name => 'Operation',
        SubDirectory => { TagTable => 'Image::ExifTool::FlashPix::Operation', },
    },
    "\x05Global Info" => {
        Name => 'GlobalInfo',
        SubDirectory =>
    ...
    "\x05Screen Nail" => { Name => 'ScreenNail',
        Groups => { 2 => 'Other' },
        ValueConv => 'length($val) > 0x1c and $val = substr($val, 0x1c); \$val',
    },
    "\x05Audio Info" => {
        Name         => 'AudioInfo',
        SubDirectory => { TagTable => 'Image::ExifTool::FlashPix::AudioInfo', },
    },
    'Audio Stream' => { Name => 'AudioStream',
        Groups => { 2 => 'Audio' },
        ValueConv => 'length($val) > 0x1c and $val = substr($val, 0x1c); \$val',
    },
    ...
    IeImg_rect => { Name => 'EmbeddedImageRectangle',
        Notes => q{
            not a real tag.  This information is extracted if available for the
            corresponding EmbeddedImage from the Contents of a VNT file
        }, },
...
%Image::ExifTool::FlashPix::SummaryInfo = (
    ...
    0x22 => { Name => 'CreatedBy', Groups => { 2 => 'Author' } }, 0x23 =>
      'DocumentID',  0x80000000 =>
      { Name => 'LocaleIndicator', Groups => { 2 => 'Other' } },
);
...
%Image::ExifTool::FlashPix::DocumentInfo = (
    ...
    0x17 => { Name => 'AppVersion',
        ValueConv => 'sprintf("%d.%.4d",$val >> 16, $val & 0xffff)', },
    0x1a => 'ContentType', 0x1b => 'ContentStatus', 0x1c => 'Language', 0x1d =>
      'DocVersion',      '_PID_LINKBASE' => {
        Name      => 'HyperlinkBase',
...
%Image::ExifTool::FlashPix::Image = (
    ...
    0x01000002 => 'ImageWidth', 0x01000003 => 'ImageHeight',
...
%Image::ExifTool::FlashPix::WordDocument = (
    ...
    5 => {
        Name      => 'DocFlags',
        Mask      => 0xff0f, RawConv => '$$self{DocFlags} = $val',
        PrintConv => {
            BITMASK => {
                0  => 'Template',
                1  => 'AutoText only',
...
'enable L<Duplicates|../ExifTool.html#Duplicates> option to extract history of up to 10 entries',
    ...
    TableOffsets => { Hidden => 2 }, CommentByBlock => { SubDirectory => {
            TagTable    => 'Image::ExifTool::FlashPix::DocTable',
            ProcessProc => \&ProcessCommentBy,
        },
        Hidden => 2, },
    LastSavedByBlock => { SubDirectory => {
            TagTable    => 'Image::ExifTool::FlashPix::DocTable',
    ...
    unless ( $$et{DOC_NUM} ) {
        ...
        foreach $tag (@pri) {
            for ( $copy = 1 ; ; ++$copy ) {
                ...
                foreach $member ( 'PRIORITY', 'VALUE', 'FILE_ORDER',
                    'TAG_INFO', 'TAG_EXTRA' )
                {
                    my $pHash = $$et{$member};
                    my $t     = $$pHash{$tag};
