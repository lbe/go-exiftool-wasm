...
sub super_languages {
    ...
    $lang1 =~ s/^nb\b/no-bok/i;
    $lang1 =~ s/^nn\b/no-nyn/i;
    $lang1 =~ s/^[ix](-hakka\b)/zh$1/i;
...
sub encode_language_tag {
    ...
    $tag           =~ s/^jw\b/jv/i;
    $tag           =~ s/^[ix]-lux\b/lb/i;
    $tag           =~ s/^[ix]-navajo\b/nv/i;
    $tag           =~ s/^ji\b/yi/i;
              $tag =~ s/^[ix]-hakka\b/zh-hakka/i;
    $tag           =~ s/^nb\b/no-bok/i;
    $tag           =~ s/^nn\b/no-nyn/i;
...

{
    ...
    my @panic = (      
        ...
        'sv' => [qw(nb no da nn)],
        'da' => [qw(nb no sv nn)], [qw(no nn nb)], [qw(no nn nb sv da)],
        'is' => [qw(da sv no nb nn)],
        'fo' => [qw(da is no nb nn sv)],

        'pt' => [qw(es ca it fr)], 'ca' => [qw(es pt it fr)],
        'es' => [qw(ca it fr pt)],
        'it' => [qw(es fr ca pt)],
        'fr' => [qw(es it ca pt)],
