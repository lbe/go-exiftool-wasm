...
First of all, I'd like to create this directory.
    ...
    my @dirs = File::Spec->catdir(
        $where,
        $self->_perl_version( perl => $^X ),
        $conf->_get_build('moddir')
      ),
      map { File::Spec->catdir( $where, $conf->_get_build($_) ) }
      qw[autdir distdir];
...
is required for the 'from' field, so choose wisely.
    ...
    if ( $email eq $other ) {
      EMAIL: {
            $email = $term->get_reply( prompt => loc('Email address: '), );
...
", 'c:\Progra~1\prog.exe' );
    ...
    for my $prog ( sort $conf->options( type => 'program' ) ) {
      PROGRAM: {
            print "\n",
              loc(
                "Where can I find your '%1' utility? "
                  . "(Enter a single space to disable)",
                $prog
              ),
              "\n";
            ...
            print $cmd
              ? loc( "Your '%1' utility has been set to '%2'.", $prog, $cmd )
...
Otherwise, select ASK to have us ask your permission to install them.
        ...
        my @map = (
            [
                PREREQ_IGNORE, loc('No, do not install prerequisites')
                , loc("I won't install prerequisites") ],
            [
                PREREQ_INSTALL,
        ...
        my %reply = map { $_->[1] => $_->[0] } @map;
        my %diag  = map { $_->[1] => $_->[2] } @map;
        my %conf  = map { $_->[0] => $_->[1] } @map;
        ...
        if ( $reply eq $other ) {
          SHELL: {
                $reply = $term->get_reply(
                    prompt => loc(
                            'Please enter the name of the shell '
                          . 'you wish to use: '
                    ),
                );
...
" );
  ...
  MIRROR: {
        ...
        my ( $continent, $country, $host ) =
          $self->_guess_from_timezone($hosts);

      CONTINENT: {
            my %seen;
            my @choices =
              sort map { $_->{'continent'} }
              grep     { not $seen{ $_->{'continent'} }++ } values %$hosts;
      ...
      COUNTRY: {
            ...
            my @choices =
              sort map { $_->{'country'} }
              grep     { not $seen{ $_->{'country'} }++ }
              grep { ( $_->{'continent'} eq $continent ) } values %$hosts;
            push @choices, qw[Custom Up Quit];
...
" );
      ...
      CHOICE: {
            ...
            if ($href) {
                push @hosts, $href
                  unless grep {
                          $href->{'scheme'} eq $_->{'scheme'}
                      and $href->{'host'}   eq $_->{'host'}
                      and $href->{'path'}   eq $_->{'path'}
                  } @hosts;
...
" );
    ...
    my $mb = CPANPLUS::Module::Fake->new(
        ...
        author  => CPANPLUS::Module::Author::Fake->new( _id => $cpan->_id ),
        _id     => $cpan->_id,
    );

    my $file = $cpan->_fetch(
        fetchdir => $conf->get_conf('base'),
        module   => $mb
    );
...
sub _parse_mirrored_by {
    ...
    $fh->open("$file")
      or ( warn( loc( 'Could not open file "%1": %2', $file, $! ) ), return );
...

{
    ...
    sub _valid_email {
        ...
        if ( !$RFC822PAT ) {
            ...
            my $Cnested =
              qq< $OpenParen $ctext* (?: $quoted_pair $ctext* )* $CloseParen >;
            my $comment =
qq< $OpenParen $ctext* (?: (?: $quoted_pair | $Cnested ) $ctext* )* $CloseParen >;
            ...
            my $domain_ref = $atom;
            my $domain_lit =
              qq< $OpenBR (?: $dtext | $quoted_pair )* $CloseBR >;
