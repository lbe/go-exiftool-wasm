...
use constant INSTALLER_BUILD      => 'CPANPLUS::Dist::Build';
use constant INSTALLER_MM         => 'CPANPLUS::Dist::MM';
use constant INSTALLER_SAMPLE     => 'CPANPLUS::Dist::Sample';
use constant INSTALLER_BASE       => 'CPANPLUS::Dist::Base';
use constant INSTALLER_AUTOBUNDLE => 'CPANPLUS::Dist::Autobundle';
...
use constant DEFAULT_SOURCE_ENGINE => 'CPANPLUS::Internals::Source::Memory';
...
use constant UNKNOWN_DL_LOCATION => 'UNKNOWN-ORIGIN';

use constant NMAKE => 'nmake.exe';
use constant NMAKE_URL =>
  'ftp://ftp.microsoft.com/Softlib/MSLFILES/nmake15.exe';

use constant INSTALL_VIA_PACKAGE_MANAGER => sub {
    my $fmt = $_[0] or return;
    return 1
      if $fmt  ne INSTALLER_BUILD
      and $fmt ne INSTALLER_MM;
};

use constant IS_CODEREF => sub { ref $_[-1] eq 'CODE' };
use constant IS_MODOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Module' );
};
use constant IS_FAKE_MODOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Module::Fake' );
};
use constant IS_AUTHOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Module::Author' );
};
use constant IS_FAKE_AUTHOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Module::Author::Fake' );
};

use constant IS_CONFOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Configure' );
};

use constant IS_RVOBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Backend::RV' );
};

use constant IS_INTERNALS_OBJ => sub {
    UNIVERSAL::isa( $_[-1], 'CPANPLUS::Internals' );
};
...
use constant FILE_EXISTS => sub {
    ...
    error( loc( q[File '%1' does not exist], $file ) );
    return;
};
...
use constant FILE_READABLE => sub {
    ...
    local $Carp::CarpLevel = $Carp::CarpLevel + 2;
    error( loc( q[File '%1' is not readable ] . q[or does not exist], $file ) );
    return;
};
...
use constant DIR_EXISTS => sub {
    ...
    local $Carp::CarpLevel = $Carp::CarpLevel + 2;
    error( loc( q[Dir '%1' does not exist], $dir ) );
    return;
};

use constant MAKEFILE => sub {
    my $file =
...
use constant MAKEFILE_PL => sub {
    return @_
      ? File::Spec->catfile( @_, 'Makefile.PL' )
      : 'Makefile.PL';
};
use constant BUILD_PL => sub {
    return @_
      ? File::Spec->catfile( @_, 'Build.PL' )
      : 'Build.PL';
};

use constant META_YML => sub {
    return @_
      ? File::Spec->catfile( @_, 'META.yml' )
      : 'META.yml';
};

use constant MYMETA_YML => sub {
    return @_
      ? File::Spec->catfile( @_, 'MYMETA.yml' )
      : 'MYMETA.yml';
};

use constant META_JSON => sub {
    return @_
      ? File::Spec->catfile( @_, 'META.json' )
      : 'META.json';
};

use constant MYMETA_JSON => sub {
    return @_
      ? File::Spec->catfile( @_, 'MYMETA.json' )
      : 'MYMETA.json';
};

use constant BLIB => sub {
    return @_
...
use constant LIB_DIR => sub {
    return @_
      ? File::Spec->catdir( @_, LIB )
      : LIB;
};
use constant AUTO         => 'auto';
use constant LIB_AUTO_DIR => sub {
    return @_
      ? File::Spec->catdir( @_, LIB, AUTO )
      : File::Spec->catdir( LIB, AUTO );
};
use constant ARCH     => 'arch';
use constant ARCH_DIR => sub {
    return @_
      ? File::Spec->catdir( @_, ARCH )
      : ARCH;
};
use constant ARCH_AUTO_DIR => sub {
    return @_
      ? File::Spec->catdir( @_, ARCH, AUTO )
      : File::Spec->catdir( ARCH, AUTO );
};

use constant BLIB_LIBDIR => sub {
    return @_
      ? File::Spec->catdir( @_, BLIB->(), LIB )
      : File::Spec->catdir( BLIB->(), LIB );
};

use constant CONFIG_USER_LIB_DIR => sub {
    require CPANPLUS::Internals::Utils;
    LIB_DIR->( CPANPLUS::Internals::Utils->_home_dir, DOT_CPANPLUS );
};
use constant CONFIG_USER_FILE => sub {
    File::Spec->catfile( CONFIG_USER_LIB_DIR->(), split( '::', CONFIG_USER ), )
      . '.pm';
};
use constant CONFIG_SYSTEM_FILE => sub {
    require CPANPLUS::Internals;
    require File::Basename;
    my $dir = File::Basename::dirname( $INC{'CPANPLUS/Internals.pm'} );

    File::Spec->catfile( $dir, qw[Config System.pm] );
};

use constant README => sub {
    my $obj = $_[0];
    my $pkg = $obj->package_name;
    $pkg .= '-' . $obj->package_version . '.readme';
    return $pkg;
};
use constant META_EXT => 'meta';

use constant META => sub {
    my $obj = $_[0];
    my $pkg = $obj->package_name;
    $pkg .= '-' . $obj->package_version . '.' . META_EXT;
    return $pkg;
};
...
use constant OPEN_DIR => sub {
    ...
    my $dh;
    opendir $dh, $dir
      or error( loc( "Could not open dir '%1': %2", $dir, $! ) );
...
use constant STRIP_GZ_SUFFIX => sub {
...
use constant ENV_CPANPLUS_CONFIG       => 'PERL5_CPANPLUS_CONFIG';
use constant ENV_CPANPLUS_IS_EXECUTING => 'PERL5_CPANPLUS_IS_EXECUTING';
use constant DEFAULT_EMAIL             => 'cpanplus@example.com';
use constant CPANPLUS_UA               => sub { require CPANPLUS::Internals;
...
use constant TESTERS_DETAILS_URL => sub {
    'http://cpantesters.org/distro/'
      . uc( substr( $_[0], 0, 1 ) ) . '/'
      . $_[0];
};

use constant CREATE_FILE_URI => sub {
...
use constant CUSTOM_AUTHOR_ID => 'LOCAL';

use constant DOT_SHELL_DEFAULT_RC => '.shell-default.rc';

use constant SOURCE_SQLITE_DB => 'db.sql';
...
use constant INSTALL_LOG_FILE => sub {
...
use constant QUOTE_PERL_ONE_LINER => sub {
