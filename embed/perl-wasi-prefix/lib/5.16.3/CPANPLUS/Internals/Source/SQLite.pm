...

{
    ...
    sub __sqlite_file {
        ...
        $DbFile =
          File::Spec->catdir( $conf->get_conf('base'), SOURCE_SQLITE_DB );
    ...
    sub __sqlite_dbh {
        ...
        my $self = shift;
        $Dbh =
          DBIx::Simple->connect( "dbi:SQLite:dbname=" . $self->__sqlite_file,
            '', '', { AutoCommit => 1 } );
...

{
    ...
    sub _save_state {
        error(
            loc( "%1 has not implemented writing state to disk", __PACKAGE__ )
        );
        return;
    }
}

{
    ...
    sub _add_author_object {
        ...
        unless ( ++$txn_count % TXN_COMMIT ) {
            $dbh->commit     or error( $dbh->error );
            $dbh->begin_work or error( $dbh->error );
        }
