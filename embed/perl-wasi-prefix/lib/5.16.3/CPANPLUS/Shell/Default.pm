...
sub new {
    ...
    my $rc_file = File::Spec->catfile( $cb->configure_object->get_conf('base'),
...
sub _input_loop {
    ...
    my $normal_quit = 0;
    while ( defined( my $input = eval { $term->readline( $self->prompt ) } )
    ...
    while ( my $input = shift @cmds ) {
        ...
        { { $input =~ s|^\s*([!?/])|$1 |; }
            ...
            if ( $input and $input !~ s/^\s+// ) {
                $self->__print(
                    loc(
                        "Could not understand command: %1\n"
        ...
        if ( $key eq '/' ) {
            $rv *=
              length $self->$method( input => $input, options => $options );
            next;
        }
        ...
        if ( $self->remote ) {
            ...
            if ( $key eq 'z'
                or ( $key eq 's' and $input =~ /^\s*edit/ ) )
            {
                $self->__print(
                    "\n",
                    loc(
                        "Command '%1' not supported over remote connection",
                        join ' ', $key, $input
                    ),
                    "\n\n"
                );
                ...
                my ( $status, $buff ) =
                  $self->__send_remote_command($org_input);
...
sub __display_results {
    ...
    if ( scalar @rv ) {
        ...
        for my $mod (@rv) {
            ...
            if ( $mod->mtime ) {
                $self->__printf( $self->dist_format, $i, $mod->module,
                    $mod->mtime, $self->_format_version( $mod->version ),
                    $mod->author->cpanid );

            }
            else {
                $self->__printf( $self->format, $i, $mod->module,
...

{
    ...
    sub _help {
        ...
        my $input;
        {
            ...
            my $tmpl = { input => { required => 0, store => \$input } };
...
sub _bang {
    ...
    my $input;
    {
        ...
        my $tmpl = { input => { required => 1, store => \$input } };
...
sub _search_module {
    ...
    my @rv = sort { $a->module cmp $b->module } $cb->search(
        %{ $args->{'options'} },
        type  => 'module',
        allow => \@regexes,
    );
...
sub _fetch {
    ...
    for my $mod (@$mods) {
        ...
        $self->__print(
            $where
            ? loc( "Successfully fetched '%1' to '%2'", $mod->module, $where )
...
sub _shell {
    ...
    for my $mod (@$mods) {
        ...
        if ( system($shell) and $! ) {
            $self->__print(
                loc( "Error executing your subshell '%1': %2", $shell, $! ),
                "\n" );
            next;
        }
    }
...
sub _reload_indices {
    ...
    my $opts;
    {
        ...
        my $tmpl = { options => { default => {}, store => \$opts }, };
...
sub _install {
    ...
    for my $mod (@$mods) {
        ...
        if ( $conf->get_conf('write_install_logs') ) {
            ...
            if ( ON_WIN32
                and !check_install( module => 'IPC::Run', version => 0.55 ) )
            {
                error(
                    loc(
                        "IPC::Run version '%1' is required on MSWin32"
                          . " in order to capture buffers."
                          . " The logfile generated may not contain"
                          . " any useful data, until it is installed",
                        0.55
                    )
                );
            }
            ...
            my $file = File::Spec->catfile( $dir, INSTALL_LOG_FILE->($mod) );
            if ( open my $fh, ">$file" ) {
    ...
    for my $mod (@$mods) {
        if ( $status->{$mod} ) {
            $self->__print(
                loc(
                    "Module '%1' %tense(%2,past) successfully\n",
                    $mod->module, $action
                )
            );
        }
    ...
    if ( !$flag ) {
        ...
        $self->__print(
            loc(
                "*** You can view the complete error buffer by pressing "
                  . "'%1' ***\n",
                'p'
            )
...
sub __ask_about_install {
    ...
    $Shell->__print(
        loc(
            "Module '%1' requires '%2' to be installed", $mod->module,
            $prereq->module
        )
    );
    $Shell->__print("\n\n");
...
sub __ask_about_send_test_report {
    ...
    $Shell->__print(
        loc(
            "Test report prepared for module '%1'.\n Would you like to "
              . "send it? (You can edit it if you like)",
            $mod->module
        )
    );
    $Shell->__print("\n\n");
...
sub __ask_about_edit_test_report {
    ...
    $Shell->__print(
        loc(
            "Test report prepared for module '%1'. You can edit this "
...
sub __ask_about_test_failure {
    ...
    $Shell->__print(
        loc(
            "The tests for '%1' failed. Would you like me to proceed "
...
sub _details {
    ...
    for my $mod (@$mods) {
        ...
        unless ($href) {
            $self->__print(
                loc(
                    "No details for %1 - it might be outdated.", $mod->module
                ),
                "\n"
            );
            next;
            ...
            my $showed;
            for my $item (@list) {
                $self->__printf( $cformat, ( $showed ? '' : 'Contains:' ),
...
sub _print {
    ...
    if ($file) {
        $fh = FileHandle->new(">$file")
          or ( warn loc( "Could not open '%1': '%2'", $file, $! ), return );
        $old = select $fh;
    }
...
sub _set_conf {
    ...
    if ( $type eq 'reconfigure' ) {
        ...
        my %valid = map { $_ => $_ } $cb->selfupdate_object->list_categories;

        unless ( $valid{$key} ) {
            $self->__print(
                loc(
                    "To update your current CPANPLUS installation, "
            ...
            my %list =
              $cb->selfupdate_object->list_modules_to_update(%update_args);
        ...
        if ( $type eq 'program' or $type eq 'conf' ) {
            ...
            unless ($key) {
                my @list =
                  grep { $_ ne 'hosts' } $conf->options( type => $type );
...
", $list )
    ...
    for my $mod (@$mods) {
        if ( $mod->status->uninstall ) {
            $self->__print(
                loc(
                    "Module '%1' %tense(uninstall,past) successfully\n",
                    $mod->module
                )
            );
        }
    ...
    if ( !$flag ) {
        ...
        $self->__print(
            loc(
                "*** You can view the complete error buffer by pressing '%1'"
                  . "***\n",
                'p'
            )
...
sub _reports {
    ...
    for my $mod (@$mods) {
        my @list = $mod->fetch_report(%$opts)
          or
          ( print( loc("No reports available for this distribution.") ), next );
        ...
        for my $href (@list) {
            ...
            $self->__printf( $format, $href->{'grade'}, $href->{'platform'},
                ( $href->{'details'} ? '(*)' : '' ) );
        ...
        for my $p ( __PACKAGE__->plugin_modules ) {
            ...
            while ( my ( $name, $func ) = each %map ) {
                ...
                if ( not length $name or not length $func ) {
                    error(
                        loc("Empty plugin name or dispatch function detected")
                    );
                    next;
                }
                ...
                if ( exists( $Dispatch{$name} ) ) {
                    error(
                        loc(
                            "'%1' is already registered by '%2'", $name,
                            $Dispatch{$name}->[0]
                        )
                    );
                    next;
    ...
    sub _list_plugins {
        ...
        for my $name ( sort keys %table ) {
            ...
            my $who =
              ...
              ? "Standard Plugin"
              : do {
                my $v = $self->_format_version( $pkg->VERSION ) || '';
                $pkg =~ s/^$this/../;
...

{
    my @tips = (
        ...
        loc(
            "You can turn off these tips using '%1'",
            's conf show_startup_tip 0'
        ),
        loc(
            "You can use wildcards like '%1' and '%2' on search results",
            '*', '2..5'
        ),
        loc(
            "You can use plugins. Type '%1' to list available plugins",
            '/plugins'
        ),
        loc( "You can show all your out of date modules using '%1'", 'o' ),
        loc(
            "Many operations take options, like '%1', '%2' or '%3'",
            '--verbose', '--force', '--skiptest'
        ),
        loc(
            "The documentation in %1 and %2 is very useful",
            "CPANPLUS::Module",
            "CPANPLUS::Backend"
        ),
        ...
        loc(
            "You can add custom sources to your index. See '%1' for details",
            '/cs --help'
        ),
        loc(
            "CPANPLUS now has an experimental SQLite backend. You can enable "
...
sub _reload_shell {
    { exec( $^X, '-MCPANPLUS', '-e', 'shell' ) };
    print STDERR "couldn't exec foo: $!";
}
