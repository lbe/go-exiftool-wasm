...
sub _dispatch_on_input {
    ...
    my $string;
    my $tmpl = { input => { required => 1, store => \$string } };
...
sub _not_supported {
    ...
    my $cmd;
    my $tmpl = { command => { required => 1, store => \$cmd } };
...
sub _install {
    ...
    my $tmpl = {
        result => { store => \$aref, default => [] },
        command =>
          { required => 1, store => \$cmd, allow => [ keys %$mapping ] },
    };
...
sub _pp_uptodate {
    ...
    for my $mod ( sort { $a->module cmp $b->module } @not_uptodate ) {
        printf $format,
          (
            $mod->module,  $mod->installed_version,
            $mod->version, $mod->path . '/' . $mod->package,
          );
    }
