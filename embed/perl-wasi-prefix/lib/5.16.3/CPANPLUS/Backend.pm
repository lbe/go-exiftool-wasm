    ...
    my $args = do {
        ...
        my $tmpl = {
            type => {
                required => 1,
                allow    => [
                    CPANPLUS::Module->accessors(),
                    CPANPLUS::Module::Author->accessors()
                ],
                store => \$type
            },
...
for my $func (qw[fetch extract install readme files distributions]) {
    ...
    *$func = sub {
        ...
        my %mods =
          map { $_ => $self->parse_module( module => $_ ) || '' } @$mods;
...
sub parse_module {
    ...
    my $mod;
    my $tmpl = { module => { required => 1, store => \$mod }, };
    ...
    my $author;
    if (    ( defined $parts[0] and length $parts[0] == 1 )
        and ( defined $parts[1] and length $parts[1] == 2 )
        and $parts[2] =~ /^$parts[0]/i
        and $parts[2] =~ /^$parts[1]/i )
    {
        splice @parts, 0, 2;
        $author = shift @parts;
    ...
    if ( IS_MODOBJ->( module => $maybe ) ) {
        ...
        if ( $maybe->package eq $mod ) {
            ...
            if ( $maybe->package_name eq $pkg ) {
                ...
                $modobj->package( $maybe->package_name . '-'
                      . $version . '.'
...
sub flush {
    ...
    my $aref = $cache->{$type}
      or ( error( loc( "No such cache '%1'", $type ) ), return );
...
sub local_mirror {
    ...
    unless ( -d $path ) {
        $self->_mkdir( dir => $path )
          or
          ( error( loc( "Could not create '%1', giving up", $path ) ), return );
    }
    elsif ( !-w _ ) {
            ...
            values %{ $self->author_tree } )
        {
          ...
          MODULE: {
                ...
                for my $mod ( $auth->modules ) {
                    ...
                    unless ( $i++ ) {
                        $mod->_get_checksums_file( %opts )
                          or (
                            error(
                                loc(
                                    "Could not fetch %1 file, "
                                      . "skipping author '%2'",
                                    CHECKSUMS,
                                    $auth->cpanid
                                )
...
sub autobundle {
    ...
    unless ( -d $path ) {
        $self->_mkdir( dir => $path )
          or
          ( error( loc( "Could not create directory '%1'", $path ) ), return );
    }
    ...
    } sort { $a->module cmp $b->module } $self->installed;
