...
sub numerate {
    ...
    return '' unless @forms;
    if ( @forms == 1 ) { return $s ? $forms[0] : ( $forms[0] . 's' );
    }
    else { return $s ? $forms[0] : $forms[1];
    }
}
...
sub fail_with { my ( $handle, @params ) = @_;
...
sub maketext {
    ...
    unless ( defined($value) ) {
        ...
        if ( ref($handle) and $handle->{'fail'} ) {
            ...
            my $fail;
            if ( ref( $fail = $handle->{'fail'} ) eq 'CODE' ) { $@ = $at;
...
sub get_handle {  
    ...
    if (@languages) {
        ...
        if ($USING_LANGUAGE_TAGS) { @languages =
    ...
    foreach my $module_name ( map { $base_class . '::' . $_ } @languages ) {
        ...
        next
          if $seen{$module_name}++ || !&_try_use($module_name);
...
sub _try_use {  return $tried{ $_[0] } if exists $tried{ $_[0] };
...
m/[^ !"\#\$%&'()*+,\-.\/0-9:;<=>?\@A-Z[\\\]^_`a-z{|}~\x07]/s
                    ...
                    if ( ord('A') == 65 ) { foreach ( $m, @params ) {
                            tr/\x7F/,/;
                        }
                    }
                    else {  foreach ( $m, @params ) {
                            tr/\x07/,/;
                        }
                    }
                        ...
                        and $m !~ m/(?:^|\:)\d/s )
                    {
                        $target->_die_pointing(
                            $string_to_compile,
                            q{Can't use "SUPER::" in a bracket-group method},
                            2 + length( $c[-1] )
                        ) if $m =~ m/^SUPER::/s;
...
m/[^ !"\#\$%&'()*+,\-.\/0-9:;<=>?\@A-Z[\\\]^_`a-z{|}~\x07]/s
    ...
    if ( @code == 0 ) { DEBUG and warn "Empty code\n";
        return \'';
    }
    elsif ( @code > 1 ) { unshift @code, "join '',\n";
    }
