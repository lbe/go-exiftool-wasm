...
sub extract_delimited (;$$$$) {
    ...
    return _succeed $wantarray, $textref,
      $matchpos, $endpos - $matchpos, $endpos,
      length($$textref) - $endpos, $startpos, $prelen;
}
...
sub extract_bracketed (;$$$) {
    ...
    my $startpos = pos $$textref || 0;
    my @match =
      _match_bracketed( $textref, $pre, $ldel, $qdel, $quotelike, $rdel );

    return _fail( $wantarray, $textref ) unless @match;

    return _succeed(
        $wantarray, $textref, $match[2],
        $match[5] + 2, @match[ 8, 9 ], @match[ 0, 1 ], );
}
...
qq{Mismatched closing bracket: expected "$expected" but found "$found"},
    ...
    return (
        $startpos, $ldelpos - $startpos, $ldelpos, 1, $ldelpos + 1,
        $endpos - $ldelpos - 2, $endpos - 1, 1, $endpos,
        length($$textref) - $endpos, );
}
...
sub _match_tagged {
  ...
  short:
    ...
    return (
        $startpos, $opentagpos - $startpos, $opentagpos,
        $textpos - $opentagpos, $textpos, $parapos - $textpos, $parapos,
        0, $parapos, length($$textref) - $parapos, );

  matched:
    $endpos = pos($$textref);
    return (
        $startpos, $opentagpos - $startpos, $opentagpos,
        $textpos - $opentagpos, $textpos,
        $closetagpos - $textpos, $closetagpos, $endpos - $closetagpos, $endpos,
        length($$textref) - $endpos, );
...
sub _match_variable($$) {
    ...
    while (1) {
        ...
        next
          if _match_codeblock( $textref, qr/\s*->\s*(?:[_a-zA-Z]\w+\s*)?/,
            qr/[({[]/, qr/[)}\]]/, qr/[({[]/, qr/[)}\]]/, 0 );
        next
          if _match_codeblock( $textref, qr/\s*/, qr/[{[]/, qr/[}\]]/,
            qr/[{[]/, qr/[}\]]/, 0 );
...
sub extract_codeblock (;$$$$$) {
    ...
    my @match = _match_codeblock( $textref, $pre, $ldel_outer, $rdel_outer,
...
sub _match_codeblock($$$$$$$) {
    ...
    while ( pos($$textref) < length($$textref) ) {
        ...
        if (
            _match_codeblock(
                $textref,    '\s*',       $ldel_inner, $rdel_inner,
                $ldel_inner, $rdel_inner, $rd
            )
          )
        {
...
sub extract_quotelike (;$$) {
    ...
    return _succeed(
        $wantarray, $textref, $match[2],
        $match[18] -
          $match[2], @match[ 18, 19 ], @match[ 0, 1 ], @match[ 2 .. 17 ]
        , @match[ 20, 21 ], );
}
...
sub _match_quotelike($$$$) {
    ...
    my (
        $textlen, $startpos,  $oppos,  $preld1pos, $ld1pos, $str1pos,
        $rd1pos,  $preld2pos, $ld2pos, $str2pos,   $rd2pos, $modpos
    ...
    {
        unless ( $$textref =~
m/ \Q$initial\E [^\\$initial]* (\\.[^\\$initial]*)* \Q$initial\E /gcsx
          )
        {
        ...
        return (
            ...
            0, $oppos,  1, $oppos + 1, $rd1pos - $oppos - 1, $rd1pos,
            1, $modpos, 0, $modpos,    0,                    $modpos,
            0, $modpos, $endpos - $modpos, $endpos, $textlen - $endpos, );
    }
    ...
    if ( $op eq '<<' ) {
        ...
        $ld2pos = pos($$textref);
        return (
            $startpos, $oppos - $startpos, $oppos, length($op), $ld1pos,
            $extrapos - $ld1pos, $str1pos, $rd1pos - $str1pos, $rd1pos,
            $ld2pos - $rd1pos, $ld2pos, 0, $ld2pos, 0, $ld2pos, 0, $ld2pos,
            0, $ld2pos, $textlen - $ld2pos, $extrapos, $str1pos - $extrapos, );
    }
    ...
    return (
        $startpos, $oppos - $startpos, $oppos, length($op), $ld1pos,
        1, $str1pos, $rd1pos - $str1pos, $rd1pos, 1, $ld2pos,
        $second_arg, $str2pos, $rd2pos - $str2pos, $rd2pos,
        $second_arg, $modpos, $endpos - $modpos, $endpos, $textlen - $endpos, );
}
...
sub gen_extract_tagged {
    ...
    my $closure = sub {
        my $textref = defined $_[0] ? \$_[0] : \$_;
        my @match =
          Text::Balanced::_match_tagged( $textref, $pre, $ldel, $rdel, $omode,
            $bad, $ignore );
