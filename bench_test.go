...
func runHelperProcess(b testing.TB, mode string) {
	b.Helper()
...
func benchmarkProcessCacheCold(b *testing.B, mode string) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		runHelperProcess(b, mode)
	}
}

func benchmarkProcessCacheWarm(b *testing.B, mode string) {
	runHelperProcess(b, mode)
